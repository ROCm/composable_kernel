...
#include "ck_tile_profiler/tile_grouped_conv_fwd_invoker.hpp"
...
bool profile_grouped_conv_forward_impl(const ck_tile::conv::ConvParam& conv_param,
                                       const int split_k)
{
    ...
    using Instance =
        ck_tile::ops::GroupedConvolutionForwardInvoker<NDimSpatial,
                                                       InLayout,
                                                       WeiLayout,
                                                       OutLayout,
                                                       InDataType,
    ...
    if(op.IsSupportedArgument(args))
    {
        ...
        ck_tile::HostTensor<WeiDataType> output_gpu_ref(out_g_n_k_wos_desc);
        output_ref_dev_buf.FromDevice(output_gpu_ref.data());

        return ck_tile::check_err(output, output_gpu_ref);
    }
    else
    {
        return true;
...
TEST(TileGroupedConv, Forward)
{
    ...
    {
        bool passed = profile_grouped_conv_forward_impl<
            2,                                          /*NDimSpatial*/
            ...
            ck_tile::bfloat16_t /*ComputeTypeB*/>(conv_param /*params*/, 1 /*split_k*/);

        passed &= profile_grouped_conv_forward_impl<
            2,                                          /*NDimSpatial*/
            ck_tile::tensor_layout::convolution::NHWGC, /*InLayout*/
            ck_tile::tensor_layout::convolution::GKYXC, /*WeiLayout*/
            ck_tile::tensor_layout::convolution::NHWGK, /*OutLayout*/
