...
{
    return array_to_seq(std::array<size_t, 4>{t.m_block, t.m_wave_per_xdl, t.n_block, t.n_wave_per_xdl});
}
...
{
    std::ostringstream oss;
    oss << to_string(t.block_transfer) << ","
        << to_string(t.block_transfer_access_order) << ","
        << to_string(t.src_access_order) << ","
...
{
    std::ostringstream oss;
    oss << t.epilogue.m_xdl_per_wave_per_shuffle << ","
        << t.epilogue.n_per_wave_per_shuffle << ","
        << to_string(t.thread_cluster_dims) << ","
        << t.epilogue.scalar_per_vector;
    return oss.str();
}
...
{
    std::ostringstream oss;
    oss << array_to_seq(t.m1_xs) << ","
        << array_to_seq(t.n1_xs);
    return oss.str();
}
...
{
    std::ostringstream oss;
    oss << array_to_seq(t.thread_slice_lengths) << ","
        << array_to_seq(t.thread_cluster_lengths) << ","
        << array_to_seq(t.thread_cluster_arrange_order) << ","
        << array_to_seq(t.src_access_order) << ","
        << array_to_seq(t.src_vector_tensor_lengths) << ","
...
{
    std::ostringstream oss;
    oss << to_string(t.fwd_specialization) << ","
    ...
    ConvAlgorithm_DeviceGroupedConvFwdMultipleABD_Xdl_CShuffle t)
{
    ...
    oss << to_string(static_cast<ThreadBlock_>(t)) << ","
        << to_string(static_cast<XdlGemm_>(t)) << ","
    ...
    ConvAlgorithm_DeviceGroupedConvFwdMultipleD_Wmma_CShuffle t)
{
    std::ostringstream oss;
    oss << to_string(static_cast<ThreadBlock_>(t)) << ","
        << to_string(static_cast<WmmaGemm_>(t)) << ","
        << to_string(static_cast<Transfer_>(t));
    return oss.str();
}
