...
consteval auto diagnose_block_transfer(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(BT t) { t.k0; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".k0: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(BT t) { t.m_n; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".m_n: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(BT t) { t.k1; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".k1: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto diagnose_block_transfer_4d(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(BT t) { t.k_batch_size; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".k_batch_size: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto diagnose_lds_transfer(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(LT t) { t.src_vector_dim; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".src_vector_dim: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(LT t) { t.src_scalar_per_vector; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".src_scalar_per_vector: " + std::string(CHECK_MARK(convertible)) +
               (convertible ? "" : std::string(get_type_info<SrcScalarType>())) + "\n";
    }
    else
    {
        msg +=
            std::string("          → ") + prefix + ".src_scalar_per_vector: [✗] (missing member)\n";
    }
    ...
    if constexpr(requires(LT t) { t.lds_dst_scalar_per_vector; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".lds_dst_scalar_per_vector: " + std::string(CHECK_MARK(convertible)) +
               (convertible ? "" : std::string(get_type_info<LdsDstScalarType>())) + "\n";
    }
    else
    {
        msg += std::string("          → ") + prefix +
               ".lds_dst_scalar_per_vector: [✗] (missing member)\n";
    }
    ...
    if constexpr(requires(LT t) { t.is_direct_load; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".is_direct_load: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(LT t) { t.lds_padding; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".lds_padding: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto diagnose_thread_cluster(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(TC t) { t.m_block; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".m_block: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(TC t) { t.m_wave_per_xdl; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".m_wave_per_xdl: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(TC t) { t.n_block; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".n_block: " + std::string(CHECK_MARK(convertible)) +
    ...
    if constexpr(requires(TC t) { t.n_wave_per_xdl; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".n_wave_per_xdl: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto diagnose_access_order(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(AO t) { t.order; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".order: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto diagnose_epilogue(const char* prefix) -> std::string
{
    ...
    if constexpr(requires(E t) { t.m_xdl_per_wave_per_shuffle; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".m_xdl_per_wave_per_shuffle: " + std::string(CHECK_MARK(convertible)) +
               (convertible ? "" : std::string(get_type_info<MXdlType>())) + "\n";
    }
    else
    {
        msg += std::string("          → ") + prefix +
               ".m_xdl_per_wave_per_shuffle: [✗] (missing member)\n";
    }
    ...
    if constexpr(requires(E t) { t.n_per_wave_per_shuffle; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".n_per_wave_per_shuffle: " + std::string(CHECK_MARK(convertible)) +
               (convertible ? "" : std::string(get_type_info<NPerWaveType>())) + "\n";
    }
    else
    {
        msg += std::string("          → ") + prefix +
               ".n_per_wave_per_shuffle: [✗] (missing member)\n";
    }
    ...
    if constexpr(requires(E t) { t.scalar_per_vector; })
    {
        ...
        msg += std::string("          → ") + prefix +
               ".scalar_per_vector: " + std::string(CHECK_MARK(convertible)) +
...
consteval auto detailed_diagnostic_SpecifiesBlockTransfer4D() -> std::string
{
    ...
    {
        msg += detail::diagnose_block_transfer_4d<T, decltype(T::transfer.a.block_transfer)>(
            "transfer.a.block_transfer");
    }
    ...
    {
        msg += detail::diagnose_block_transfer_4d<T, decltype(T::transfer.b.block_transfer)>(
            "transfer.b.block_transfer");
    }
...
consteval auto detailed_diagnostic_SpecifiesThreadClusterAccessOrder() -> std::string
{
    ...
    {
        msg +=
            detail::diagnose_access_order<T, decltype(T::transfer.a.block_transfer_access_order)>(
                "transfer.a.block_transfer_access_order");
    }
    ...
    {
        msg +=
            detail::diagnose_access_order<T, decltype(T::transfer.b.block_transfer_access_order)>(
                "transfer.b.block_transfer_access_order");
    }
...
consteval auto detailed_diagnostic_SpecifiesSourceAccessOrder() -> std::string
{
    ...
    {
        msg += detail::diagnose_access_order<T, decltype(T::transfer.a.src_access_order)>(
            "transfer.a.src_access_order");
    }
    else if constexpr(has_a)
    {
    ...
    {
        msg += detail::diagnose_access_order<T, decltype(T::transfer.b.src_access_order)>(
            "transfer.b.src_access_order");
    }
    else if constexpr(has_b)
    {
...
consteval auto detailed_diagnostic_SpecifiesBlockGemm() -> std::string
{
    ...
    if constexpr(requires { T::block_gemm_pipeline.pipeline_version; })
    {
        ...
        msg += "      → block_gemm_pipeline.pipeline_version: " +
               std::string(CHECK_MARK(convertible)) +
...
consteval auto detailed_diagnostic_SpecifiesLargeTensorSupport() -> std::string
{
    ...
    if constexpr(requires { T::specialization; })
    {
        ...
        {
            constexpr bool is_large_tensor =
                (T::specialization == ConvAlgorithmSpecialization::LARGE_TENSOR);
            msg += "      → specialization == LARGE_TENSOR: " +
                   std::string(CHECK_MARK(is_large_tensor)) + "\n";
        }
    }
    else
    {
    ...
    if constexpr(requires { T::specialization; })
    {
        ...
        if constexpr(convertible)
        {
            constexpr bool is_reference =
                (T::specialization == ConvAlgorithmSpecialization::REFERENCE);
...
consteval auto detailed_diagnostic_SpecifiesTwoStageSupport() -> std::string
{
    ...
    if constexpr(requires { T::specialization; })
    {
        ...
        if constexpr(convertible)
        {
            constexpr bool is_two_stage =
                (T::specialization == ConvAlgorithmSpecialization::TWO_STAGE);
...
consteval auto detailed_diagnostic_SpecifiesMultipleDSupport() -> std::string
{
    ...
    if constexpr(requires { T::specialization; })
    {
        ...
        if constexpr(convertible)
        {
            constexpr bool is_multiple_d =
                (T::specialization == ConvAlgorithmSpecialization::MULTIPLE_D);
            msg +=
                "      → specialization == MULTIPLE_D: " + std::string(CHECK_MARK(is_multiple_d)) +
                "\n";
        }
    }
    else
...
consteval auto detailed_diagnostic_TransposeTransferWellDefinedIfProvided() -> std::string
{
    ...
    else
    {
        ...
        {
            msg += "          → T::max_transpose_transfer_src_scalar_per_vector: [✗] (missing, but "
                   "dst is provided)\n";
        }
        ...
        {
            msg += "          → T::max_transpose_transfer_dst_scalar_per_vector: [✗] (missing, but "
                   "src is provided)\n";
        }
    }
...
{
    std::string msg;
    constexpr bool has_gridwise_gemm = requires(T t) {
    ...
    using GG              = decltype(T::gridwise_gemm);
    constexpr bool has_k1 = requires(GG t) {
        { t.k1 } -> std::convertible_to<size_t>;
    };
    constexpr bool has_m_per_wmma = requires(GG t) {
        { t.m_per_wmma } -> std::convertible_to<size_t>;
    };
    constexpr bool has_n_per_wmma = requires(GG t) {
        { t.n_per_wmma } -> std::convertible_to<size_t>;
    };
    constexpr bool has_m_wmma_per_wave = requires(GG t) {
        { t.m_wmma_per_wave } -> std::convertible_to<size_t>;
    };
    constexpr bool has_n_wmma_per_wave = requires(GG t) {
        { t.n_wmma_per_wave } -> std::convertible_to<size_t>;
    };

    msg += "      → gridwise_gemm.k1: " + std::string(CHECK_MARK(has_k1)) +
           (has_k1 ? "\n" : " (missing or wrong type)\n");
    msg += "      → gridwise_gemm.m_per_wmma: " + std::string(CHECK_MARK(has_m_per_wmma)) +
           (has_m_per_wmma ? "\n" : " (missing or wrong type)\n");
    msg += "      → gridwise_gemm.n_per_wmma: " + std::string(CHECK_MARK(has_n_per_wmma)) +
           (has_n_per_wmma ? "\n" : " (missing or wrong type)\n");
    msg +=
        "      → gridwise_gemm.m_wmma_per_wave: " + std::string(CHECK_MARK(has_m_wmma_per_wave)) +
        (has_m_wmma_per_wave ? "\n" : " (missing or wrong type)\n");
    msg +=
        "      → gridwise_gemm.n_wmma_per_wave: " + std::string(CHECK_MARK(has_n_wmma_per_wave)) +
        (has_n_wmma_per_wave ? "\n" : " (missing or wrong type)\n");
...
consteval auto detailed_diagnostic_SpecifiesTileThreadBlock() -> std::string
{
    ...
    else
    {
        ...
        constexpr bool has_tile_m = requires(TB t) {
            { t.tile_size.m } -> std::convertible_to<size_t>;
        };
        constexpr bool has_tile_n = requires(TB t) {
            { t.tile_size.n } -> std::convertible_to<size_t>;
        };
        constexpr bool has_tile_k = requires(TB t) {
...
consteval auto detailed_diagnostic_SpecifiesTileTransfer() -> std::string
{
    ...
    constexpr bool has_a_scalar = requires {
        { T::transfer.a_scalar_per_vector } -> std::convertible_to<size_t>;
    };
    constexpr bool has_b_scalar = requires {
        { T::transfer.b_scalar_per_vector } -> std::convertible_to<size_t>;
    };
    constexpr bool has_c_scalar = requires {
        { T::transfer.c_scalar_per_vector } -> std::convertible_to<size_t>;
    };

    msg += "      → transfer.a_scalar_per_vector: " + std::string(CHECK_MARK(has_a_scalar)) +
           (has_a_scalar ? "\n" : " (missing or wrong type)\n");
    msg += "      → transfer.b_scalar_per_vector: " + std::string(CHECK_MARK(has_b_scalar)) +
           (has_b_scalar ? "\n" : " (missing or wrong type)\n");
    msg += "      → transfer.c_scalar_per_vector: " + std::string(CHECK_MARK(has_c_scalar)) +
           (has_c_scalar ? "\n" : " (missing or wrong type)\n");
...
{
    std::string msg;
    constexpr bool has_block_gemm = requires {
        { T::block_gemm } -> TileBlockGemmDescriptor;
    };
    ...
    constexpr bool has_warps_m = requires(BG t) {
        { t.warps.m } -> std::convertible_to<int>;
    };
    constexpr bool has_warps_n = requires(BG t) {
        { t.warps.n } -> std::convertible_to<int>;
    };
    constexpr bool has_warps_k = requires(BG t) {
        { t.warps.k } -> std::convertible_to<int>;
    };
    constexpr bool has_warp_tile_m = requires(BG t) {
        { t.warp_tile.m } -> std::convertible_to<int>;
    };
    constexpr bool has_warp_tile_n = requires(BG t) {
        { t.warp_tile.n } -> std::convertible_to<int>;
    };
    constexpr bool has_warp_tile_k = requires(BG t) {
        { t.warp_tile.k } -> std::convertible_to<int>;
    };
    constexpr bool has_double_smem = requires(BG t) {
        { t.double_smem_buffer } -> std::convertible_to<bool>;
    };
    constexpr bool has_num_wave_groups = requires(BG t) {
        { t.num_wave_groups } -> std::convertible_to<int>;
    };
    constexpr bool has_pipeline = requires(BG t) {
        { t.pipeline_version } -> std::convertible_to<PipelineVersion>;
    };
    constexpr bool has_scheduler = requires(BG t) {
        { t.scheduler } -> std::convertible_to<PipelineScheduler>;
    };

    msg += "      → block_gemm.warps.m: " + std::string(CHECK_MARK(has_warps_m)) +
           (has_warps_m ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.warps.n: " + std::string(CHECK_MARK(has_warps_n)) +
           (has_warps_n ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.warps.k: " + std::string(CHECK_MARK(has_warps_k)) +
           (has_warps_k ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.warp_tile.m: " + std::string(CHECK_MARK(has_warp_tile_m)) +
           (has_warp_tile_m ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.warp_tile.n: " + std::string(CHECK_MARK(has_warp_tile_n)) +
           (has_warp_tile_n ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.warp_tile.k: " + std::string(CHECK_MARK(has_warp_tile_k)) +
           (has_warp_tile_k ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.double_smem_buffer: " + std::string(CHECK_MARK(has_double_smem)) +
           (has_double_smem ? "\n" : " (missing or wrong type)\n");
    msg += "      → block_gemm.num_wave_groups: " + std::string(CHECK_MARK(has_num_wave_groups)) +
           (has_num_wave_groups ? "\n" : " (missing or wrong type)\n");
...
{
    std::string msg;
    constexpr bool has_optimizations = requires {
        { T::optimizations } -> TileOptimizationsDescriptor;
    };
    ...
    constexpr bool has_num_groups = requires(OPT t) {
        { t.num_groups_to_merge } -> std::convertible_to<int>;
    };
    constexpr bool has_split_image = requires(OPT t) {
        { t.split_image } -> std::convertible_to<bool>;
    };
    constexpr bool has_explicit_gemm = requires(OPT t) {
        { t.explicit_gemm } -> std::convertible_to<bool>;
    };

    msg += "      → optimizations.num_groups_to_merge: " + std::string(CHECK_MARK(has_num_groups)) +
           (has_num_groups ? "\n" : " (missing or wrong type)\n");
    msg += "      → optimizations.split_image: " + std::string(CHECK_MARK(has_split_image)) +
           (has_split_image ? "\n" : " (missing or wrong type)\n");
    msg += "      → optimizations.explicit_gemm: " + std::string(CHECK_MARK(has_explicit_gemm)) +
           (has_explicit_gemm ? "\n" : " (missing or wrong type)\n");
...
{
    std::string msg;
    constexpr bool has_thread_config = requires {
    ...
    constexpr bool has_k0 = requires(TC t) {
        { t.k0_per_block } -> std::convertible_to<size_t>;
    };
    constexpr bool has_k1 = requires(TC t) {
        { t.k1 } -> std::convertible_to<size_t>;
    };
    constexpr bool has_m1 = requires(TC t) {
        { t.m1_per_thread } -> std::convertible_to<size_t>;
    };
    constexpr bool has_n1 = requires(TC t) {
        { t.n1_per_thread } -> std::convertible_to<size_t>;
    };
    constexpr bool has_k = requires(TC t) {
        { t.k_per_thread } -> std::convertible_to<size_t>;
    };

    msg += "      → thread_config.k0_per_block: " + std::string(CHECK_MARK(has_k0)) +
           (has_k0 ? "\n" : " (missing or wrong type)\n");
    msg += "      → thread_config.k1: " + std::string(CHECK_MARK(has_k1)) +
           (has_k1 ? "\n" : " (missing or wrong type)\n");
    msg += "      → thread_config.m1_per_thread: " + std::string(CHECK_MARK(has_m1)) +
           (has_m1 ? "\n" : " (missing or wrong type)\n");
    msg += "      → thread_config.n1_per_thread: " + std::string(CHECK_MARK(has_n1)) +
           (has_n1 ? "\n" : " (missing or wrong type)\n");
    msg += "      → thread_config.k_per_thread: " + std::string(CHECK_MARK(has_k)) +
           (has_k ? "\n" : " (missing or wrong type)\n");
...
{
    std::string msg;
    constexpr bool has_thread_cluster = requires {
        { T::thread_cluster } -> DlThreadClusterDescriptor;
    };
    msg +=
        "      → T::thread_cluster member: " + std::string(CHECK_MARK(has_thread_cluster)) + "\n";
    ...
    constexpr bool has_m1_xs = requires(TC t) {
        { t.m1_xs } -> std::convertible_to<std::array<size_t, 2>>;
    };
    constexpr bool has_n1_xs = requires(TC t) {
        { t.n1_xs } -> std::convertible_to<std::array<size_t, 2>>;
    };

    msg += "      → thread_cluster.m1_xs: " + std::string(CHECK_MARK(has_m1_xs)) +
           (has_m1_xs ? "\n" : " (missing or wrong type)\n");
    msg += "      → thread_cluster.n1_xs: " + std::string(CHECK_MARK(has_n1_xs)) +
           (has_n1_xs ? "\n" : " (missing or wrong type)\n");
...
consteval auto detailed_diagnostic_SpecifiesDlFwdBlockTransfer() -> std::string
{
    ...
    if constexpr(has_a)
    {
        ...
        constexpr bool has_thread_slice = requires(ABT t) {
            { t.thread_slice_lengths } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_thread_cluster = requires(ABT t) {
            { t.thread_cluster_lengths } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_cluster_arrange = requires(ABT t) {
            { t.thread_cluster_arrange_order } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_src_access = requires(ABT t) {
            { t.src_access_order } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_src_vector = requires(ABT t) {
            { t.src_vector_tensor_lengths } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_src_contiguous = requires(ABT t) {
            {
                t.src_vector_tensor_contiguous_dim_order
            } -> std::convertible_to<std::array<size_t, 4>>;
        };
        constexpr bool has_dst_vector = requires(ABT t) {
            { t.dst_vector_tensor_lengths } -> std::convertible_to<std::array<size_t, 4>>;
        };

        msg += "          → transfer.a.thread_slice_lengths (4D): " +
               std::string(CHECK_MARK(has_thread_slice)) +
               (has_thread_slice ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.thread_cluster_lengths (4D): " +
               std::string(CHECK_MARK(has_thread_cluster)) +
               (has_thread_cluster ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.thread_cluster_arrange_order (4D): " +
               std::string(CHECK_MARK(has_cluster_arrange)) +
               (has_cluster_arrange ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_access_order (4D): " +
               std::string(CHECK_MARK(has_src_access)) +
               (has_src_access ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_vector_tensor_lengths (4D): " +
               std::string(CHECK_MARK(has_src_vector)) +
               (has_src_vector ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_vector_tensor_contiguous_dim_order (4D): " +
               std::string(CHECK_MARK(has_src_contiguous)) +
               (has_src_contiguous ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.dst_vector_tensor_lengths (4D): " +
               std::string(CHECK_MARK(has_dst_vector)) +
               (has_dst_vector ? "\n" : " (missing or wrong type)\n");
    }
    else
    {
...
consteval auto detailed_diagnostic_SpecifiesDlBwdBlockTransfer() -> std::string
{
    ...
    if constexpr(has_a)
    {
        using ABT                       = decltype(T::transfer.a);
        constexpr bool has_thread_slice = requires(ABT t) {
            { t.thread_slice_lengths } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_thread_cluster = requires(ABT t) {
            { t.thread_cluster_lengths } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_cluster_arrange = requires(ABT t) {
            { t.thread_cluster_arrange_order } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_src_access = requires(ABT t) {
            { t.src_access_order } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_src_vector = requires(ABT t) {
            { t.src_vector_tensor_lengths } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_src_contiguous = requires(ABT t) {
            {
                t.src_vector_tensor_contiguous_dim_order
            } -> std::convertible_to<std::array<size_t, 5>>;
        };
        constexpr bool has_dst_vector = requires(ABT t) {
            { t.dst_vector_tensor_lengths } -> std::convertible_to<std::array<size_t, 5>>;
        };

        msg += "          → transfer.a.thread_slice_lengths (5D): " +
               std::string(CHECK_MARK(has_thread_slice)) +
               (has_thread_slice ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.thread_cluster_lengths (5D): " +
               std::string(CHECK_MARK(has_thread_cluster)) +
               (has_thread_cluster ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.thread_cluster_arrange_order (5D): " +
               std::string(CHECK_MARK(has_cluster_arrange)) +
               (has_cluster_arrange ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_access_order (5D): " +
               std::string(CHECK_MARK(has_src_access)) +
               (has_src_access ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_vector_tensor_lengths (5D): " +
               std::string(CHECK_MARK(has_src_vector)) +
               (has_src_vector ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.src_vector_tensor_contiguous_dim_order (5D): " +
               std::string(CHECK_MARK(has_src_contiguous)) +
               (has_src_contiguous ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.a.dst_vector_tensor_lengths (5D): " +
               std::string(CHECK_MARK(has_dst_vector)) +
               (has_dst_vector ? "\n" : " (missing or wrong type)\n");
    }
    else
...
consteval auto detailed_diagnostic_SpecifiesDlEpilogue() -> std::string
{
    ...
    if constexpr(has_c && requires { T::transfer.c.src_dst_access_order; })
    {
        ...
        constexpr bool has_src_dst_access = requires(C t) {
            { t.src_dst_access_order } -> std::convertible_to<std::array<size_t, 6>>;
        };
        constexpr bool has_src_dst_vector_dim = requires(C t) {
            { t.src_dst_vector_dim } -> std::convertible_to<size_t>;
        };
        constexpr bool has_dst_scalar = requires(C t) {
            { t.dst_scalar_per_vector } -> std::convertible_to<size_t>;
        };

        msg += "          → transfer.c.src_dst_access_order: " +
               std::string(CHECK_MARK(has_src_dst_access)) +
               (has_src_dst_access ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.c.src_dst_vector_dim: " +
               std::string(CHECK_MARK(has_src_dst_vector_dim)) +
               (has_src_dst_vector_dim ? "\n" : " (missing or wrong type)\n");
        msg += "          → transfer.c.dst_scalar_per_vector: " +
               std::string(CHECK_MARK(has_dst_scalar)) +
               (has_dst_scalar ? "\n" : " (missing or wrong type)\n");
    }
