...
template <int SPATIAL_DIM>
FilterExtent<SPATIAL_DIM> filter_extent_from_vector(const std::vector<std::size_t>& vec);

template <>
FilterExtent<1> filter_extent_from_vector<1>(const std::vector<std::size_t>& vec)
{
    return FilterExtent<1>{.width = vec[0]};
}

template <>
FilterExtent<2> filter_extent_from_vector<2>(const std::vector<std::size_t>& vec)
{
    return FilterExtent<2>{.width = vec[1], .height = vec[0]};
}

template <>
FilterExtent<3> filter_extent_from_vector<3>(const std::vector<std::size_t>& vec)
{
