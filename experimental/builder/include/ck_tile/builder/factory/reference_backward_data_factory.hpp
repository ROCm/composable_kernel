...
// Factory for GPU Reference Backward Data Convolution
...
struct ReferenceBackwardDataFactory
{
    ...
    struct Instance
    {
        ...
        // Use common invoker with backward data convolution lambda
        struct Invoker
        {
        ...
        static void Run(InDataType* input_grad,
        ...
        {
            ck_tile::naive_grouped_conv_bwd_data<SPATIAL_DIM, InDataType, WeiDataType, OutDataType>(
                input_grad,
                weight,
                output_grad,
                G,
        ...
        {
            return std::string("GPU_Reference_BackwardData_") + std::to_string(SPATIAL_DIM) + "D";
        }

        // Old CK interface: Create argument pointer
        std::unique_ptr<Argument>
        MakeArgumentPointer(InDataType* input_grad,
                            const WeiDataType* weight,
                            const OutDataType* output_grad,
                            int G,
                            int N,
        ...
        {
            return std::make_unique<Argument>(input_grad,
                                              weight,
                                              output_grad,
                                              G,
                                              N,
