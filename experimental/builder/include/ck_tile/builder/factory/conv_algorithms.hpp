...
struct ReferenceAlgorithm
{
    ...
    static consteval bool is_valid() { return c1 && c2; }
...
struct FwdXdlV3Algorithm
{
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== Forward XDL V3 Algorithm Diagnostic (closest match) ===\n"
               ...
               DIAGNOSTIC_LINE(ConvAlgorithmDescriptor) + DIAGNOSTIC_LINE(SpecifiesThreadBlock) +
               DIAGNOSTIC_LINE(SpecifiesBlockTransfer) + DIAGNOSTIC_LINE(SpecifiesLdsTransfer) +
               ...
               DIAGNOSTIC_LINE(SpecifiesGemmSpecialization) + DIAGNOSTIC_LINE(SpecifiesBlockGemm);
    }
};
...
struct FwdXdlAlgorithmBase
{
    ...
    static consteval auto message() -> std::string
    {
        return DIAGNOSTIC_LINE(ConvAlgorithmDescriptor) + DIAGNOSTIC_LINE(SpecifiesThreadBlock) +
               DIAGNOSTIC_LINE(SpecifiesBlockTransfer) + DIAGNOSTIC_LINE(SpecifiesLdsTransfer) +
               DIAGNOSTIC_LINE(SpecifiesThreadClusterAccessOrder) +
               DIAGNOSTIC_LINE(SpecifiesSourceAccessOrder) +
               DIAGNOSTIC_LINE(SpecifiesGridwiseFwdXdlGemm) +
...
struct FwdXdlAlgorithm : public FwdXdlAlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c13 && FwdXdlAlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Forward XDL Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for FwdXdl Algorithm:\n") +
               FwdXdlAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesGenericInstance);
    }
};
...
struct FwdDlAlgorithm
{
    ...
    static consteval bool is_valid() { return c1 && c2 && c3 && c4 && c5 && c6 && c7 && c8; }
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== Forward DL Algorithm Diagnostic (closest match) ===\n"
               ...
               DIAGNOSTIC_LINE(SpecifiesDlFwdBlockTransfer) + DIAGNOSTIC_LINE(SpecifiesDlEpilogue);
    }
};
...
struct TileAlgorithm
{
    ...
    static consteval bool is_valid() { return c1 && c2 && c3 && c4 && c5 && c6; }
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== CK Tile Algorithm Diagnostic (closest match) ===\n"
               ...
               DIAGNOSTIC_LINE(ConvAlgorithmDescriptor) +
               DIAGNOSTIC_LINE(SpecifiesTileThreadBlock) + DIAGNOSTIC_LINE(SpecifiesTileTransfer) +
...
struct LargeTensorAlgorithm : public FwdXdlAlgorithmBase<T>
{
    ...
    static consteval auto message() -> std::string
    {
        return std::string(
               ...
               FwdXdlAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesLargeTensorSupport);
    }
};
...
struct BwdXdlAlgorithmBase
{
    ...
    {
        return DIAGNOSTIC_LINE(ConvAlgorithmDescriptor) + DIAGNOSTIC_LINE(SpecifiesThreadBlock) +
               DIAGNOSTIC_LINE(SpecifiesBlockTransfer4D) + DIAGNOSTIC_LINE(SpecifiesLdsTransfer) +
               DIAGNOSTIC_LINE(SpecifiesThreadClusterAccessOrder) +
               DIAGNOSTIC_LINE(SpecifiesSourceAccessOrder) +
...
struct BwdXdlAlgorithm : public BwdXdlAlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c9 && c10 && BwdXdlAlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward XDL Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdXdl Algorithm:\n") +
               BwdXdlAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesTransposeTransfer) +
...
struct BwdMultiDXdlAlgorithm : public BwdXdlAlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c9 && BwdXdlAlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward XDL Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdXdl Algorithm:\n") +
               BwdXdlAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesMultipleDSupport);
    }
};
...
struct BwdXdlV3AlgorithmBase
{
    ...
    static consteval bool is_valid() { return c1 && c2 && c3 && c4 && c5 && c6 && c7 && c8 && c9; }
...
struct BwdXdlV3Algorithm : public BwdXdlV3AlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c10 && BwdXdlV3AlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward XDL V3 Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdXdlV3 Algorithm:\n") +
               BwdXdlV3AlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesGenericInstance);
    }
};
...
struct BwdTwoStageXdlAlgorithm : public BwdXdlV3AlgorithmBase<T>
{
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward two stage XDL Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdXdlV3 Algorithm:\n") +
               BwdXdlV3AlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesTransposeTransfer) +
...
struct BwdWmmaAlgorithm : public BwdWmmaAlgorithmBase<T>
{
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward WMMA Algorithm Diagnostic (closest match) ===\n"
               ...
               BwdWmmaAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesNumPrefetchStages) +
               DIAGNOSTIC_LINE(SpecifiesLoopScheduler) +
               DIAGNOSTIC_LINE(SpecifiesGridwiseGemmPipeline) +
               DIAGNOSTIC_LINE(SpecifiesGenericInstance);
    }
...
struct BwdMultiDWmmaV3Algorithm : public BwdWmmaV3AlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c10 && BwdWmmaAlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward WMMA Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdMultiDWmma Algorithm:\n") +
               BwdWmmaAlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesMultipleDSupport);
    }
};
...
struct BwdWmmaV3Algorithm : public BwdWmmaV3AlgorithmBase<T>
{
    ...
    static consteval bool is_valid() { return c10 && c11 && BwdWmmaV3AlgorithmBase<T>::is_valid(); }

    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward WMMA V3 Algorithm Diagnostic (closest match) ===\n"
                           "Concepts for BwdWmmaV3 Algorithm:\n") +
               BwdWmmaV3AlgorithmBase<T>::message() + DIAGNOSTIC_LINE(SpecifiesTransposeTransfer) +
               DIAGNOSTIC_LINE(SpecifiesGenericInstance);
    }
};
...
struct BwdDlAlgorithm
{
    ...
    static consteval bool is_valid() { return c1 && c2 && c3 && c4 && c5 && c6 && c7; }
    ...
    static consteval auto message() -> std::string
    {
        return std::string("\n=== Backward DL Algorithm Diagnostic (closest match) ===\n"
               ...
               DIAGNOSTIC_LINE(SpecifiesDlThreadConfig) +
               DIAGNOSTIC_LINE(SpecifiesDlThreadCluster) +
               DIAGNOSTIC_LINE(SpecifiesDlBwdBlockTransfer) + DIAGNOSTIC_LINE(SpecifiesDlEpilogue);
    }
};
...
consteval int count_matches_fwd_xdl()
{
    ...
    return Alg::c1 + Alg::c2 + Alg::c3 + Alg::c4 + Alg::c5 + Alg::c6 + Alg::c7 + Alg::c8 + Alg::c9 +
           Alg::c10 + Alg::c11 + Alg::c12 + Alg::c13;
}
...
consteval void diagnose_fwd_algorithm_signature()
{
    ...
    else
    {
        ...
        else
        {
            ...
            static_assert(false,
                          "Internal Error: No matching algorithm variant found for diagnostics.");
        }
    }
}
...
consteval void diagnose_bwd_weight_algorithm_signature()
{
    ...
    {
        constexpr int max_1       = wmma_v3_matches > two_stage_wmma_v3_matches
                                        ? wmma_v3_matches
                                        : two_stage_wmma_v3_matches;
