...
TileDistribution abstracts the mapping between logical problem coordinates and physical processing resources. Given a thread's position in the GPU's thread hierarchy (specified by warp ID and lane ID within the warp), TileDistribution computes two pieces of information: the global memory addresses that this thread should access, and the specific access pattern that ensures efficient memory transactions. This abstraction is implemented in C++ through the following core structure:
...
The following shows how to create and use a TileDistribution in practice:
...
.. code-block:: cpp
   ...
   int main()
   {
       ...
       std::cout << "Kernel completed. Average time: " << ave_time << " ms" << std::endl;
...
TileDistribution generates optimal memory access patterns. The encoding parameters control how threads access memory:
...
Continue to :ref:`ck_tile_terminology` to understand the concepts and terminology used in CK Tile.
