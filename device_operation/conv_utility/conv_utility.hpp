#ifndef CONV_UTILITY_HPP
#define CONV_UTILITY_HPP
...
ComputeOutputSpatialLengthsOfConvFwd(std::vector<ck::index_t> input_spatial_lengths,
                                     ...
                                     std::vector<ck::index_t> in_right_pads)
{
    if(input_spatial_lengths.size() == 2)
    {
        ...
        const index_t Ho = (Hi + in_left_pads[0] + in_right_pads[0] - YEff) / conv_strides[0] + 1;
        const index_t Wo = (Wi + in_left_pads[1] + in_right_pads[1] - XEff) / conv_strides[1] + 1;
