...
{
    static_assert(std::is_same<inType, compType>::value, "inType and compType should be the same to use add_device_reduce_instance_blockwise_second_call!"); 
    ...
    static_for<0, std::tuple_size<reduce_configuration_1_instances>::value, 1>{}( [&](auto i) {
          ...
          static_for<0, std::tuple_size<reduce_configuration_2_instances>::value, 1>{}( [&](auto j) {
              ...
              using ReduceOpInstance = DeviceReduceBlockWiseSecondCall<inType, compType, outType, rank, toReduceDims, reduceOp, nanOpt, indicesOpt, 
