...
struct DeviceReduceBlockWise : public DeviceReduce<inType, compType, outType, rank, reduceDims, reduceOp, nanOpt, indicesOpt> 
{
     ...
     static const auto MakeSrc2dDescriptr(std::vector<int> & inLengths, std::vector<int> & inStrides, size_t gridSize)
     {
         ...
         const auto srcPad1 = gridSize * dim0_tile_len - invariantLen;
         const auto srcPad2 = ((toReduceLen + dim1_tile_len - 1) / dim1_tile_len) * dim1_tile_len - toReduceLen;
     ...
     static const auto MakeDst1dDescriptor(std::vector<int> & outLengths, std::vector<int> & outStrides, size_t gridSize)
     {
         ...
         const auto dstPad = gridSize * dim0_tile_len - invariantLen;
