...
struct DeviceReduceMultiBlockAtomicAdd : public DeviceReduce<inType, compType, outType, rank, toReduceDims, reduceOp, nanOpt, indicesOpt>
{
     ...
     static constexpr bool support_AtomicAdd = std::is_same<outType, float>::value && std::is_same<outType, double>::value; 
     ...
     static auto MakeSrc2dDescriptor(const std::vector<int> & inLengths, const std::vector<int> & inStrides, size_t gridSize, int blkGroupSize)
     {
         ...
         const int reduceSizePerBlock = (((toReduceLen + blkGroupSize - 1) / blkGroupSize + dim1_tile_size - 1) / dim1_tile_size) * dim1_tile_size;
     ...
     static auto MakeDst1dDescriptor(const std::vector<int> & outLengths, const std::vector<int> & outStrides, size_t gridSize, int blkGroupSize)
     {
     ...
     struct Argument : public BaseArgument
     {
	     ...
	     while (true) {
                    int test_blkGroupSize =
                        (dim1_total_length + (dim1_thread_cluster_size * dim1_thread_slice_size * iteration) - 1) /
             ...
             blkGroupSize = (dim1_total_length + (dim1_tile_size * iteration) - 1) / (dim1_tile_size * iteration);
     ...
     struct Invoker : public BaseInvoker
     {
         ...
         {
             const auto src2dDesc = DeviceReduceMultiBlockAtomicAdd::MakeSrc2dDescriptor(arg.inLengths_, arg.inStrides_, arg.gridSize, arg.blkGroupSize);
             const auto dst1dDesc = DeviceReduceMultiBlockAtomicAdd::MakeDst1dDescriptor(arg.outLengths_, arg.outStrides_, arg.gridSize, arg.blkGroupSize);
             using src2dDescType = decltype(src2dDesc);
             using dst1dDescType = decltype(dst1dDesc);

             using gridwise_reduce = GridwiseReduction_xy_to_x_multiblock_atomic_add<inType, outType, compType, src2dDescType, dst1dDescType,
                                                 reduceOp, nanOpt, blockSize, dim0_thread_cluster_size, dim1_thread_cluster_size,
	     ...
	     const auto kernel_main = kernel_reduce_multiblock_atocmi_add<gridwise_reduce, inType, outType, src2dDescType, dst1dDescType>;


             avg_time_pre = launch_and_time_kernel(kernel_pre, nrepeat, 
             ...
             avg_time_main = launch_and_time_kernel(kernel_main, nrepeat,
                                                  dim3(arg.gridSize),
                                                  dim3(blockSize),
                                                  0,
                                                  src2dDesc,
