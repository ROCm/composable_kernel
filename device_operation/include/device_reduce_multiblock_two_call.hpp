...
struct DeviceReduceMultiBlockTwoCall : public DeviceReduce<inType, compType, outType, rank, toReduceDims, reduceOp, nanOpt, indicesOpt>		      
{
     ...
     size_t getWorkspaceSize(const std::vector<int> & inLengths) override
     {
          ...
          int iteration = 1;
          while (true) {
              int test_blkGroupSize = (dim1_total_length + (dim1_tile_size * iteration) - 1) / (dim1_tile_size * iteration);
          ...
          int blkGroupSize = (dim1_total_length + (dim1_tile_size * iteration) - 1) / (dim1_tile_size * iteration);
     ...
     struct Argument : public BaseArgument
     {
            ...
            : in_dev_{in_dev}, out_dev_{out_dev}, out_indices_dev_{out_indices_dev}, workspace_dev_{workspace_dev}
         {
             ...
             if constexpr(need_indices) 
 	       	 workspace_indices_dev_ = reinterpret_cast<int*>( reinterpret_cast<char *>(workspace_dev_) + ws_buf2_bytes_offset ); 
             else 
         ...
         struct ClusterAssign
	 {
		...
		>;
         ...
         {
             const auto src2dDesc = DeviceReduceMultiBlockTwoCall::MakeSrc2dDescriptor(arg.inLengths_, arg.inStrides_, arg.gridSize, arg.blkGroupSize);
             const auto ws2dDesc = DeviceReduceMultiBlockTwoCall::MakeWorkspace2dDescriptor(arg.dim0_total_length, arg.blkGroupSize); 
	     ...
	     const auto kernel = kernel_reduce_multiblock_two_call<gridwise_reduce, RunId, inType, compType, src2dDescType, ws2dDescType>;
