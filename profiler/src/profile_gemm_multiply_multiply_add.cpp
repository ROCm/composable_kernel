...
#include "profiler/profile_gemm_multiply_multiply_add_impl.hpp"
...
#define OP_NAME "gemm_multiply_multiply_add"
#define OP_DESC "GEMM_Multiply_Multiply_ADD"

int profile_gemm_multiply_multiply_add(int argc, char* argv[])
{
    if(argc != 16 && argc != 20)
    {
    ...
    auto profile = [&](auto a_type,
        ...
        using D2DataType      = decltype(d2_type);
        ...
        using D2Layout = decltype(d2_layout);
        ...
        const int DefaultStrideD2 = ck::is_same_v<D2Layout, Row> ? N : M;
        const int DefaultStrideE  = ck::is_same_v<ELayout, Row> ? N : M;

        bool pass = ck::profiler::profile_gemm_multiply_multiply_add_impl<ADataType,
                                                                          BDataType,
                                                                          ComputeDataType,
                                                                          AccDataType,
                                                                          D0DataType,
            ...
            (StrideD1 < 0) ? DefaultStrideD2 : StrideD1,
...
REGISTER_PROFILER_OPERATION(OP_NAME, OP_DESC, profile_gemm_multiply_multiply_add);
