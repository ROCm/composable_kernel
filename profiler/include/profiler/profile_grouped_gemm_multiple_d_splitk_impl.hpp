...
#include "ck/library/tensor_operation_instance/gpu/grouped_gemm_multiple_d.hpp"
...
template <typename ADataType,
          ...
          typename BLayout,
          typename CLayout>
bool profile_ggemm_multid_splitk(int do_verification,
                                 int init_method,
                                 bool do_log,
                                 bool time_kernel,
                                 ...
                                 int kernel_iter = 10)
{
    ...
    std::vector<GroupedGemmKernelArgument> grouped_gemm_kernel_args;
    grouped_gemm_kernel_args.reserve(group_count);
    ...
    for(std::size_t i = 0; i < group_count; i++)
    {
        ...
        grouped_gemm_kernel_args.push_back({a_device_buf[i]->GetDeviceBuffer(),
    ...
    using DeviceOp = ck::tensor_operation::device::DeviceGroupedGemmMultipleDSplitK<ALayout,
                                                                                    BLayout,
                                                                                    ck::Tuple<>,
                                                                                    CLayout,
                                                                                    ADataType,
    ...
    for(auto& gemm_ptr : op_ptrs)
    {
        ...
        DeviceMem gemm_arg_dev_mem(gptr->GetDeviceKernelArgSize(argument_ptr.get()));
        ...
        gptr->SetDeviceKernelArgs(argument_ptr.get(), gemm_arg_dev_mem.GetDeviceBuffer());

        std::string gemm_name        = gemm_ptr->GetTypeString();
        std::vector<int> kbatch_list = {1, 2, 4, 8, 12, 16, 20, 24, 32, 48, 64, 128};
