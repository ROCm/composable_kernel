...
"""
CPU-only unit tests for grouped_gemm_tensorquant_utils.py.

Tests kernel name generation, config serialization, and problem dimension helpers.
No GPU or hipcc required.

Run:
    python3 -m pytest dispatcher/tests/test_grouped_gemm_tensorquant_utils.py -v
"""
...
import grouped_gemm_tensorquant_utils as UTILS
...
# =============================================================================
# TensorQuantKernelConfig.name — byte-exact match with codegen KERNEL_NAME
# =============================================================================
...
class TestKernelName:
    ...
    def test_fp8_rcr_default_name(self):
        ...
        assert cfg.name == (
            "grouped_gemm_tensorquant_fp8_rcr_compv3_cshuffle_intrawave_"
    ...
    def test_bf8_rcr_default_name(self):
        ...
        assert cfg.name == (
            "grouped_gemm_tensorquant_bf8_rcr_compv3_cshuffle_intrawave_"
            "False_False_True_False_"
            "128x128x64_2x2x1_32x32x16"
        )
    ...
    def test_tensorquant_differs_from_rowcolquant(self):
        cfg = default_fp8_config()
        assert "tensorquant" in cfg.name
        assert "rowcolquant" not in cfg.name
    ...
    def test_default_fp8_config_name(self):
        ...
        assert "grouped_gemm_tensorquant" in cfg.name
...
# =============================================================================
# TensorQuantKernelConfig.to_codegen_config — round-trip shape
# =============================================================================
...
# =============================================================================
# TensorQuantGemmProblem dimension helpers
# =============================================================================
...
class TestTensorQuantGemmProblem:
    ...
    def test_default_k_batch(self):
        p = TensorQuantGemmProblem(M=16, N=64, K=256)
        assert p.k_batch == 1

    def test_k_batch_set(self):
        p = TensorQuantGemmProblem(M=16, N=64, K=256, k_batch=4)
        assert p.k_batch == 4

    def test_dimensions_stored(self):
        p = TensorQuantGemmProblem(M=32, N=128, K=512)
        assert p.M == 32
        assert p.N == 128
        assert p.K == 512
    ...
    def _make_configs(self):
        ...
        for dtype in ("fp8", "bf8"):
            for pad_k in (True, False):
                configs.append(TensorQuantKernelConfig(
...
class TestCodegenHeaderGeneration:
    ...
    def _generate_header(self, dtype="fp8"):
        ...
        from unified_grouped_gemm_tensorquant_codegen import (
            TensorQuantKernelHeaderGenerator,
            TensorQuantKernelSpec,
            TensorQuantTileConfig,
        )
    ...
    def test_header_contains_kernel_name(self):
        ...
        assert "grouped_gemm_tensorquant_fp8_rcr" in header

    def test_header_contains_tensorquant_type(self):
        header = self._generate_header()
        assert "ck_tile::QuantType::TensorQuant" in header
...
class TestArchDefines:
    ...
    def _compile_argv(self, monkeypatch, tmp_path, gfx_arch):
        ...
        ok = UTILS._compile_tensorquant_kernel(
...
class TestSoCacheAbiKey:
    ...
    setup_multiple_tensorquant_dispatchers() reuses a .so when the filename
    ...
    def test_pre_abi_artifact_is_not_reused(self, monkeypatch, tmp_path, old_suffix):
        ...
        monkeypatch.setattr(UTILS, "_compile_tensorquant_kernel", fake_compile)

        out = UTILS.setup_multiple_tensorquant_dispatchers(
    ...
    def test_second_call_hits_the_cache(self, monkeypatch, tmp_path):
        ...
        first = UTILS.setup_multiple_tensorquant_dispatchers(
            configs=[cfg], output_dir=tmp_path, gfx_arch="gfx950", parallel=False)
        second = UTILS.setup_multiple_tensorquant_dispatchers(
            configs=[cfg], output_dir=tmp_path, gfx_arch="gfx950", parallel=False)
