...
try:
    ...
    warnings.warn(
        "C++ extension not available. Build with -DBUILD_DISPATCHER_PYTHON=ON"
    )
...
def build_dispatcher_executable(
    kernel_files: List[Path], output_executable: Path, verbose: bool = True
) -> bool:
    """
...
class Dispatcher:
    ...
    def load_generated_kernels(self, kernels_dir: Optional[Path] = None):
        ...
        kernels_dir / "registration" / "dispatcher_registration.hpp"
    ...
    def generate_and_load_kernels(
        self, datatype: str = "fp16", layout: str = "rcr", preset: str = "essential"
    ):
    ...
    def run_gpu_gemm(
        self, M: int, N: int, K: int, executable: Optional[Path] = None
    ) -> Dict:
        """
        ...
        return {"success": True, "output": result.stdout, "problem_size": (M, N, K)}
    ...
    def get_registered_kernels(self) -> List[str]:
        ...
        if not HAS_CPP_EXTENSION or self.registry is None:
            # Read from manifest
            manifest = (
                get_generated_kernels_dir() / "registration" / "kernels_manifest.json"
            )
            if manifest.exists():
                with open(manifest) as f:
                    data = json.load(f)
...
class SimpleGemmAPI:
    ...
    def execute(self, M: int, N: int, K: int, verbose: bool = True) -> Dict:
        """
        Execute GEMM on GPU
        ...
        if self.executable is None:
            raise RuntimeError(
                "Executable not ready. Call ensure_kernels_ready() first"
            )
