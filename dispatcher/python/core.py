...
class DataType(Enum):
    ...
    def from_string(cls, s: str) -> "DataType":
        ...
        s = s.lower()
        mapping = {
            "fp16": cls.FP16, "half": cls.FP16,
            "bf16": cls.BF16, "bfloat16": cls.BF16,
            "fp32": cls.FP32, "float": cls.FP32, "float32": cls.FP32,
            "fp64": cls.FP64, "double": cls.FP64, "float64": cls.FP64,
            "fp8": cls.FP8, "fp8_e4m3": cls.FP8,
            "bf8": cls.BF8, "fp8_e5m2": cls.BF8,
            "int8": cls.INT8,
            "int4": cls.INT4,
            "int32": cls.INT32,
        }
    ...
    def element_size(self) -> float:
        ...
        sizes = {
            ...
            DataType.FP8: 1, DataType.BF8: 1,
            DataType.INT8: 1, DataType.INT4: 0.5,
            DataType.INT32: 4,
        }
...
class Problem:
    ...
    def from_arrays(
        ...
        return cls(
            ...
            dtype_c=DataType.from_numpy(C.dtype) if C is not None else DataType.from_numpy(A.dtype),
    ...
    def from_ab(
        ...
        return cls(M=M, N=N, K=K_from_A, transpose_a=transpose_a, transpose_b=transpose_b)
    ...
    def validate(self) -> Tuple[bool, str]:
        ...
        if isinstance(self.A, np.ndarray):
            ...
            if self.A.size != expected_a * self.batch_size:
                return False, f"A tensor size mismatch: got {self.A.size}, expected {expected_a * self.batch_size}"
        
        if isinstance(self.B, np.ndarray):
            expected_b = self.K * self.N if not self.transpose_b else self.N * self.K
            if self.B.size != expected_b * self.batch_size:
                return False, f"B tensor size mismatch: got {self.B.size}, expected {expected_b * self.batch_size}"
        
        if isinstance(self.C, np.ndarray):
            expected_c = self.M * self.N
            if self.C.size != expected_c * self.batch_size:
                return False, f"C tensor size mismatch: got {self.C.size}, expected {expected_c * self.batch_size}"
