...
class ProfileReport:
    ...
    def add_profile(self, profile: KernelProfile):
        ...
        stats["avg_gflops"] = (
            stats.get("avg_gflops", 0.0) * (stats["count"] - 1) + profile.gflops
        ) / stats["count"]
        ...
        ps_stats["count"] += 1
        ps_stats["avg_time_ms"] = (
            ps_stats["avg_time_ms"] * (ps_stats["count"] - 1)
            + profile.execution_time_ms
        ) / ps_stats["count"]
        ps_stats["avg_gflops"] = (
            ps_stats["avg_gflops"] * (ps_stats["count"] - 1) + profile.gflops
        ) / ps_stats["count"]
    ...
    def get_summary(self) -> str:
        ...
        lines.append(
            f"Average time per call: {self.total_time_ms / max(1, self.total_calls):.2f} ms"
        )
        lines.append("")
        ...
        for kernel_name, stats in sorted(
            self.kernel_stats.items(), key=lambda x: x[1]["total_time_ms"], reverse=True
        ):
            lines.append(
                f"{kernel_name:<40} {stats['count']:>8} "
        ...
        lines.append("-" * 80)
        lines.append(
            f"{'Size (MxNxK)':<30} {'Calls':>8} {'Avg (ms)':>12} {'GFLOPS':>12}"
        )
        lines.append("-" * 80)

        for size, stats in sorted(
            self.problem_size_stats.items(), key=lambda x: x[1]["count"], reverse=True
        ):
            size_str = f"{size[0]}x{size[1]}x{size[2]}"
            lines.append(
                f"{size_str:<30} {stats['count']:>8} "
                f"{stats['avg_time_ms']:>12.3f} {stats['avg_gflops']:>12.2f}"
            )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "problem_size_stats": {
                str(k): v for k, v in self.problem_size_stats.items()
            },
...
class ComparativeProfiler:
    ...
    def print_comparison(self):
        ...
        print("=" * 80)
        print(
            f"{'Implementation':<20} {'Mean (ms)':>12} {'Std (ms)':>12} {'Speedup':>12}"
        )
        print("-" * 80)
        ...
        for name, result in sorted(self.results.items(), key=lambda x: x[1]["mean_ms"]):
            ...
            print(
                f"{name:<20} {result['mean_ms']:>12.3f} {result['std_ms']:>12.3f} "
                f"{speedup:>12.2f}x"
            )
...
def visualize_timeline(report: ProfileReport, output_file: Optional[str] = None):
    ...
    [p.kernel_name for p in report.timeline]
