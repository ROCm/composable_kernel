...
TEST_F(DispatcherTest, SelectKernelHeuristicFallback) {
    ...
    dispatcher.set_heuristic([](const Problem&) {
        return std::vector<std::string>{"nonexistent_kernel"};
    });
...
TEST_F(DispatcherTest, RunExplicitNotFound) {
    ...
    EXPECT_THROW(
        dispatcher.run_explicit("nonexistent", a, b, c, nullptr, problem),
        std::runtime_error
    );
}
...
TEST_F(DispatcherTest, RunExplicitNotSupported) {
    ...
    EXPECT_THROW(
        dispatcher.run_explicit(kernel_id, a, b, c, nullptr, problem),
        std::runtime_error
    );
}
