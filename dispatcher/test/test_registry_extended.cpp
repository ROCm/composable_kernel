...
TEST_F(RegistryFilterTest, FilterByTileSize) {
    auto large = Registry::instance().filter([](const KernelInstance& k) {
...
TEST_F(RegistryFilterTest, FilterByDataType) {
    auto fp16 = Registry::instance().filter([](const KernelInstance& k) {
        return k.get_key().signature.dtype_a == DataType::FP16;
    });
...
TEST_F(RegistryFilterTest, FilterMatchesNone) {
    auto none = Registry::instance().filter([](const KernelInstance& k) {
        return k.get_key().algorithm.tile_shape.m > 2048;
    });
...
TEST_F(RegistryFilterTest, FilterMatchesAll) {
    auto all = Registry::instance().filter([](const KernelInstance& k) {
        return true;
    });
...
TEST_F(MultipleRegistriesTest, SingletonIndependence) {
    ...
    Registry::instance().register_kernel(std::make_shared<MockKernelInstance>(key2, "global_kernel"));
        ...
        threads.emplace_back([t, kernels_per_thread, &success_count]() {
            for (int k = 0; k < kernels_per_thread; k++) {
                ...
                auto kernel = std::make_shared<MockKernelInstance>(
                    key, "kernel_" + std::to_string(tile));
