...
int main(int argc, char** argv)
{
    ...
    std::cout << "  - " << fp16_registry.get_name() << ": " << fp16_registry.size()
              << " kernel(s)\n";
    std::cout << "  - " << production_registry.get_name() << ": " << production_registry.size()
              << " kernel(s)\n";
    std::cout << "  - " << experimental_registry.get_name() << ": " << experimental_registry.size()
              << " kernel(s)\n\n";
    ...
    auto merged_from_exp =
        combined_registry.merge_from(experimental_registry, Registry::Priority::Low);
