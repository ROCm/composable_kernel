...
int main()
{
    ...
    std::cout << "  A sample values: " << float(a_m_k.mData[0]) << ", " << float(a_m_k.mData[1])
              << ", " << float(a_m_k.mData[2]) << "\n";
    std::cout << "  B sample values: " << float(b_k_n.mData[0]) << ", " << float(b_k_n.mData[1])
              << ", " << float(b_k_n.mData[2]) << "\n\n";

    // Step 2: Compute CPU reference
    std::cout << "Step 2: Computing CPU reference...\n";
    ck_tile::reference_gemm<ADataType, BDataType, AccDataType, CDataType>(a_m_k, b_k_n, c_cpu_ref);
    ...
    {
        if(a_m_k.mData[i] == a_verify[i])
            a_match++;
    }
    for(size_t i = 0; i < b_k_n.get_element_space_size(); i++)
    {
        if(b_k_n.mData[i] == b_verify[i])
            b_match++;
    }
    ...
    HIP_CHECK(
        hipMemcpy(c_gpu_result.data(), c_dev, M * N * sizeof(CDataType), hipMemcpyDeviceToHost));
    ...
    bool pass = ck_tile::check_err(c_gpu_result,
                                   c_cpu_ref,
                                   "GPU vs CPU",
