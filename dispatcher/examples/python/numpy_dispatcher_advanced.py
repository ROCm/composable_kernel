...
def test_with_random_matrices(lib, M, N, K):
    ...
    if rel_error < 0.02:  # 2% tolerance for FP16
        print(f"  Result: [OK] GPU matches NumPy!")
        return True
    else:
        print(f"  Result: [FAIL] Difference too large")
        return False
...
def benchmark_multiple_sizes(lib):
    ...
    print(f"{'Size':<15} | {'Time (ms)':<12} | {'TFLOPS':<10} | {'vs NumPy':<12} | Status")
    print("-" * 75)
    ...
    for M, N, K in sizes:
        try:
            ...
            print(f"{size_str:<15} | {gpu_time:<12.4f} | {tflops:<10.2f} | {speedup:<12.1f}x | {status}")
...
def demo_kernel_selection_info(lib):
    ...
    if len(parts) > 3:
        ...
        for part in parts:
            if 'x' in part and part.replace('x', '').replace('False', '').replace('True', '').replace('_', '').isdigit():
...
def main():
    ...
    print("  [OK] GPU execution: up to " + 
          f"{max(r['tflops'] for r in results):.2f} TFLOPS" if results else "N/A")
