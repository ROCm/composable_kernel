...
def load_library(lib_path):
    ...
    lib.dispatcher_is_supported.argtypes = [ctypes.c_int64, ctypes.c_int64, ctypes.c_int64]
...
def main():
    ...
    for name, M, N, K in batch_operations:
        ...
        if result.time_ms >= 0:
            ...
            print(f"  {name:20s} {M:5d}x{N:5d}x{K:5d}  {result.time_ms:8.4f} ms  {result.tflops:6.2f} TFLOPS  [{status}]")
        else:
            print(f"  {name:20s} {M:5d}x{N:5d}x{K:5d}  {'skipped':>8s}     {'---':>6s} TFLOPS  [UNSUPPORTED]")
    ...
    all_correct = all(r.correct for r in supported_results) if supported_results else False
    ...
    if unsupported_count > 0:
        print(f"  Unsupported sizes: {unsupported_count} (need additional kernel configs)")
    ...
    print()
    print(f"{'Operation':25s} {'Size':20s} {'Time (ms)':>12s} {'% Total':>10s} {'TFLOPS':>10s}")
    print("-" * 80)
    
    for r in results:
        pct = (r.time_ms / total_time * 100) if total_time > 0 else 0
        size_str = f"{r.M}x{r.N}x{r.K}"
        print(f"{r.name:25s} {size_str:20s} {r.time_ms:>12.4f} {pct:>10.1f}% {r.tflops:>10.2f}")
