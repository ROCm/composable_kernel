...
def validate_test(lib, name: str, A: np.ndarray, B: np.ndarray, expected: np.ndarray = None) -> bool:
    ...
    # NumPy reference
    if expected is None:
        expected = np.matmul(A.astype(np.float32), B.astype(np.float32)).astype(np.float16)
