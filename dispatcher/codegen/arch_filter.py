...
try:
    from arch_specs_generated import (
        ...
        get_supported_archs as _get_supported_archs,
    )
    _USING_GENERATED = True
except ImportError:
    # Fallback to hardcoded values if generated module not available
    logger.warning("arch_specs_generated.py not found, using fallback values. "
    ...
    ELEMENT_SIZE_MAP = {
        ...
        "fp8": 1, "bf8": 1, "int8": 1, "int4": 0.5, "int32": 4,
    }
...
class ArchFilter:
    ...
    def __init__(self, gpu_arch: str, strict_mode: bool = True):
        ...
        if self.family is None and strict_mode:
            raise ValueError(f"Unknown GPU architecture: {gpu_arch}. "
    ...
    def _validate_dimensions(self, config: KernelConfig, result: ValidationResult):
        ...
        if config.tile_m <= 0 or config.tile_n <= 0 or config.tile_k <= 0:
            result.add_error(f"Tile dimensions must be positive: "
                           f"{config.tile_m}x{config.tile_n}x{config.tile_k}")
        
        if config.warp_m <= 0 or config.warp_n <= 0 or config.warp_k <= 0:
            result.add_error(f"Warp dimensions must be positive: "
                           f"{config.warp_m}x{config.warp_n}x{config.warp_k}")
        
        if config.warp_tile_m <= 0 or config.warp_tile_n <= 0 or config.warp_tile_k <= 0:
            result.add_error(f"Warp tile dimensions must be positive: "
        ...
        if config.warp_m * config.warp_tile_m > config.tile_m:
            result.add_error(f"warp_m * warp_tile_m ({config.warp_m}*{config.warp_tile_m}="
                           f"{config.warp_m * config.warp_tile_m}) > tile_m ({config.tile_m})")
        if config.warp_n * config.warp_tile_n > config.tile_n:
            result.add_error(f"warp_n * warp_tile_n ({config.warp_n}*{config.warp_tile_n}="
                           f"{config.warp_n * config.warp_tile_n}) > tile_n ({config.tile_n})")
        if config.warp_k * config.warp_tile_k > config.tile_k:
            result.add_error(f"warp_k * warp_tile_k ({config.warp_k}*{config.warp_tile_k}="
    ...
    def _validate_lds_capacity(self, config: KernelConfig, result: ValidationResult):
        ...
        max_lds = LDS_CAPACITY_LIMITS.get(config.pipeline, LDS_CAPACITY_LIMITS["default"])
    ...
    def _validate_dimension_alignment(self, config: KernelConfig, result: ValidationResult):
...
if __name__ == "__main__":
    ...
    result = filter_942.validate_kernel(KernelConfig(
        datatype_a="fp16", datatype_b="fp16", datatype_c="fp16",
        tile_m=256, tile_n=256, tile_k=64,
        warp_m=2, warp_n=2, warp_k=1,
        warp_tile_m=32, warp_tile_n=32, warp_tile_k=16,
    ...
    result = filter_942.validate_kernel(KernelConfig(
        datatype_a="fp16", datatype_b="fp16", datatype_c="fp16",
        tile_m=256, tile_n=256, tile_k=64,
        warp_m=3, warp_n=3, warp_k=1,  # Invalid!
        warp_tile_m=32, warp_tile_n=32, warp_tile_k=16,
    ))
    print(f"  Valid: {result.valid}")
    if result.errors:
    ...
    result = filter_942.validate_kernel(KernelConfig(
        datatype_a="fp16", datatype_b="fp16", datatype_c="fp16",
        tile_m=512, tile_n=512, tile_k=256,  # Too large!
        warp_m=2, warp_n=2, warp_k=1,
        warp_tile_m=32, warp_tile_n=32, warp_tile_k=16,
        pipeline="compv4"
    ))
