...
RowColQuant: A has per-row scales (AQ shape [M, 1]), B has per-column scales (BQ shape [1, N]).
Both ADataType and BDataType are fp8 or bf8; AQDataType=BQDataType=float.
...
ROWCOLQUANT_PIPELINE_MAP      = dict(ROWCOL_TENSOR_QUANT_PIPELINE_MAP)
ROWCOLQUANT_BASE_PIPELINE_MAP = dict(ROWCOL_TENSOR_QUANT_BASE_PIPELINE_MAP)
ROWCOLQUANT_EPILOGUE_MAP      = dict(ROWCOL_TENSOR_QUANT_EPILOGUE_MAP)
ROWCOLQUANT_SUPPORTED_LAYOUTS = ROWCOL_TENSOR_QUANT_SUPPORTED_LAYOUTS
...
class RowColQuantKernelHeaderGenerator:
    ...
    def generate(self, spec: RowColQuantKernelSpec) -> str:
        variant = ROWCOLQUANT_VARIANTS[spec.dtype]
        t = spec.tile
        ns = "ns_" + spec.name
        ...
        # AQ is RowMajor [M,1]; BQ is ColMajor [1,N]
        ...
        epilogue_ck      = ROWCOLQUANT_EPILOGUE_MAP[spec.epilogue]
        ...
        return f"""\
// SPDX-License-Identifier: MIT
// Auto-generated RowColQuant Grouped GEMM kernel header.
// DO NOT EDIT — regenerate via unified_grouped_gemm_rowcolquant_codegen.py
#pragma once
...
def _default_config() -> dict:
    ...
    # default_{fp8,bf8}_config() in grouped_gemm_rowcolquant_utils.py cannot drift.
    return {
        "dtypes": ["fp8", "bf8"],
        "layouts": list(ROWCOLQUANT_SUPPORTED_LAYOUTS),
        **ROWCOL_TENSOR_QUANT_DEFAULT_TRAITS,
...
def _build_specs(config: dict) -> List[RowColQuantKernelSpec]:
    ...
    if pipeline not in ROWCOLQUANT_PIPELINE_MAP:
        log.warning(
            ...
            pipeline, ", ".join(sorted(ROWCOLQUANT_PIPELINE_MAP)),
        )
        return []
    if epilogue not in ROWCOLQUANT_EPILOGUE_MAP:
        log.warning(
            "Unsupported epilogue '%s' (supported: %s) — no kernels generated",
            epilogue, ", ".join(sorted(ROWCOLQUANT_EPILOGUE_MAP)),
        )
        return []
    if scheduler not in ROWCOLQUANT_SCHEDULER_TO_CK:
        log.warning(
            "Unsupported scheduler '%s' (supported: %s) — no kernels generated",
            scheduler, ", ".join(sorted(ROWCOLQUANT_SCHEDULER_TO_CK)),
        )
        return []

    for dtype, layout, tile_dict in itertools.product(
        config.get("dtypes", ["fp8"]),
        config.get("layouts", list(ROWCOLQUANT_SUPPORTED_LAYOUTS)),
        config.get("tile_configs", []),
    ):
        if dtype not in ROWCOLQUANT_VARIANTS:
        ...
        if layout not in ROWCOLQUANT_SUPPORTED_LAYOUTS:
            log.warning(
                "Unsupported layout '%s' (supported: %s) — skipping",
                layout, ", ".join(ROWCOLQUANT_SUPPORTED_LAYOUTS),
            )
            continue
