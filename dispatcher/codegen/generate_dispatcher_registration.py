...
def scan_generated_headers(generated_dir: Path) -> List[KernelConfig]:
    ...
    for header_file in generated_dir.glob("**/*.hpp"):
        try:
            ...
            # Extract kernel name
            name_match = re.search(r'constexpr const char\* KERNEL_NAME\s*=\s*"([^"]+)"', content)
            if not name_match:
                continue
...
def main():
    parser = argparse.ArgumentParser(description='Generate dispatcher registration code')
    ...
    parser.add_argument('--manifest', type=str,
                       help='Optional manifest file with kernel configurations')
