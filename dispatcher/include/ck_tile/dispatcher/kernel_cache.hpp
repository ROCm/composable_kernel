...
inline std::uint64_t hash_directory(const std::filesystem::path& dir, 
                                     const std::vector<std::string>& extensions = {".hpp", ".h", ".cpp"}) {
...
struct CacheMetadata {
    ...
    static std::optional<CacheMetadata> deserialize(const std::string& data) {
        ...
        while (std::getline(ss, line)) {
            ...
            if (key == "kernel_id") meta.kernel_identifier = value;
            else if (key == "gpu_arch") meta.gpu_arch = value;
            else if (key == "source_hash") meta.source_hash = std::stoull(value);
            else if (key == "kernel_hash") meta.kernel_hash = std::stoull(value);
            else if (key == "compiler") meta.compiler_version = value;
            else if (key == "flags") meta.compile_flags = value;
            else if (key == "created") meta.created_timestamp = std::stoll(value);
            else if (key == "accessed") meta.last_accessed = std::stoll(value);
            else if (key == "size") meta.binary_size = std::stoull(value);
        }
        
        if (meta.kernel_identifier.empty()) return std::nullopt;
        return meta;
    }
};
...
public:
    ...
    {
        if (!enabled_ || binary.empty()) return false;
...
private:
    ...
    std::filesystem::path get_index_path() const {
        return cache_dir_ / "cache_index.txt";
    }
    ...
    void load_cache_index() {
        ...
        while (std::getline(file, line)) {
            ...
            if (!std::filesystem::exists(meta_path)) continue;
