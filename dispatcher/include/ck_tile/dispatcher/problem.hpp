...
struct TensorShape {
    ...
    [[nodiscard]] std::int64_t logical_rows() const {
        return is_transposed ? cols : rows;
    }
    
    /// Get logical N (cols when not transposed)  
    [[nodiscard]] std::int64_t logical_cols() const {
        return is_transposed ? rows : cols;
    }
};
...
struct Problem {
     ...
     */
    [[nodiscard]] static Problem from_shapes(
        TensorShape a_shape,
        TensorShape b_shape,
        TensorShape c_shape)
    {
     ...
     */
    [[nodiscard]] static Problem from_dimensions(
        std::int64_t a_rows, std::int64_t a_cols,
        std::int64_t b_rows, std::int64_t b_cols,
        std::int64_t c_rows, std::int64_t c_cols)
    {
        return from_shapes(
            TensorShape(a_rows, a_cols),
            TensorShape(b_rows, b_cols),
            TensorShape(c_rows, c_cols));
    }
     ...
     */
    [[nodiscard]] static Problem from_ab(
        std::int64_t a_rows, std::int64_t a_cols,
        std::int64_t b_rows, std::int64_t b_cols)
    {
        if (a_cols != b_rows) {
            throw std::invalid_argument(
                "K dimension mismatch: A.cols=" + std::to_string(a_cols) +
...
public:
    ...
    ProblemBuilder& from_ab(std::int64_t a_rows, std::int64_t a_cols,
                            std::int64_t b_rows, std::int64_t b_cols) {
