...
inline std::vector<WarpTileConfig> get_supported_warp_tiles(
    GpuArch arch, DataType dtype_a, DataType dtype_b, [[maybe_unused]] DataType dtype_c) 
{
    ...
    if (dtype_a == DataType::FP16 && dtype_b == DataType::FP16) {
        if (arch == GpuArch::GFX_1201) return rdna4_fp16;
        return fp16_configs;
    }
    if (dtype_a == DataType::BF16 && dtype_b == DataType::BF16) {
        if (arch == GpuArch::GFX_1201) return {};  // Not supported on RDNA4
        return fp16_configs;  // Same as FP16
    }
    if (dtype_a == DataType::FP8 || dtype_a == DataType::BF8) {
        if (arch == GpuArch::GFX_950) return fp8_gfx950;
        if (arch == GpuArch::GFX_942) return fp8_gfx942;
        if (arch == GpuArch::GFX_90A) return {{32, 32, 16}, {32, 32, 32}};
    }
    if (dtype_a == DataType::INT8 && dtype_b == DataType::INT8) {
        if (arch == GpuArch::GFX_942) return int8_configs;
    }
...
public:
     ...
     */
    [[nodiscard]] bool is_valid(const KernelKey& key) const {
...
private:
    void validate_dimensions(const KernelKey& key, ValidationResult& result) const {
        ...
        if (warp_m_coverage > alg.tile_shape.m) {
            result.add_error("warp_m * warp_tile_m > tile_m: " + 
                std::to_string(warp_m_coverage) + " > " + std::to_string(alg.tile_shape.m));
        }
        if (warp_n_coverage > alg.tile_shape.n) {
            result.add_error("warp_n * warp_tile_n > tile_n: " +
                std::to_string(warp_n_coverage) + " > " + std::to_string(alg.tile_shape.n));
        }
        if (warp_k_coverage > alg.tile_shape.k) {
            result.add_error("warp_k * warp_tile_k > tile_k: " +
                std::to_string(warp_k_coverage) + " > " + std::to_string(alg.tile_shape.k));
        }
    ...
    void validate_warp_config(const KernelKey& key, ValidationResult& result) const {
        ...
        WarpConfig current = {key.algorithm.wave_shape.m, 
                             key.algorithm.wave_shape.n, 
                             key.algorithm.wave_shape.k};
        ...
        if (!found) {
            result.add_error("Invalid warp configuration [" + 
                std::to_string(current[0]) + ", " +
                std::to_string(current[1]) + ", " +
                std::to_string(current[2]) + "] for " + get_arch_string());
        }
    }
    ...
    void validate_warp_tiles(const KernelKey& key, ValidationResult& result) const {
        ...
        if (!found) {
            result.add_error("Invalid warp tile [" +
                std::to_string(current[0]) + ", " +
                std::to_string(current[1]) + ", " +
                std::to_string(current[2]) + "] for " + get_arch_string());
        }
    ...
    void validate_traits(const KernelKey& key, ValidationResult& result) const {
        if (is_trait_unsupported(key.algorithm.pipeline, 
                                  key.algorithm.epilogue,
                                  key.algorithm.scheduler)) {
    ...
    void validate_lds(const KernelKey& key, ValidationResult& result) const {
        ...
        if (total_lds > max_lds) {
            result.add_error("LDS capacity exceeded: " + std::to_string(total_lds) +
                " bytes > " + std::to_string(max_lds) + " bytes limit");
        }
    }
