                         ...
                         float atol = 1e-5f)
{
    ...
    // Copy from device
    hipMemcpy(a_host.data(),
             a_dev_ptr,
             M * K * sizeof(ADataType),
             hipMemcpyDeviceToHost);
    hipMemcpy(b_host.data(),
             b_dev_ptr,
             K * N * sizeof(BDataType),
             hipMemcpyDeviceToHost);
    hipMemcpy(c_host.data(),
             c_dev_ptr,
             M * N * sizeof(CDataType),
             hipMemcpyDeviceToHost);
    
    // Compute reference
    reference_gemm_cpu<ADataType, BDataType, CDataType, AccDataType>(
        a_host.data(),
        b_host.data(),
        c_ref.data(),
        M,
...
struct TestTensor
{
    ...
    void zero()
    {
        hipMemset(device_ptr, 0, size * sizeof(T));
    }
};
