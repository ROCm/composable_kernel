...
void Dispatcher::set_strategy(SelectionStrategy strategy)
{
    strategy_ = strategy;
}
...
KernelInstancePtr Dispatcher::select_kernel(const Problem& problem) const
{
    ...
    switch (strategy_) {
        case SelectionStrategy::FirstFit:
    ...
    void* stream) const
{
    ...
    if (!kernel) {
        ...
        oss << "No suitable kernel found for problem: M=" << problem.M
