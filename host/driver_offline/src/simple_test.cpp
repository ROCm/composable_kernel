...
int test_slice_transform()
{
    ...
    auto check_values = [](const auto& desc2d,
                           const std::vector<data_t>& data,
                           const std::vector<data_t>& expected) {
        constexpr auto I0 = Number<0>{};
        constexpr auto I1 = Number<1>{};
        ...
        for(int i0 = 0; i0 < desc2d.GetLength(I0); ++i0)
        {
            ...
            for(int i1 = 0; i1 < desc2d.GetLength(I1); ++i1)
            {
                ...
                if(diff > tol)
                {
                    std::cout << "[" << i0 << ", " << i1 << "], offset = " << offset
                              << ", val = " << *(data.begin() + offset) << std::endl;
                }
            }
        }
    };
    ...
    const auto desc_slice =
        transform_tensor_descriptor(desc_3d,
    ...
    const auto desc_2d = transform_tensor_descriptor(
        desc_slice,
        make_tuple(make_merge_transform(make_tuple(1, dim1)), make_pass_through_transform(dim2)),
...
int test_freeze_transform()
{
    ...
    auto check_values = [](const auto& desc2d,
        ...
        for(int i0 = 0; i0 < desc2d.GetLength(I0); ++i0)
        {
            ...
            for(int i1 = 0; i1 < desc2d.GetLength(I1); ++i1)
            {
                ...
                {
                    std::cout << "[" << i0 << ", " << i1 << "], offset = " << offset
                              << ", val = " << *(data.begin() + offset) << ", " << expected[idx] << std::endl;
                }
            }
        }
    };
    ...
    const auto desc_2d = transform_tensor_descriptor(
        desc_3d,
        make_tuple(make_freeze_transform(1), 
                   make_pass_through_transform(dim1),
                   make_pass_through_transform(dim2)),
        make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}),
        make_tuple(Sequence<0>{}, Sequence<1>{}));
