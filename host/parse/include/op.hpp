...
struct CKGenOp_Xdl_CShuffle
{
    ...
    std::string CKGenSetOp(CKGenOp_Xdl_CShuffle &op, DataType_fe ADataType, DataType_fe BDataType, DataType_fe DsDataType, DataType_fe EDataType, Layout_fe ALayout, Layout_fe BLayout, Layout_fe DsLayout, Layout_fe ELayout, std::size_t M, std::size_t N, std::size_t K); //set up problem size for the operation
    nlohmann::json CKGenGetOpParams(); // get and parse the JSON file of requirements
    void CKGenSetOpFusion(std::string Prologue); // override the prologue/epilogue TODO: fix the prologue type
    char* CKGenGetBuffer(CKGenOp_Xdl_CShuffle &op, std::string key); // get a kernel file from the JSON
};
