...
void Copy_Operation_Conv::update_epilogue(const std::string& epilogue)
{
    if(!epilogue.empty())
    {
...
{
    std::vector<Copy_Operation_Conv> result;
    ...
    const Copy_Problem_Conv& prob, const std::string& prologue, const std::string& epilogue)
{
    return CreateOperationsImpl(
        [&](Copy_Operation_Conv x) -> std::array<Copy_Operation_Conv, 1> {
