...
class ParametersBlockwise:
    ...
    def __str__(self):
        ...
        warp_per_block = "x".join(str(i) for i in [self.warp_per_block_m, self.warp_per_block_n])
        warp_size = "x".join(str(i) for i in [self.warp_m, self.warp_n])
        thread_tile_size = "x".join(str(i) for i in [self.thread_tile_m, self.thread_tile_n])
        input_shape = "x".join(str(i) for i in self.input_shape)

        return "_".join([tile_size, warp_per_block, warp_size, thread_tile_size, input_shape])
...
class ReduceConfig:
    ...
    def get_parameter_combinations(
        self
    ) -> List[ParametersBlockwise]:  # TODO: move this method (and the validation one) elsewhere

        input_shape_configs = self.config_dict["problem_size"]["input_shape"]

        fixed_configs = self.config_dict["tile_config"].get("fixed", None)
        ...
        combo_config = self.config_dict["tile_config"].get("combination", None)
        if combo_config is None:
            ...
            for combo in product(
                ...
                if combo:
                    p = ParametersBlockwise(*combo)
                    hashable_combo = (tuple(combo[-1]),) + combo[0:-1]
                    if self.is_valid_combination(p) and hashable_combo not in seen_config:
                        yield p

    def is_valid_combination(self, p: ParametersBlockwise) -> bool:
        ...
        if len(p.input_shape) == 4 and (p.input_shape[2]*p.input_shape[3] % p.thread_tile_n != 0):
            return False
        
        if len(p.input_shape) == 3 and (p.input_shape[1]*p.input_shape[2] % p.thread_tile_n != 0):
            return False
