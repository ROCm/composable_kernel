...
struct GemmDispatcher {
    static auto& get_kernel_map() {
        ...
        static std::unordered_map<std::string, 
            std::function<void(std::size_t flop, std::size_t num_byte,
                         ck_tile::index_t K, ck_tile::index_t kbatch,
    ...
    static void init() {
        ...
        for group in self.all_kernels:
            content += f"""            kernel_map["{group}"] = [](std::size_t flop, std::size_t num_byte,
                         ck_tile::index_t K, ck_tile::index_t kbatch,
                         ck_tile::DeviceMem& c_m_n_dev_buf,
                         ck_tile::HostTensor<CDataType>& c_m_n_host_result,
                         ck_tile::HostTensor<CDataType>& c_m_n_dev_result,
            ...
            for tile in tile_params:
                ...
                if ((tile[0]/(tile[3] * tile[7]) * tile[3] * tile[7]) != tile[0]) or \
                   ((tile[1]/(tile[4] * tile[8]) * tile[4] * tile[8]) != tile[1]):
                    continue
                content += f"""
                run_kernel<{group}::GemmKernel<{tile[0]}, {tile[1]}, {tile[2]}, {tile[3]}, {tile[4]}, {tile[5]}, {tile[6]}, {tile[7]}, {tile[8]}>>(flop, num_byte, K, kbatch, c_m_n_dev_buf, c_m_n_host_result, c_m_n_dev_result, verify, args, s);"""
            content += f"""
            }};\n"""
    ...
    static void run_kernel(std::size_t flop, std::size_t num_byte,
                         ck_tile::index_t K, ck_tile::index_t kbatch,
                         ck_tile::DeviceMem& c_m_n_dev_buf,
                         ck_tile::HostTensor<CDataType>& c_m_n_host_result,
                         ck_tile::HostTensor<CDataType>& c_m_n_dev_result,
                         int verify, ck_tile::GemmHostArgs& args, const ck_tile::stream_config& s)
    {
        ...
        if(verify)
            compare(K, kbatch, c_m_n_dev_result, c_m_n_host_result);
    ...
    static auto dispatch(std::size_t flop, std::size_t num_byte,
                         ck_tile::index_t K, ck_tile::index_t kbatch,
                         ck_tile::DeviceMem& c_m_n_dev_buf,
                         ck_tile::HostTensor<CDataType>& c_m_n_host_result,
                         ck_tile::HostTensor<CDataType>& c_m_n_dev_result,
        ...
        if(auto it = kernel_map.find(key); it != kernel_map.end()) {
            return it->second(flop, num_byte, K, kbatch, c_m_n_dev_buf, c_m_n_host_result, c_m_n_dev_result, verify,gemm_args, s); 
        }
