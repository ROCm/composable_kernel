...
User can provide kernel configuration such as tile size, warp size, padding, pipeline, scheduler and epilogue in the config file with custom configurations which will reduce the build time as it builds for few configurations. For reference please see `./configs/user_provided_config.json`. 
