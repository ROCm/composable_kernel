...
class GemmRowColQuantBenchmark(GemmBenchmark):
    def __init__(self, build_dir: str, verbose: bool = False):
        super().__init__(build_dir, verbose, name="benchmark_gemm_rowcolquant_")


def main():
    parser = argparse.ArgumentParser(
        description="RowColQuant GEMM Kernel Benchmarking Tool"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--csv",
        default="gemm_rowcolquant_benchmark_results.csv",
    ...
    benchmark = GemmRowColQuantBenchmark(args.build_dir, verbose=args.verbose)

    # Run benchmark sweep
    print("Starting RowColQuant GEMM kernel benchmark sweep...")
