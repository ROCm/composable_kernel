...
class GemmProfiler
{
    ...
    void benchmark(GemmProblem& gemm_problem,
                   std::vector<std::function<std::tuple<std::string, float>(
                       ck_tile::BatchedGemmHostArgs&, const ck_tile::stream_config&)>>& callables)
    {
        ...
        if(gemm_problem.batch_stride_a_ <= 0)
        {
            gemm_problem.batch_stride_a_ = gemm_problem.m_ * gemm_problem.k_;
        }
        if(gemm_problem.batch_stride_b_ <= 0)
        {
            gemm_problem.batch_stride_b_ = gemm_problem.k_ * gemm_problem.n_;
        }
        if(gemm_problem.batch_stride_c_ <= 0)
        {
