...
class GemmBenchmark:
    ...
    def discover_kernels(self) -> List[Path]:
        ...
        kernels = list(bin_dir.glob("benchmark_batched_gemm_*"))
        if self.verbose:
...
def main():
    parser = argparse.ArgumentParser(description="Batched GEMM Kernel Benchmarking Tool")
    parser.add_argument(
    ...
    # Run benchmark sweep
    print("Starting batched GEMM kernel benchmark sweep...")
