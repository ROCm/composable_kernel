...
#include "grouped_gemm_tensorquant_benchmark.hpp"

class GroupedTensorQuantGemmProfiler
{
    public:
    static GroupedTensorQuantGemmProfiler& instance(Settings settings)
    {
        static GroupedTensorQuantGemmProfiler instance{settings};
        return instance;
    }

    // Overload for single kernel benchmarking
    void benchmark(GroupedTensorQuantGemmProblem& problem,
                       ...
                       void*)>>& callables)
    {
        ...
        for(int i = 0; i < group_count; ++i)
        {
            ...
            // TensorQuant: AQ is scalar [1, 1], BQ is scalar [1, 1]
            aq_tensors.push_back(ck_tile::HostTensor<AccDataType>(
                ck_tile::host_tensor_descriptor(1, 1, 1, is_row_major(layout_a))));
            bq_tensors.push_back(ck_tile::HostTensor<AccDataType>(
                ck_tile::host_tensor_descriptor(1, 1, 1, is_row_major(layout_b))));
            ...
            // TensorQuant: QK_A=1, QK_B=1, stride_AQ=1, stride_BQ=1
            gemm_descs.push_back({p_a,
                                  ...
                                  1,   // stride_AQ (scalar, shape [1,1])
                                  1}); // stride_BQ (scalar, shape [1,1])
        }
    ...
    void process_result(const GroupedTensorQuantGemmProblem& problem,
                        std::vector<std::unique_ptr<ck_tile::DeviceMem>>& c_dev_bufs,
                        std::vector<ck_tile::HostTensor<CDataType>>& c_host_results,
                        std::vector<ck_tile::HostTensor<CDataType>>& c_dev_results,
                        const std::tuple<std::string, float>& kernel_run_result)
    {
        ...
        KernelInstance<GroupedTensorQuantGemmProblem> kernel_instance{
        ...
        for(int i = 0; i < problem.group_count_; ++i)
        {
            ...
            num_byte += sizeof(ADataType) * problem.Ms_[i] * problem.Ks_[i] +
                        ...
                        sizeof(AccDataType) * 1 + // AQ scale (scalar)
                        sizeof(AccDataType) * 1;  // BQ scale (scalar)
        }
    ...
    KernelInstance<GroupedTensorQuantGemmProblem> select_best_instance(Metric metric)
    {
        ...
        if(!settings_.csv_filename.empty())
        {
            ...
            else
            {
                ...
                file << get_rocm_version() << "," << ck_tile::get_device_name() << ","
                     ...
                     << "," << problem.layout_c_ << "," << "TensorQuant," << name << ","
    ...
    GroupedTensorQuantGemmProfiler(const GroupedTensorQuantGemmProfiler&)            = delete;
    GroupedTensorQuantGemmProfiler& operator=(const GroupedTensorQuantGemmProfiler&) = delete;

    private:
    ~GroupedTensorQuantGemmProfiler() { kernel_instances_.clear(); }
    GroupedTensorQuantGemmProfiler(Settings settings) : settings_(settings) {}

    Settings settings_;

    std::vector<KernelInstance<GroupedTensorQuantGemmProblem>> kernel_instances_;
};
