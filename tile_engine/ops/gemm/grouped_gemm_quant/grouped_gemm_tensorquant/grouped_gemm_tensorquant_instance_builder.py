...
def _import_gemm_kernel_builder():
    ...
    # Navigate up: grouped_gemm_tensorquant -> grouped_gemm_quant -> gemm
...
_OP_NAME = "grouped_gemm_tensorquant"
...
class GroupedTensorQuantGemmKernelBuilder(GemmKernelBuilder):
    def __init__(
        self,
        kernel_name_prefix,
        working_path,
    ...
    static float launch(const std::vector<ck_tile::QuantGroupedGemmHostArgs>& gemm_descs,
        ...
        const auto Run = [&](const auto has_hot_loop_, const auto tail_number_) {{
            ...
            if(!Kernel::IsSupportedArgument(kargs)) {{
                throw std::runtime_error("Wrong! Arguments not supported! Skipping grouped tensorquant gemm!");
            }}
...
def _generate_single_kernel_individual(work_item):
    ...
    builder = GroupedTensorQuantGemmKernelBuilder(
...
def main():
    parser = argparse.ArgumentParser(
        description="Grouped TensorQuant GEMM kernel instance builder with parallel support"
    )
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["fp8", "bf8"],
        help="Data type (fp8 or bf8 only for TensorQuant)",
    )
    parser.add_argument(
        "--layout",
        required=True,
    ...
    if args.datatype not in ["fp8", "bf8"]:
        parser.error(
            f"Invalid datatype string: {args.datatype} (TensorQuant supports fp8 and bf8 only)"
        )
    ...
    kernel_name_prefix = "grouped_gemm_tensorquant"
    builder = GroupedTensorQuantGemmKernelBuilder(
        kernel_name_prefix,
        args.working_path,
        args.gpu_target,
