...
#include "grouped_gemm_rowcolquant_profiler.hpp"
#include "grouped_gemm_rowcolquant_common.hpp"
...
// DataTypeTraits are now defined in grouped_gemm_rowcolquant_common.hpp
...
void benchmark_single(const ck_tile::ArgParser& arg_parser)
{
    ...
    // Create GroupedRowColQuantGemmProblem struct
    GroupedRowColQuantGemmProblem problem{group_count,
                                          kbatch,
                                          Ms,
                                          Ns,
                                          Ks,
    ...
    Settings settings{arg_parser.get_int("warmup"),
    ...
    auto& profiler = GroupedRowColQuantGemmProfiler::instance(settings);

    try
    {
        // Create a lambda that wraps the kernel launch
        auto kernel_func = [](const std::vector<ck_tile::QuantGroupedGemmHostArgs>& descs,
