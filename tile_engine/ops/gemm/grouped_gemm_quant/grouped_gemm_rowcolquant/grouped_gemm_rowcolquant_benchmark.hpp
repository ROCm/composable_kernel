...
struct GroupedRowColQuantGemmProblem
{
    int group_count_;
    int kbatch_;
    ...
    friend std::ostream& operator<<(std::ostream& os, const GroupedRowColQuantGemmProblem& problem)
    {
        os << "{\n"
           ...
           << "   \"quant_type\":\"RowColQuant\"\n"
           << "}";
        return os;
    }
};

/// @brief Function to compare grouped RowColQuant gemm results across all groups
bool compare_grouped(std::string instanceName,
                     const GroupedRowColQuantGemmProblem& problem,
                     ...
                     std::vector<ck_tile::HostTensor<CDataType>>& c_host_results)
{
    ...
    {
        pass &= compare<GroupedRowColQuantGemmProblem>(instanceName + "[" + std::to_string(i) + "]",
                                                       problem.Ks_[i],
                                                       problem.kbatch_,
                                                       c_dev_results[i],
                                                       c_host_results[i]);
...
/// @brief Compute host reference for RowColQuant grouped GEMM using GPU reference then apply scales
/// C_ref[m][n] = aq[m] * (sum_k A[m][k]*B[k][n]) * bq[n]
