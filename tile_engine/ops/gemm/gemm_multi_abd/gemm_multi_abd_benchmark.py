...
class GemmMultiABDBenchmark(GemmBenchmark):
    def __init__(self, build_dir: str, verbose: bool = False):
        super().__init__(build_dir, verbose, name="benchmark_gemm_multi_abd_")


def main():
    parser = argparse.ArgumentParser(
        description="GEMM Multi ABD Kernel Benchmarking Tool"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--csv",
        default="gemm_multi_abd_benchmark_results.csv",
    ...
    benchmark = GemmMultiABDBenchmark(args.build_dir, verbose=args.verbose)

    # Run benchmark sweep
    print("Starting GEMM Multi ABD kernel benchmark sweep...")
