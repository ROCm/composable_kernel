...
void benchmark_gemm_preshuffle(const ck_tile::ArgParser& arg_parser)
{
    ...
    try
    {
        ...
        printf("Completed 2\n");
        profiler.benchmark(gemm_problem, kernelinfo.warptilevalues, kernelinfo.functions);
        printf("Completed 3\n");
...
int main(int argc, char* argv[])
{
    try
    {
        ...
        if(!result)
            return EXIT_FAILURE;
        benchmark_gemm_preshuffle(parser);
        return 0;
    }
    catch(const std::exception& e)
    {
