...
using AElementOp   = PassThrough;//AddScale;
...
int main(int argc, char* argv[])
{
    ...
    [[maybe_unused]] float alpha = 1.0f;
    [[maybe_unused]] float beta  = 1.0f;
    ...
    auto a_element_op   = AElementOp{};//AElementOp{0.2};
    ...
    auto argument =
        device_op.MakeArgument(std::array<const void*, 1>{a0_device_buf.GetDeviceBuffer()},
    ...
    if(do_verification)
    {
        ...
        for(int m = 0; m < M; ++m)
        {
            for(int k = 0; k < K; ++k)
            {
                ...
                a_element_op(a_m_k(m, k), a0_m_k(m, k));
            }
        }
