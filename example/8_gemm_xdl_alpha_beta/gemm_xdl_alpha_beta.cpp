...
#include "device_gemm_xdl_c_shuffle_alpha_beta.hpp"
...
using CElementOp = ck::tensor_operation::element_wise::AlphaBeta;

// clang-format off
using DeviceGemmInstance = ck::tensor_operation::device::DeviceGemmXdl_C_Shuffle_Alpha_Beta<
    ADataType,              // ADataType
    BDataType,              // BDataType
    CDataType,              // CDataType
                        ...
                        const CElementwiseOperation& c_element_op)
{
    auto f_mk_kn_mn = [&](auto m, auto n) {
        ...
        for(int k = 0; k < K; ++k)
        {
            v += static_cast<AccDataType>(a_element_op(a_m_k(m, k))) *
                 static_cast<AccDataType>(b_element_op(b_k_n(k, n)));
        }

        CType y = static_cast<CType>(v);

        c_element_op(c_m_n(m, n), y, c0_k_n(m, n), alpha, beta);
    };
