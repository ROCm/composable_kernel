...
CK_TILE_HOST void my_reference_gemm(const ck_tile::HostTensor<ADataType>& a_m_k,
                                    ...
                                    const ck_tile::HostTensor<AScaleDataType>& sa_m_1,
                                    const ck_tile::HostTensor<GScaleDataType>& sb_1_n,
                                    ck_tile::HostTensor<DDataType>& d_m_n,
                                    ck_tile::HostTensor<ODataType>& d_f16,
                                    ...
                                    const ACCElementOp& acc_element_op = {})
{
    ...
    const std::size_t N = b_n_k.get_length(0);
...
auto create_args(int argc, char* argv[])
{
    ...
    arg_parser.insert("m", "32", "num of m")    // 64,  32,
        ...
        .insert("prec_i", "fp8", "input precision")
        .insert("prec_w", "fp8", "weight precision")
        ...
        .insert("prec_kw", "fp32", "topk-weight data type. auto will set to fp32")
...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    if(init == 0)
    {
        ...
        ck_tile::FillStepRange<GScaleDataType>{0.f, 1.f, 0.01f}(sb_host);
    }
    else if(init == 1)
    {
        ...
        ck_tile::FillUniformDistribution<GScaleDataType>{-.5f, .5f, seed, true}(sb_host);
    }
    else if(init == 2)
    {
        ...
        ck_tile::FillNormalDistribution<GScaleDataType>{0.f, 1.f, seed, true}(sb_host);
    }    
    /*
    ck_tile::FillStepRange<ADataType>{1.f, .5f, 0.0f}(a_host);
    ck_tile::FillStepRange<BDataType>{1.f, .5f, 0.0f}(b_host);
    ...
    // b_host
    {
        ...
        for(int y = 0; y < Y; y++)
        {
            ...
            {
                int idx = X * y + x;
                b_host.mData[idx] = ck_tile::type_convert<BDataType>(idx * 0.1f);
                //b_host.mData[idx] = ck_tile::type_convert<GDataType>(y * 1.0f);
                //b_host.mData[idx] = ck_tile::type_convert<GDataType>(y*1.f + x * 0.0001f);
            }
        }
    ...
    */
    ...
    ck_tile::DeviceMem d_f16_buf(d_f16_host);
    ck_tile::DeviceMem sa_buf(sa_host);
    ck_tile::DeviceMem sb_buf(sb_host);

    ck_tile::DeviceMem dbg_int_buf(dbg_int);
    ck_tile::DeviceMem dbg_fp8_buf(dbg_fp8);
    ck_tile::DeviceMem dbg_f16_buf(dbg_f16);
    ...
    flatmm_uk_args args{a_buf.GetDeviceBuffer(),
                        ...
                        d_f16_buf.GetDeviceBuffer(),
                        dbg_int_buf.GetDeviceBuffer(),
                        dbg_fp8_buf.GetDeviceBuffer(),
                        dbg_f16_buf.GetDeviceBuffer(),
    ...
    float ave_time = flatmm_uk(traits, args, ck_tile::stream_config{nullptr, true, kname ? 1 : 0, warmup, repeat});
    if(ave_time < 0)
    {
    ...
    std::cout << "Perf: " << ave_time * 1.E3 << " us, " << cal_tflops(ave_time) << " tflops, " << cal_tbps(ave_time)
              << " TB/s" << std::endl;
    ...
    if(do_validation)
    {
        ...
        d_host.SetZero();
        d_f16_host.SetZero();
        my_reference_gemm<ADataType, BDataType, CDataType, AScaleDataType, GScaleDataType, DDataType, ODataType>(a_host, b_host, sa_host, sb_host, d_host, d_f16_host);
        
        //auto d_dev = d_buf.ToHost<float>();
        //pass = ck_tile::check_err(d_dev, d_host);
        auto d_f16_dev = d_f16_buf.ToHost<ODataType>();
        pass = ck_tile::check_err(d_f16_dev, d_f16_host);
...
int main(int argc, char* argv[])
{
    ...
    else if(prec_i == "fp8" && prec_w == "fp8" && prec_o == "bf16" && prec_kw == "fp32")
    {
        return run<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::bf16_t, float, float, float, float>(
                   arg_parser)
                   ? 0
                   : -2;
    }
    else if(prec_i == "fp8" && prec_w == "fp8" && prec_o == "fp16" && prec_kw == "fp32")
    {
        return run<ck_tile::fp8_t, ck_tile::fp8_t, ck_tile::fp16_t, float, float, float, float>(
                   arg_parser)
                   ? 0
                   : -2;
    }
