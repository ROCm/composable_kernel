...
run_fp16_bf16_tests() {
    ...
    for mode in 1 0 ; do
    for perm in 0 1 ; do
    for hdim in 96 160 192 ; do
    for bias in "n" "e" "a" ; do

    # run_exe -prec=$prec -mode=$mode -b=1 -h=1 -d=$hdim -s=1024 -bias=$bias -p_drop=$p_drop -lse=$lse -iperm=$perm -operm=$perm  -num_splits=$num_splits -page_block_size=$page_block_size -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=2 -h=2 -h_k=1 -d=16    -d_v=$hdim -s=55   -s_k=256            -bias=$bias -iperm=$perm -operm=$perm                -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=1 -h=3        -d=$hdim            -s=100  -s_k=51             -bias=$bias -iperm=$perm -operm=$perm                -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=2 -h=1        -d=16    -d_v=$hdim -s=99   -s_k=256            -bias=$bias -iperm=$perm -operm=$perm -mask=1        -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=1 -h=2 -h_k=1 -d=$hdim            -s=1024 -s_k=256            -bias=$bias -iperm=$perm -operm=$perm -mask=2        -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=2 -h=1        -d=$hdim -d_v=24    -s=3    -s_k=99             -bias=$bias -iperm=$perm -operm=$perm -mask=2        -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=3 -h=2 -h_k=1 -d=$hdim            -s=200  -s_k=520            -bias=$bias -iperm=$perm -operm=$perm -mask=t:128,30 -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=2 -h=1        -d=$hdim            -s=99   -s_k=32             -bias=$bias -iperm=$perm -operm=$perm -mask=b:4,35   -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=1 -h=2 -h_k=1 -d=$hdim            -s=33   -s_k=0              -bias=$bias -iperm=$perm -operm=$perm -mask=2        -kname=$KNAME $COMMON_ARGS  
    run_exe -prec=$prec -mode=$mode -b=1 -h=2 -h_k=1 -d=$hdim            -s=1    -s_k=10  -s_kpad=32 -bias=$bias -iperm=$perm -operm=$perm -mask=2        -kname=$KNAME $COMMON_ARGS  
