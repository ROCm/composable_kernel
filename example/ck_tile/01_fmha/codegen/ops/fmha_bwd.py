...
class FmhaBwdApiTrait:
    ...
    @property
    def dcheck(self) -> str:
        if self.dpad == 't': return f'true /*a.hdim_q % {self.bhdq} != 0*/'
        else:                return f'a.hdim_q % {self.bhdq} == 0'

    @property
    def dvcheck(self) -> str:
        if self.dvpad == 't': return f'true /*a.hdim_v % {self.bhdv} != 0*/'
    ...
    def dq_dk_dv_kernel(self) -> FmhaBwdDQDKDVKernel:
        return FmhaBwdDQDKDVKernel(F_archs=[self.arch], F_idx=self.idx, F_hdim=self.hdim, F_dtype=self.dtype, F_tile=self.tile,
    ...
    def convert_dq_kernel(self) -> FmhaBwdConvertQGradKernel:
        ...
        return FmhaBwdConvertQGradKernel(F_archs=[self.arch], F_idx=self.idx, F_hdim=self.hdim, F_dtype=self.dtype,
...
class FmhaBwdApiPool:
    ...
    def _api_inners(self, traits: List[FmhaBwdApiTrait]) -> str:
        ...
        for i_trait, trait in enumerate(traits):
            inners += FMHA_BWD_API_INNER_DISPATCH.format(F_if=if_(i_trait), F_mode=MODE_MAP[trait.mode],
                ...
                F_convert_dq_enabled=BOOL_MAP[not trait.convert_dq_kernel.disabled], F_max_seq_q_cond=trait.max_seq_q_cond, F_cond_extra=trait.extra_cond,
                F_bn0=trait.tile.F_bn0, F_convert_dq_bn0=trait.convert_dq_bn0)
        return inners
    ...
    def max_seq_q_sort_key(trait):
        return trait.tile.max_seq_q if trait.tile.max_seq_q != 0 else 1000000  # sort 0 to the end
        ...
        for i_arch, (arch, pool_by_arch) in enumerate(self.dq_dk_dv_pool.items()):
            ...
            for i_dtype, (dtype, pool_by_dtype) in enumerate(pool_by_arch.items()):
                ...
                for i_hdim, (hdim, pool_by_hdim) in enumerate(pool_by_dtype.items()):
                    ...
                    per_hdim_case += FMHA_BWD_API_COND_STATEMENT(if_i=i_hdim, F_cond=self.hdim_cond(hdim), F_body=inners)
                per_dtypes += FMHA_BWD_API_COND_STATEMENT(if_i=i_dtype, F_cond=self.dtype_cond(dtype), F_body=per_hdim_case)
            per_arch += FMHA_BWD_API_COND_STATEMENT(if_i=i_arch, F_cond=self.arch_cond(arch), F_body=per_dtypes)
        if not per_arch:
            # empty string we add some ignore to suppress warning in api
            per_arch = '(void)t ; (void)s ; (void)a;'
        result = FMHA_BWD_KERNEL_HEADER + FMHA_BWD_API.format(F_dispatch=indent(per_arch))
...
def get_bwd_blobs(targets: List[str], filter_list: str, receipt, mask_impl, optdim_list) -> Tuple[FmhaBwdApiPool, List[FmhaBwdOGradDotOKernel], List[FmhaBwdDQDKDVKernel], List[FmhaBwdConvertQGradKernel]]:
    ...
    gen_dot_do_o: List[FmhaBwdOGradDotOKernel] = []
    gen_dq_dk_dv: List[FmhaBwdDQDKDVKernel] = []
    gen_convert_dq: List[FmhaBwdConvertQGradKernel] = []
    ...
    for factory, dtype, tr_load in itertools.product(factories, BWD_DTYPE_MAP.keys(), ["t", "f"]):
        ...
        for tile, mode, mask, bias, dbias, dropout, spad1d, dpad, dvpad, deterministic in itertools.product(tiles, MODE_MAP.keys(), get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"], DROPOUT_MAP.keys(), *([["f", "t"]] * 3), ["t", "f"]):
            ...
            if optdim_list != [-1]:
                if hdim not in optdim_list:
                    continue
            t = FmhaBwdApiTrait(arch=arch, idx=0, hdim=hdim, dtype=dtype, mode=mode, tile=tile, mask=mask, bias=bias, dbias=dbias, dropout=dropout, spad1d=spad1d, dpad=dpad, dvpad=dvpad, deterministic=deterministic, mask_impl=mask_impl, tr_load=tr_load)
            ...
            # aiter::mha_bwd C++ api integration
                ...
                if not cond:
                    continue
            gen_dot_do_o.append(t.dot_do_o_kernel)
            gen_dq_dk_dv.append(t.dq_dk_dv_kernel)
            if not t.convert_dq_kernel.disabled:
                gen_convert_dq.append(t.convert_dq_kernel)
            api_pool.register_dq_dk_dv_traits(t)

    return api_pool, group_kernels_by_filename(gen_dot_do_o), group_kernels_by_filename(gen_dq_dk_dv), group_kernels_by_filename(gen_convert_dq)
