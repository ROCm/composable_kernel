...
FMHA_FWD_SPLITKV_API_INNER_DISPATCH="""            {F_if}((t.is_group_mode == {F_mode}) && (t.is_v_rowmajor == {F_vlayout}) && ({F_mask_check}) && (t.bias_type == {F_bias_check}) && (t.has_lse == {F_lse}) && (t.do_fp8_static_quant == {F_squant}) &&
                ...
                using traits2_ = fmha_fwd_splitkv_combine_traits_<{F_hdim}, {F_dtype}, {F_mode}, {F_bm0}, {F_bn1}/2, {F_lse}, {F_squant}, {F_spad}, {F_dvpad}>;
    ...
    def get_pipelines(dtype, hdim) -> List[FmhaFwdSplitKVPipeline]:
        ...
        if dtype in ['fp16', 'bf16']:
            for mask, bias, lse, pagedkv in itertools.product(get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"], ["t", "f"]):
                ...
                # if True:
                    pipelines.append(Pipeline('qr_smallq', 'row', 'f', 't', 'f', 'f', bias, lse, squant, pagedkv, mask))
                    pipelines.append(Pipeline('qr_smallq', 'col', 'f', 't', 'f', 'f', bias, lse, squant, pagedkv, mask))

                    pipelines.append(Pipeline('qr_smallq', 'row', 't', 't', 't', 't', bias, lse, squant, pagedkv, mask))
                    pipelines.append(Pipeline('qr_smallq', 'col', 't', 't', 't', 't', bias, lse, squant, pagedkv, mask))
                else:
