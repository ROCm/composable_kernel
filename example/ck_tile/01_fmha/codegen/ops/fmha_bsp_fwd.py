...
PIPELINE_MAP = {
    "qr_async" : "ck_tile::BlockFmhaBspPipelineQRKSVSAsync",
}

PIPELINE_ENUM_MAP = {
    "qr_async" : "ck_tile::BlockFmhaBspPipelineEnum::QRKSVS_ASYNC",
}
...
using fmha_trait_{F_idx} = ck_tile::TileFmhaBspTraits<{F_spad},
                                                    {F_skpad},
                                                    {F_dpad},
                                                    {F_dvpad},
                                                    {F_bias},
...
using fmha_pipeline_problem_{F_idx} = ck_tile::BlockFmhaBspPipelineProblem<
    ...
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::SaccDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::SMPLComputeDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::BiasDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::RandValOutputDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::LSEDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::PDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::OaccDataType,
    typename FmhaBspFwdTypeConfig<fmha_dtype_{F_idx}>::ODataType,
    fmha_shape_{F_idx},
    {F_mode},
    fmha_mask_{F_idx},
    fmha_trait_{F_idx}>;
...
using fmha_epilogue_{F_idx} =
    ck_tile::Default2DEpilogue<ck_tile::Default2DEpilogueProblem<typename FmhaBspFwdTypeConfig<{F_dtype}>::OaccDataType,
                                           typename FmhaBspFwdTypeConfig<{F_dtype}>::ODataType,
                                           {F_spad}, {F_dvpad}>>;
...
template<>
float fmha_bsp_fwd_<trait_{F_idx}>(const ck_tile::stream_config& s, fmha_bsp_fwd_args a)
{{
    using k_ = fmha_kernel_{F_idx};
    if(s.log_level_ > 0)
        std::cout << ", " << k_::GetName() << std::flush;
    auto [kargs, grids] = fmha_bsp_fwd_create_kargs_and_grids<k_>(a);
...
class FmhaBspFwdKernel:
    ...
    F_tile          : FmhaBspFwdTileSize
    F_pipeline      : FmhaBspFwdPipeline
    mask_impl       : str
    ...
    def name(self) -> str:
        ...
        return f"fmha_bsp_fwd_d{self.F_hdim}_{self.F_dtype}_{self.F_mode}_" + \
...
def get_fwd_blobs(kernel_filter : Optional[str], receipt, optdim_list, mask_impl) -> Tuple[FmhaBspFwdApiPool, List[FmhaBspFwdKernel]]:
    ...
    def get_pipelines(dtype, hdim) -> List[FmhaBspFwdPipeline]:
        ...
        if dtype in ['fp16', 'bf16']:
            for mask, bias, lse, dropout in itertools.product(get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"], ["t", "f"]):
                pipelines.append(FmhaBspFwdPipeline('qr_async', 'row', 't', 'f', 't', 't', bias, lse, dropout, squant, mask))
                pipelines.append(FmhaBspFwdPipeline('qr_async', 'row', 't', 't', 't', 't', bias, lse, dropout, squant, mask))
                pipelines.append(FmhaBspFwdPipeline('qr_async', 'col', 't', 'f', 't', 't', bias, lse, dropout, squant, mask))
                pipelines.append(FmhaBspFwdPipeline('qr_async', 'col', 't', 't', 't', 't', bias, lse, dropout, squant, mask))
                # if hdim == 256:
                # # if True:
                #     pipelines.append(FmhaBspFwdPipeline('qr', 'row', 'f', 'f', 'f', 'f', bias, lse, dropout, squant, mask))
                #     pipelines.append(FmhaBspFwdPipeline('qr', 'col', 'f', 'f', 'f', 'f', bias, lse, dropout, squant, mask))
                #     # the below two is used for hdim vectorize load
                #     pipelines.append(FmhaBspFwdPipeline('qr', 'row', 't', 't', 'f', 'f', bias, lse, dropout, squant, mask))
                #     pipelines.append(FmhaBspFwdPipeline('qr', 'col', 't', 't', 'f', 'f', bias, lse, dropout, squant, mask))

                #     pipelines.append(FmhaBspFwdPipeline('qr', 'row', 't', 't', 't', 't', bias, lse, dropout, squant, mask))
                #     pipelines.append(FmhaBspFwdPipeline('qr', 'col', 't', 't', 't', 't', bias, lse, dropout, squant, mask))
                # else:
                #     if bias == "bias":
                ...
                #         pipelines.append(FmhaBspFwdPipeline('qr', 'row', 't', 't', 't', 't', bias, lse, dropout, squant, mask)) # TODO: cover arbitraty hdim
                #         pipelines.append(FmhaBspFwdPipeline('qr', 'col', 't', 'f', 't', 't', bias, lse, dropout, squant, mask)) # TODO: cover arbitraty hdim
        # elif dtype in ['fp8', 'bf8']:
        #     # no need lse/dropout kernels
        #     for mask, bias in itertools.product(get_mask_map(mask_impl).keys(), BIAS_MAP.keys()):
        #         pipelines.append(FmhaBspFwdPipeline('qr', 'col', 'f', 'f', 'f', 'f', bias, 'f', 'f', squant, mask))
...
def write_single_fwd_kernel(kernel: FmhaBspFwdKernel, autogen_dir: Path) -> None:
    (autogen_dir / kernel.filename).write_text(kernel.template)

def write_fwd_api(api_pool : FmhaBspFwdApiPool, autogen_dir: Path) -> None:
