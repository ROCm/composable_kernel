    ...
    def get_pipelines(dtype, hdim, hdim_v, receipt, mask_impl) -> List[FmhaFwdPipeline]:
        ...
        if dtype in ['fp32']:
            for logits, mask, bias, lse, dropout, skip in itertools.product(["t", "f"], get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"], ["t", "f"], ["t", "f"]):
                ...
                pipelines.append(FmhaFwdPipeline('qr', 'row', 'f', 't', 'f', 'f', logits, bias, lse, dropout, squant, mask, skip))
                ...
                pipelines.append(FmhaFwdPipeline('qr', 'col', 'f', 't', 'f', 'f', logits, bias, lse, dropout, squant, mask, skip))
