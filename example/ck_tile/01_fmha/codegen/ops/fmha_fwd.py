...
class FmhaFwdApiPool:
    ...
    def register_traits(self, trait : FmhaFwdApiTrait) -> None:
        ...
        fields = [f.name for f in dataclasses.fields(FmhaFwdApiTrait)]
    ...
    def api(self) -> str:
        ...
        for i_arch, (arch, pool_by_arch) in enumerate(self.pool.items()):
            ...
            for i_dtype, (dtype, pool_by_dtype) in enumerate(pool_by_arch.items()):
                per_hdim_case = str()
                for i_hdim, ((hdim, hdim_v), pool_by_hdim) in enumerate(pool_by_dtype.items()):
                    ...
                    for i_trait, trait in enumerate(pool_by_hdim):
                        inners += FMHA_FWD_API_INNER_DISPATCH.format(F_if=if_(i_trait), F_mode=MODE_MAP[trait.mode], F_vlayout=LAYOUT_MAP[trait.vlayout],
                                       ...
                                       F_lse=BOOL_MAP[trait.lse], F_dropout=BOOL_MAP[trait.dropout], F_skip=BOOL_MAP[trait.skip], F_trload=BOOL_MAP[trait.trload],
                    ...
                    per_hdim_case += FMHA_FWD_API_PER_HDIM_CASE.format(F_if=if_(i_hdim), F_hdim=hdim, F_hdim_v=hdim_v, F_inner_dispatch=indent(inners))
...
class FmhaFwdKernel:
    ...
    def api_trait(self) -> FmhaFwdApiTrait:
        return FmhaFwdApiTrait(
                ...
                trload=self.F_pipeline.F_trload,
...
class KernelComponentFactoryGfx950(KernelComponentFactoryGfx9):
    ...
    @staticmethod
    def get_pipelines(dtype, hdim, hdim_v, receipt, mask_impl) -> List[FmhaFwdPipeline]:
        pipelines = KernelComponentFactoryGfx9.get_pipelines(dtype, hdim, hdim_v, receipt, mask_impl)
        if dtype in ['fp16', 'bf16']:
            squant = 'f'
            for logits, mask, bias, lse, dropout, skip in itertools.product(["t", "f"], get_mask_map(mask_impl).keys(), BIAS_MAP.keys(), ["t", "f"], ["t", "f"], ["t", "f"]):
...
def get_fwd_blobs(targets: List[str], kernel_filter : Optional[str], receipt, optdim_list, mask_impl) -> Tuple[FmhaFwdApiPool, List[FmhaFwdKernel]]:
    gen = list()
...
def write_blobs(targets: List[str], output_dir : Path, kernel_filter : str, receipt, optdim_list, mask_impl) -> None:
    api_pool, kernels = get_fwd_blobs(targets, kernel_filter, receipt, optdim_list, mask_impl)
...
def list_blobs(targets: List[str], file_path : Path, kernel_filter : str, receipt, optdim_list, mask_impl) -> None:
    with file_path.open('a') as f:
        _, kernels = get_fwd_blobs(targets, kernel_filter, receipt, optdim_list, mask_impl)
        for kernel in kernels:
