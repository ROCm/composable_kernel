...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    auto fmha_args   = [&]() {
        ...
        const ck_tile::index_t stride_v       = (i_perm ? batch * nhead_k * hdim_v : nhead_k * hdim_v);
        ...
        const ck_tile::index_t stride_do      = (o_perm ? batch * nhead * hdim_v : nhead * hdim_v);
        const ck_tile::index_t stride_dq_acc  = hdim_q;
        const ck_tile::index_t stride_dk      = (i_perm ? batch * nhead* hdim_q : nhead * hdim_q);
        const ck_tile::index_t stride_dv      = (i_perm ? batch * nhead* hdim_v : nhead * hdim_v);
        ...
        const ck_tile::index_t batch_stride_v       = (i_perm? nhead_k * hdim_v : nhead_k * shape_seqlen_k * hdim_v);
        ...
        const ck_tile::index_t batch_stride_do      = (o_perm? nhead * hdim_v : nhead * shape_seqlen_q * hdim_v);
        const ck_tile::index_t batch_stride_lsed    = (nhead * shape_seqlen_q);
        const ck_tile::index_t batch_stride_dk      = (i_perm? nhead * hdim_q : nhead * shape_seqlen_k * hdim_q);
        const ck_tile::index_t batch_stride_dv      = (i_perm? nhead * hdim_q : nhead * shape_seqlen_k * hdim_v);
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        // clang-format off
        // permute
        if(o_perm) o_host_ref.ForEach([&](auto& self, auto idx) { o_host(idx[1] + query_offset, b, idx[0], idx[2]) = self(idx); });
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        // clang-format off
        if(o_perm) do_host_ref.ForEach([&](auto& self, auto i) { self(i) = do_host(i[1] + query_offset, b, i[0], i[2]); });
        ...
        // clang-format off
        // permute
        if(i_perm) dq_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dq_host(idx[1] + query_offset, b, idx[0], idx[2]); });
        else       dq_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dq_host(b, idx[1] + query_offset, idx[0], idx[2]); });

        if(i_perm) dk_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dk_host(idx[1] + key_offset, b, idx[0], idx[2]); });
        else       dk_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dk_host(b, idx[1] + key_offset, idx[0], idx[2]); });

        if(i_perm) dv_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dv_host(idx[1] + key_offset, b, idx[0], idx[2]); });
