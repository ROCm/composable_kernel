...
auto create_args(int argc, char* argv[])
{
    ...
    arg_parser.insert("v", "1", "weather do CPU validation or not")
        ...
        .insert("iperm",
                "1",
                "permute input\n"
                "if true, will be bs3hd, else sb3hd")//here fixed to bs3hd as ck acknowledge no sb3hd
...
bool run(const ck_tile::ArgParser& arg_parser)
{
    ...
    ck_tile::HostTensor<QDataType> qkv_host(
        std::array<ck_tile::index_t, 5>{shape_batch, shape_seqlen_q, 3, nhead, hdim_q});
    ...
    auto layout_str = [&](bool permute){
        if (permute) return std::string("bs3hd");
        else return std::string("bs3hd");
    };
    ...
    auto fmha_args   = [&]() {
        ...
        return fmha_bwd_args{qkv_buf.GetDeviceBuffer(),
                             static_cast<void*>(static_cast<int8_t*>(qkv_buf.GetDeviceBuffer()) + nhead*hdim_q*sizeof(QDataType)),
                             static_cast<void*>(static_cast<int8_t*>(qkv_buf.GetDeviceBuffer()) + 2*nhead*hdim_q*sizeof(QDataType)),
                             ...
                             static_cast<void*>(static_cast<int8_t*>(dqkv_buf.GetDeviceBuffer()) + nhead*hdim_q*sizeof(QDataType)),
                             static_cast<void*>(static_cast<int8_t*>(dqkv_buf.GetDeviceBuffer()) + 2*nhead*hdim_q*sizeof(QDataType)),
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        // clang-format off
        q_host_ref.ForEach([&](auto& self, auto i) { self(i) = qkv_host(b, i[1] + query_offset, 0, i[0], i[2]); });
        k_host_ref.ForEach([&](auto& self, auto i) { self(i) = qkv_host(b, i[1] + key_offset, 1, i[0] / nr, i[2]); });
        v_host_ref.ForEach([&](auto& self, auto i) { self(i) = qkv_host(b, i[2] + key_offset, 2, i[0] / nr, i[1]);});
    ...
    for(ck_tile::index_t wb = 0; wb < batch; ++wb)
    {
        ...
        // permute
        // dqkv is bs3hd
        dq_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dqkv_host(b, idx[1] + query_offset, 0, idx[0], idx[2]); });
        dk_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dqkv_host(b, idx[1] + key_offset, 1, idx[0], idx[2]); });
        dv_host_result.ForEach([&](auto& self, auto idx) {self(idx) = dqkv_host(b, idx[1] + key_offset, 2, idx[0], idx[2]); });
