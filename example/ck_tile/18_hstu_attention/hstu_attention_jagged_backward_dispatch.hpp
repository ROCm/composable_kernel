...
// Backward dispatch for jagged (variable-length, non-group) mode.
...
struct jagged_backward_dispatch
{
    ...
    static void Run(HstuAttentionNoGroupBwdParams& param, hipStream_t stream)
    {
        ...
        // In jagged mode seqlen_k always requires padding since it varies per sample.
    ...
    {
        // Delta layout in jagged mode: flat over the sequence dimension, strided by head.
...
template <typename InOutDataType,
          ...
          bool kHasDropout,
          ck_tile::index_t MaxK>
void run_jagged_backward_dispatch(HstuAttentionNoGroupBwdParams& param, hipStream_t stream)
{
    jagged_backward_dispatch<InOutDataType, kUseCausal, kUseSoftmax, kHasBias, kHasDropout, MaxK>::
        Run(param, stream);
}
