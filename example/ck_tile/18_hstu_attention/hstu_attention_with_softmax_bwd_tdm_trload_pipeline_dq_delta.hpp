...
struct HstuAttentionWithSoftmaxBwdTdmTrLoadPipelineQRKSVS_dQ_D
{
    ...
    static constexpr index_t kM0        = HstuAttentionTileSetting::kM;
               ...
               DropoutType& dropout) const
    {
        ...
        //   [k_lds | v_lds]
        // k_lds : two complete-buffered [kN0Sub, kQKHeaddim], each used for tdm write/normal read
        // and transposed read using trload; one used for even iteration, the other used for odd
        // iteration
        //
        ...
        auto k_lds_monolithic_window_0 = make_tile_window(
            k_lds, Policy::template MakeKLdsBlockDescriptor<Problem>().get_lengths(), {0, 0});

        auto k_lds_monolithic_window_1 = k_lds_monolithic_window_0;
        k_lds_monolithic_window_1.set_bottom_tensor_view_data_ptr(
            k_lds_ptr + k_smem_size / sizeof(QKVDataType));

        static_assert(
            Policy::template MakeKLdsBlockDescriptor<Problem>().get_lengths()[number<0>{}] == kN0,
            "Check failed!");
        static_assert(
        ...
        // k_lds windows for tdm write and normal read
        using k_lds_window_type = decltype(get_slice_tile(
            k_lds_monolithic_window_0, sequence<0, 0>{}, sequence<kN0Sub, kQKHeaddim>{}));
        statically_indexed_array<k_lds_window_type, n0_loops> k_lds_windows_0;
        statically_indexed_array<k_lds_window_type, n0_loops> k_lds_windows_1;
        static_for<0, n0_loops, 1>{}([&](auto i_buf) {
            k_lds_windows_0[i_buf] = get_slice_tile(k_lds_monolithic_window_0,
                                                    sequence<i_buf * kN0Sub, 0>{},
                                                    sequence<(i_buf + 1) * kN0Sub, kQKHeaddim>{});
            k_lds_windows_1[i_buf] = get_slice_tile(k_lds_monolithic_window_1,
                                                    sequence<i_buf * kN0Sub, 0>{},
                                                    sequence<(i_buf + 1) * kN0Sub, kQKHeaddim>{});
        });
        ...
        using k_lds_trload_window_type = decltype(get_slice_tile(
            k_lds_monolithic_window_0, sequence<0, 0>{}, sequence<kK1, kQKHeaddim>{}));
        statically_indexed_array<k_lds_trload_window_type, k1_loops> k_lds_trload_windows_0;
        statically_indexed_array<k_lds_trload_window_type, k1_loops> k_lds_trload_windows_1;
        static_for<0, k1_loops, 1>{}([&](auto i_buf) {
            k_lds_trload_windows_0[i_buf] =
                get_slice_tile(k_lds_monolithic_window_0,
                               sequence<i_buf * kK1, 0>{},
                               sequence<(i_buf + 1) * kK1, kQKHeaddim>{});
            k_lds_trload_windows_1[i_buf] =
                get_slice_tile(k_lds_monolithic_window_1,
                               sequence<i_buf * kK1, 0>{},
                               sequence<(i_buf + 1) * kK1, kQKHeaddim>{});
        });

        QKVDataType* v_lds_ptr =
            reinterpret_cast<QKVDataType*>(static_cast<char*>(smem_ptr) + k_smem_size * 2);
        ...
        constexpr auto pad_cfg_k             = Policy::template GetKLdsPaddingConfig<Problem>();
        tdm_config_k.pad_enable              = pad_cfg_k[number<0>{}];
        tdm_config_k.pad_config.pad_amount   = pad_cfg_k[number<1>{}];
        tdm_config_k.pad_config.pad_interval = pad_cfg_k[number<2>{}];

        TDMConfig tdm_config_v;
        constexpr auto pad_cfg_v             = Policy::template GetVLdsPaddingConfig<Problem>();
        tdm_config_v.pad_enable              = pad_cfg_v[number<0>{}];
        tdm_config_v.pad_config.pad_amount   = pad_cfg_v[number<1>{}];
        tdm_config_v.pad_config.pad_interval = pad_cfg_v[number<2>{}];

        // Ensure the delta shuffle reads are done by all warps before the K TDM writes
        // reuse the aliased delta_lds space at smem_ptr.
        block_sync_lds();

        load_tile_tdm(tdm_config_k, k_lds_windows_0[number<0>{}], k_dram_window);
        move_tile_window(k_dram_window, {kN0Sub, 0});

        load_tile_tdm(tdm_config_v, v_lds_windows[number<0>{}], v_dram_window);
        ...
        do
        {
            ...
            static_for<0, n0_loops, 1>{}([&](auto i_n0) {
                ...
                constexpr auto i_lds_buf_1 = number<(i_n0 + 1) % NumVLdsBuffers>{};
                ...
                if(even_iter)
                {
                    gemm_0(sacc_tile, q_tile, k_lds_windows_0[i_n0]);

                    if constexpr(i_n0 < n0_loops - 1)
                    {
                        load_tile_tdm(
                            tdm_config_k, k_lds_windows_0[number<i_n0 + 1>{}], k_dram_window);
                        move_tile_window(k_dram_window, {kN0Sub, 0});
                    }
                }
                else
                {
                    gemm_0(sacc_tile, q_tile, k_lds_windows_1[i_n0]);

                    if constexpr(i_n0 < n0_loops - 1)
                    {
                        load_tile_tdm(
                            tdm_config_k, k_lds_windows_1[number<i_n0 + 1>{}], k_dram_window);
                        move_tile_window(k_dram_window, {kN0Sub, 0});
                    }
                }
                ...
                if constexpr(i_n0 < n0_loops - 1)
                {
                    load_tile_tdm(tdm_config_v, v_lds_windows[i_lds_buf_1], v_dram_window);
                    move_tile_window(v_dram_window, {kN0Sub, 0});
                }
            ...
            if(even_iter)
            {
                load_tile_tdm(tdm_config_k, k_lds_windows_1[number<0>{}], k_dram_window);
                move_tile_window(k_dram_window, {kN0Sub, 0});
            }
            else
            {
            ...
            if constexpr(kHasDropout)
            {
                ...
                auto randval_lds_ptr =
                    reinterpret_cast<char*>(smem_ptr) + k_smem_size * 2 + v_smem_size;
            ...
            // K is already staged in k_lds_trload_windows from Stage 1 and read transposed via
            // trload.
            static_for<0, k1_loops, 1>{}([&](auto i_k1) {
                ...
                if(even_iter)
                {
                    gemm_4(dq_acc, ds_slice, k_lds_trload_windows_0[i_k1]);
                }
                else
                {
                    gemm_4(dq_acc, ds_slice, k_lds_trload_windows_1[i_k1]);
                }
            });
