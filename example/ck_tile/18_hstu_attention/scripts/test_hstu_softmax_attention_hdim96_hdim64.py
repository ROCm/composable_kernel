#!/usr/bin/env python3
"""Run the HSTU softmax attention forward hdim96/hdim64 correctness tests.

Python port of test_hstu_softmax_attention_hdim96_hdim64.sh, keeping the same
...
are hardcoded to match the original shell script. The training flag is read
from the TEST_HSTU_FWD_TRAINING environment variable (default 0). The body
sweeps hdim over 96 then 64, running the 14 test cases for each.
"""
...
# Training flag read from the environment (Training=${TEST_HSTU_FWD_TRAINING:-0}).
...
def main():
    parser = argparse.ArgumentParser(
        description="Run HSTU softmax attention forward hdim96/hdim64 "
                    "correctness tests.")
    parser.parse_args()
