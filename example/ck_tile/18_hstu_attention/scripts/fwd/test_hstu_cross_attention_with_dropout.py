#!/usr/bin/env python3
"""Run the HSTU cross-attention forward correctness tests with dropout.

Python port of test_hstu_cross_attention_with_dropout.sh, keeping the same
functionality and argument list:

    test_hstu_cross_attention_with_dropout.py [USE_SOFTMAX]
...
prefix. The EXE prefix always ends with -p_drop=0.2. The Training value is read
from the TEST_HSTU_FWD_TRAINING environment variable (default 0).
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run HSTU cross-attention forward correctness tests with "
                    "dropout.")
    ...
    if args.use_softmax == 1:
        exe = [BINARY, "-softmax=1", f"-training={training}", "-p_drop=0.2"]
    else:
        exe = [BINARY, f"-training={training}", "-p_drop=0.2"]
