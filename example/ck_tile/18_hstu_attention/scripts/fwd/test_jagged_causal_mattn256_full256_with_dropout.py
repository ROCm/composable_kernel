#!/usr/bin/env python3
"""Run the HSTU attention forward jagged-causal (mattn256/full256) dropout tests.

Python port of test_jagged_causal_mattn256_full256_with_dropout.sh, keeping the
same functionality and argument list:

    test_jagged_causal_mattn256_full256_with_dropout.py [use_softmax]

The optional positional argument defaults to 0, matching the original shell
script. When use_softmax is 1, the executable is invoked with -softmax=1. The
executable always runs with -p_drop=0.2.
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Run HSTU attention forward jagged-causal mattn256/full256 dropout tests.")
    ...
    if args.use_softmax == 1:
        exe = [BIN, "-softmax=1", f"-training={training}", "-p_drop=0.2"]
    else:
        exe = [BIN, f"-training={training}", "-p_drop=0.2"]
