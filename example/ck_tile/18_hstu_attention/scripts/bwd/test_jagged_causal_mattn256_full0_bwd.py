#!/usr/bin/env python3
"""Run the HSTU attention backward jagged-causal (mattn256/full0) tests.

Python port of test_jagged_causal_mattn256_full0_bwd.sh, keeping the same
functionality and argument list:

    test_jagged_causal_mattn256_full0_bwd.py [norm_dist]

The single optional positional argument sets norm_dist and defaults to 0,
matching the original shell script. This script has no softmax option.
"""
...
def run(exe_prefix, b, seqlen, targets, ndist):
...
def main():
    ...
    for b, targets in cases:
        for seqlen in SEQLENS:
            ret = run(exe_prefix, b, seqlen, targets, ndist)
            if ret != 0:
                rc = ret
