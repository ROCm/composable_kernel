#!/usr/bin/env python3
"""Run the HSTU attention backward correctness tests for hdim 96 and 64 with dropout.

Python port of test_hstu_attention_hdim96_hdim64_with_dropout_bwd.sh, keeping the
same functionality.
...
def run(exe, dtype, hdim, attn_scale, ndist, **kwargs):
    ...
    cmd = (
        [exe, "-softmax=0", "-p_drop=0.2", "-v=1", f"-prec={dtype}", "-b=10"]
