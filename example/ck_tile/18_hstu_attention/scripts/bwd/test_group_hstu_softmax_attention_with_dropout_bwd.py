#!/usr/bin/env python3
"""Run the grouped HSTU softmax attention (with dropout) backward correctness tests.

Python port of test_group_hstu_softmax_attention_with_dropout_bwd.sh, keeping the same functionality
and argument list:

    test_group_hstu_softmax_attention_with_dropout_bwd.py [norm_dist] [extra]
...
BUILD = "build"
EXE = [f"{BUILD}/bin/tile_example_hstu_attention_bwd", "-softmax=1", "-p_drop=0.2"]
