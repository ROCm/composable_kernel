...
auto parse_cmd_args(int argc, char* argv[]) -> std::pair<bool, ck_tile::ArgParser>
{
    ...
    arg_parser
        ...
        // TODO scale factors
        .insert("fp8", "0", "0: no fp8 , 1: fp8, 2 fp8 kv")
        .insert("iperm",
                "0",
                "permute input\n"
...
template <typename DataType>
auto quantize_qkv(ck_tile::HostTensor<DataType>& q,
                  ck_tile::HostTensor<DataType>& k,
                  ck_tile::HostTensor<DataType>& v,
                  int fp8_mode) -> std::tuple<float, float, float>
{
    auto compute_scale = [](auto& tensor) -> float {
        float max_abs = 0.0f;
        tensor.ForEach([&](auto& self, auto idx) {
            float val = ck_tile::abs(static_cast<float>(self(idx)));
            max_abs   = ck_tile::max(max_abs, val);
        });
        constexpr float fp8_max = 240.0f; // Approximate max for fp8
        return (max_abs > 0.0f) ? (fp8_max / max_abs) : 1.0f;
    };
    ...
    {
        // No quantization
        return std::make_tuple(scale_q, scale_k, scale_v);
    }
    else if(fp8_mode == 1)
    {
        // Quantize all to fp8
        scale_q = compute_scale(q);
        q.ForEach([&](auto& self, auto idx) {
            float val = static_cast<float>(self(idx)) * scale_q;
            self(idx) = static_cast<DataType>(static_cast<ck_tile::fp8_t>(val));
        });

        scale_k = compute_scale(k);
        k.ForEach([&](auto& self, auto idx) {
            float val = static_cast<float>(self(idx)) * scale_k;
            self(idx) = static_cast<DataType>(static_cast<ck_tile::fp8_t>(val));
        });

        scale_v = compute_scale(v);
        v.ForEach([&](auto& self, auto idx) {
            float val = static_cast<float>(self(idx)) * scale_v;
            self(idx) = static_cast<DataType>(static_cast<ck_tile::fp8_t>(val));
        });
    }
    else
    {
        // fp8_mode == 2: quantize only k and v
        scale_k = compute_scale(k);
        k.ForEach([&](auto& self, auto idx) {
            float val = static_cast<float>(self(idx)) * scale_k;
            self(idx) = static_cast<DataType>(static_cast<ck_tile::fp8_t>(val));
...
bool run_impl(const Problem& problem, const RunConfig& run_config)
{
    ...
    auto [scale_q, scale_k, scale_v] = quantize_qkv(q, k, v, problem.fp8_mode);
    ...
    // Update problem scales if quantization was applied
    if(problem.fp8_mode != 0)
    {
        args.scale_q = scale_q;
    ...
    args.fp8_mode           = problem.fp8_mode;
