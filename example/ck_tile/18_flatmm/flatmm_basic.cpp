...
float flatmm_calc(const ck_tile::FlatmmHostArgs& args, const ck_tile::stream_config& s)
{
    ...
    //     using CodegenFlatmmPolicy = ck_tile::UniversalFlatmmPipelineAgBgCrPolicy;
    //     using CodegenFlatmmPipeline =
    //         ck_tile::FlatmmPipelineAGmemBGmemCRegV1<GemmPipelineProblem, CodegenFlatmmPolicy>;

    //     // ToDo: Will add the codegen part to test different pipeline policies in GEMM.
    //     // Now we only use the BlockGemmASmemBSmemCRegV1DefaultPolicy.
    //     using Kernel = ck_tile::FlatmmKernel<TilePartitioner, CodegenFlatmmPipeline,
    //     GemmEpilogue>;
    ...
    //                   << GemmPipelineProblem::GetName() << " grid: {" << grids.x << ", "
    //                   << grids.y << ", " << grids.z << "}"
    //                   << ", blocks: {" << blocks.x << ", " << blocks.y << ", " << blocks.z << "}"
    //                   << std::endl;
    //     }
