...
struct layernorm2d_fwd_traits_
{
    ...
    static constexpr bool is_warp_per_row = ThreadPerBlock_N_ <= CK_GPU_ARCH_WARP_SIZE;
    static_assert((ThreadPerBlock_M_ * ThreadPerBlock_N_) % CK_GPU_ARCH_WARP_SIZE == 0);
    static constexpr ck_tile::index_t total_warps =
        (ThreadPerBlock_M_ * ThreadPerBlock_N_) / CK_GPU_ARCH_WARP_SIZE;

    // num of warps along m
    static constexpr ck_tile::index_t BlockWarps_M = []() {
        if constexpr(is_warp_per_row)
        {
            static_assert(CK_GPU_ARCH_WARP_SIZE % ThreadPerBlock_N_ == 0);
            return total_warps * (CK_GPU_ARCH_WARP_SIZE / ThreadPerBlock_N_);
        }
        else
        {
            // static_assert(CK_GPU_ARCH_WARP_SIZE % ThreadPerBlock_M_ == 0);
            return total_warps / (ThreadPerBlock_N_ / CK_GPU_ARCH_WARP_SIZE);
        }
    }();
    ...
    static constexpr ck_tile::index_t BlockWarps_N = []() {
        ...
        {
            static_assert(ThreadPerBlock_N_ % CK_GPU_ARCH_WARP_SIZE == 0);
            return ThreadPerBlock_N_ / CK_GPU_ARCH_WARP_SIZE;
        }
    }();
