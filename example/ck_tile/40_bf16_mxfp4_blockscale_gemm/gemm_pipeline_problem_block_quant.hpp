...
struct GemmPipelineProblemBase_block_quant
{
    ...
    {
        constexpr index_t PackedSize =
            ck_tile::numeric_traits<remove_cvref_t<BInDataType>>::PackedSize;
        if constexpr(std::is_same_v<BLayout, ck_tile::tensor_layout::gemm::RowMajor>)
        {
            constexpr index_t pixels_per_thread =
                BlockGemmShape::kN * BlockGemmShape::kK / kBlockSize;
            return pixels_per_thread < PackedSize * VectorLoadSize / sizeof(BInDataType)
                       ? pixels_per_thread
                       : PackedSize * VectorLoadSize / sizeof(BInDataType);
        }
        else
        {
            return PackedSize * VectorLoadSize / sizeof(BInDataType);
        }
    }
...
template <typename ADataType_,
          ...
          index_t VectorSizeA_             = 1,
          index_t VectorSizeB_             = 1>
struct UniversalGemmPipelineProblem_block_quant
{
    using Traits = remove_cvref_t<Traits_>;
