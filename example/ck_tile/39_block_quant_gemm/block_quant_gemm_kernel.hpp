...
struct Block_quant_GemmKernel
{
    ...
    CK_TILE_HOST static constexpr Block_quant_GemmKernelArgs
    MakeKernelArgs(const Block_quant_GemmHostArgs& hostArgs)
    {
        ...
        const auto& a_scale_tensor_view = [&]() {
            if constexpr(std::is_same_v<ALayout, tensor_layout::gemm::RowMajor>)
            {
                return make_naive_tensor_view<address_space_enum::global>(
                    a_scale_ptr,
                    make_tuple(kargs.M,
                               (splitk_batch_offset.splitted_k + ScaleBlockK - 1) / ScaleBlockK),
            ...
            else
            {
                return make_naive_tensor_view<address_space_enum::global>(
                    a_scale_ptr,
                    make_tuple((splitk_batch_offset.splitted_k + ScaleBlockK - 1) / ScaleBlockK,
                               kargs.M),
                    make_tuple((kargs.stride_A + ScaleBlockK - 1) / ScaleBlockK, 1),
                    number<1>{},
                    number<1>{});
        ...
        const auto& a_scale_block_window = [&]() {
            if constexpr(std::is_same_v<ALayout, tensor_layout::gemm::RowMajor>)
            {
                return make_tile_window(
                    a_scale_pad_view,
                    make_tuple(number<TilePartitioner::MPerBlock>{}, number<1>{}),
                    {i_m, 0});
            }
            else
            {
                return make_tile_window(
                    a_scale_pad_view,
                    make_tuple(number<1>{}, number<TilePartitioner::MPerBlock>{}),
                    {0, i_m});
            }
        }();
        //      const auto& a_scale_block_window = [&]() {
    ...
    {
        const float* b_per_blcok_scale =
            b_scale_ptr + (block_idx_n) / ScaleBlockN * ((kargs.K + ScaleBlockK - 1) / ScaleBlockK);
        ...
        else
        {
                           ...
                           is_any_of<CDataType, fp16_t, bf16_t>::value))
            {
                RunGemm(a_ptr,
                        b_ptr,
                        c_ptr,
                        static_cast<const float*>(kargs.a_scale_ptr),
                        static_cast<const float*>(kargs.b_scale_ptr),
                        smem_ptr_0,
                        kargs,
                        splitk_batch_offset,
                        i_m,
