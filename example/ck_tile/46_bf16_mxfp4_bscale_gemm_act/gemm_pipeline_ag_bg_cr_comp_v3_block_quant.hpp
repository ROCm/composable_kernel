    ...
    : public BaseGemmPipelineAgBgCrCompV3_block_quant<Problem>
{
    ...
    static constexpr index_t GetVectorSizeC_Act()
    {
        return Policy::template GetVectorSizeC_Act<Problem>();
    }
    ...
    struct PipelineImpl<GemmPipelineScheduler::Intrawave> : public PipelineImplBase
    {
                   ...
                   void* p_smem) const
        {
            ...
            CDataType* __restrict__ p_c_lds = static_cast<CDataType*>(p_smem);
            auto c_lds                      = make_tensor_view<address_space_enum::lds>(
                p_c_lds, Policy::template MakeCLdsBlockDescriptor<Problem>());
            auto c_lds_window = make_tile_window(
                c_lds, make_tuple(number<MPerBlock>{}, number<NPerBlock>{}), {0, 0});

            auto c_read_lds_window =
                make_tile_window(c_lds,
                                 make_tuple(number<MPerBlock>{}, number<NPerBlock>{}),
                                 {0, 0},
                                 Policy::template MakeRRegBlockDescriptor<Problem>());

            store_tile(c_lds_window, c_block_tile);

            auto r_reg_tensor = make_static_distributed_tensor<CDataType>(
                Policy::template MakeRRegBlockDescriptor<Problem>());
            block_sync_lds();
            clear_tile(r_reg_tensor);
            r_reg_tensor = load_tile(c_read_lds_window);

            auto s_reg_tensor = make_static_distributed_tensor<CDataType>(
                Policy::template MakeSRegBlockDescriptor<Problem>());

            constexpr auto s_block = decltype(s_reg_tensor)::get_distributed_spans();
            sweep_tile_span(s_block[number<0>{}], [&](auto idx0) {
                sweep_tile_span(s_block[number<1>{}], [&](auto idx1) {
                    constexpr auto i_j_idx     = make_tuple(idx0, idx1);
                    constexpr auto idx1_gelu   = tile_distributed_index<idx1.impl_.at(0) * 2>{};
                    constexpr auto idx1_linear = tile_distributed_index<idx1.impl_.at(0) * 2 + 1>{};
                    constexpr auto i_j_idx_ge  = make_tuple(idx0, idx1_gelu);
                    constexpr auto i_j_idx_li  = make_tuple(idx0, idx1_linear);

                    // clamp
                    auto a_gelu   = min(r_reg_tensor(i_j_idx_ge), limit);
                    auto a_linear = clamp(r_reg_tensor(i_j_idx_li), -limit, limit);

                    // gelu
                    auto a_gelu_  = alpha * type_convert<float>(a_gelu);
                    auto out_gelu = a_gelu / (1 + ck_tile::exp(-a_gelu_));

                    // linear
                    s_reg_tensor(i_j_idx) = type_convert<CDataType>(
                        type_convert<float>(out_gelu) * type_convert<float>(a_linear + 1));
                });
            });
