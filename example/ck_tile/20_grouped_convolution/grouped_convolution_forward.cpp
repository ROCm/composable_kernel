...
int run_grouped_conv_fwd_example(ck_tile::ArgParser& arg_parser)
{
...
int main(int argc, char* argv[])
{
    ...
    {
#if CK_TILE_USE_WMMA
        return !run_grouped_conv_fwd_example<GemmWarpConfig_Wmma,
                                                        GemmTileConfig,
                                                        GemmVectorLoads>(arg_parser);
#else
        //return !run_grouped_conv_fwd_example<GemmWarpConfig_Mfma>(arg_parser);
        ck_tile::index_t num_groups_to_merge = arg_parser.get_int("num_groups_to_merge");
        if(num_groups_to_merge < 1)
        {
            // By default, we have the "num_groups_to_merge" set to -1,
            // which means we will run the example with the default config.
            return !run_grouped_conv_fwd_example<GemmWarpConfig_Mfma,
                                                        GemmTileConfig,
                                                        GemmVectorLoads>(arg_parser);
        }
        else
        {
            ...
            return !run_grouped_conv_fwd_example<GemmWarpConfig_Mfma_fwd,
                                                        GemmTileConfig_fwd,
                                                        GemmVectorLoads_fwd>(arg_parser);
        }
#endif
    }
