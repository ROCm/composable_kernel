...
// clang-format off
//                                                                                    rm  rn  tm    tn  vn    pd     rms     welford     2p
{F_instance_def}
// clang-format on
...
"""
    ...
    class h_traits:
        ...
        def trait_name(self) ->str:
            ...
            t_ += f', {self.F_Vector_N:2}, {BOOL_MAP(self.F_kPadN):5}, {BOOL_MAP(self.F_kSaveInvRms):5}, {BOOL_MAP(self.F_kFastFDiv_):5}, {BOOL_MAP(self.F_kWelford_):5}'
...
def list_blobs(args):
    ...
    for api in api_list:
        if api == 'fwd':
            rmsnorm_fwd_codegen(args.working_path, args.filter).list_blobs(args)
...
def gen_blobs(args):
    ...
    for api in api_list:
        if api == 'fwd':
            rmsnorm_fwd_codegen(args.working_path, args.filter).gen_blobs(args)
