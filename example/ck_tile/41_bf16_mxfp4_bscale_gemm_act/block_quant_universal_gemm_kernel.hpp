...
template <index_t NumATensor = 1, index_t NumBTensor = 1, index_t NumDTensor = 0>
struct Block_quant_UniversalGemmKernelArgs
{
...
struct Block_quant_UniversalGemmKernel
{
    ...
    static constexpr bool BDataTypeIsTuple =
        is_detected<is_tuple, typename GemmPipeline::BInDataType>::value;
    ...
    using BInDataType = remove_cvref_t<std::tuple_element_t<I0, BsDataType>>;
    ...
    using KernelArgs =
        Block_quant_UniversalGemmKernelArgs<AsLayout::size(), BsLayout::size(), DsLayout::size()>;

    [[nodiscard]] CK_TILE_HOST static const std::string GetName()
    {
        // clang-format off
        return concat('_', "gemm", gemm_prec_str<ADataType, BInDataType>(), GemmPipeline::GetName());
        // clang-format on
    }
    ...
    MakeGemmTensorViews(const std::array<const ADataType*, NumATensor>& as_ptr,
                        const std::array<const BInDataType*, NumBTensor>& bs_ptr,
                        const std::array<const void*, NumDTensor>& ds_ptr,
                        EDataType* e_ptr,
                        const CDataType* bias_ptr,
                        const void* b_scale_ptr,
                        ...
                        const SplitKBatchOffset& splitk_batch_offset)
    {
        ...
        const auto& e_tensor_view = [&]() {
            if constexpr(std::is_same_v<ELayout, tensor_layout::gemm::RowMajor>)
            {
                return make_naive_tensor_view<address_space_enum::global, DstInMemOp>(
                    e_ptr,
                    make_tuple(kargs.M, kargs.N / 2), // arguments not matching with flatmm.
        ...
        const auto& bias_tensor_view = [&]() {
            return make_naive_tensor_view<address_space_enum::global>(
                bias_ptr,
                make_tuple(kargs.N),
                make_tuple(1),
                number<GemmPipeline::GetVectorSizeC_Act()>{},
                number<1>{});
        }();
    ...
    CK_TILE_DEVICE static auto MakeGemmPadViews(const TensorView& views)
    {
        ...
        const auto& bs_pad_view = generate_tuple(
            [&](auto i) {
                ...
                {
                    return pad_tensor_view(b_tensor_view[i],
                                           make_tuple(number<TilePartitioner::KPerBlock / 2>{},
        ...
        const auto& ds_pad_view = generate_tuple(
            [&](auto i) {
                ...
                {
                    return pad_tensor_view(d_tensor_view[i],
                                           make_tuple(number<TilePartitioner::NPerBlock / 2>{},
                                                      number<TilePartitioner::MPerBlock>{}),
                                           sequence<false, GemmPipeline::kPadM>{});
                }
            },
        ...
        const auto& e_pad_view = [&]() {
            ...
            {
                return pad_tensor_view(e_tensor_view,
                                       make_tuple(number<TilePartitioner::MPerBlock / 2>{},
                                                  number<TilePartitioner::NPerBlock>{}),
                                       sequence<GemmPipeline::kPadM, false>{});
            }
        }();

        const auto& bias_pad_view = [&]() {
                                                   ...
                                                   const index_t i_n_c)
    {
        ...
        auto e_block_window = make_tile_window(e_pad_view,
                                               make_tuple(number<TilePartitioner::MPerBlock>{},
                                                          number<TilePartitioner::NPerBlock / 2>{}),
                                               {i_m, i_n_c});
    ...
    CK_TILE_DEVICE static void RunGemm(const std::array<const ADataType*, NumATensor>& as_ptr,
                                       ...
                                       EDataType* e_ptr,
                                       const CDataType* bias_ptr,
                                       const ScaleDataType* b_scale_ptr,
                                       void* smem_ptr_0,
                                       const KernelArgs& kargs,
                                       const SplitKBatchOffset& splitk_batch_offset,
                                       const index_t block_idx_m,
                                       const index_t block_idx_n,
                                       const index_t block_idx_n_c)
    {
        // Create Gemm tensor views, pad views and tile windows
        const auto& gemm_tensor_views_tuple =
            MakeGemmTensorViews<EpiloguePipeline::MemoryOperation>(
                as_ptr, bs_ptr, ds_ptr, e_ptr, bias_ptr, b_scale_ptr, kargs, splitk_batch_offset);

        const auto& gemm_pad_views = MakeGemmPadViews(gemm_tensor_views_tuple);
        auto gemm_tile_windows =
            MakeGemmTileWindows(gemm_pad_views, block_idx_m, block_idx_n, block_idx_n_c);
    ...
    CK_TILE_DEVICE void operator()(KernelArgs kargs) const
    {
        ...
        const index_t i_n_c = __builtin_amdgcn_readfirstlane(iN * TilePartitioner::NPerBlock / 2);
        ...
        std::array<const BInDataType*, NumBTensor> bs_ptr;
        static_for<0, NumBTensor, 1>{}([&](auto i) {
            bs_ptr[i] = static_cast<const BInDataType*>(kargs.bs_ptr[i]) +
        ...
        else
        {
                           ...
                           is_any_of<EDataType, fp16_t, bf16_t>::value))
            {
                ...
                RunGemm<scheduler_type>(as_ptr,
                                        ...
                                        kargs.ds_ptr,
                                        e_ptr,
                                        static_cast<const CDataType*>(kargs.bias_ptr),
    ...
    CK_TILE_DEVICE void operator()(KernelArgs kargs) const
    {
        ...
        while(block_id < num_work)
        {
            ...
            const index_t i_n   = __builtin_amdgcn_readfirstlane(iN * TilePartitioner::NPerBlock);
            const index_t i_n_c =
                __builtin_amdgcn_readfirstlane(iN * TilePartitioner::NPerBlock / 2);
