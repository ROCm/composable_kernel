...
struct UniversalGemmBasePolicy_block_quant
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetGlobalVectorLoadSize()
    {
        ...
        constexpr int KPerBlock     = (Type == DType::TypeB_Fp4) ? (Problem::BlockGemmShape::kK / 2)
                                      : (Type == DType::TypeC)   ? Problem::BlockGemmShape::kN
    ...
    template <typename Problem, DType Type>
    CK_TILE_HOST_DEVICE static constexpr auto GetVectorSizeB()
    {
        ...
        {
            return GetGlobalVectorLoadSize<Problem, BDataType, NPerBlock, KPerBlock, Type>();
        }
    }
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto GetVectorSizeC_Act()
    {
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeRRegBlockDescriptor()
    {
        ...
        constexpr index_t MPerBlock     = Problem::BlockGemmShape::kM;
        constexpr index_t NPerBlock     = Problem::BlockGemmShape::kN;
        constexpr index_t VecLoadSize   = GetVectorSizeC_Act<Problem>();
        constexpr index_t NumWaveGroups = Problem::NumWaveGroups;

        constexpr index_t warp_size  = get_warp_size();
        constexpr index_t num_warps  = BlockSize / get_warp_size();
        constexpr index_t LargestVec = (MPerBlock * NPerBlock) / (num_warps * warp_size);
        constexpr index_t c_vec      = VecLoadSize > LargestVec ? LargestVec : VecLoadSize;
        constexpr index_t N0         = NPerBlock / c_vec; // # of threads in X dim  //8
        constexpr index_t N1         = c_vec;             // vec load

        constexpr index_t M0 = num_warps / NumWaveGroups; // M warp
        constexpr index_t M1 = warp_size / N0;            // M th
        constexpr index_t M2 = MPerBlock / (M0 * M1);     // M loop
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeSRegBlockDescriptor()
    {
        ...
        constexpr index_t c_vec      = VecLoadSize > LargestVec ? LargestVec : VecLoadSize;
        constexpr index_t N0         = NPerBlock / c_vec; // # of threads in X dim  //8
        constexpr index_t N1         = c_vec / 2;         // vec load
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto MakeCLdsBlockDescriptor()
    {

        constexpr index_t MPerBlock = Problem::BlockGemmShape::kM;
        constexpr index_t NPerBlock = Problem::BlockGemmShape::kN;

        constexpr auto c_lds_block_desc =
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto GetSmemPackC()
    {
        using BlockGemm = remove_cvref_t<decltype(Derived::template GetBlockGemm<Problem>())>;
        constexpr index_t KPack = BlockGemm::Traits::KPack;
        return KPack;
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr index_t GetSmemSizeC()
    {
        constexpr auto c_lds_desc     = MakeCLdsBlockDescriptor<Problem>();
        constexpr index_t smem_size_c = integer_least_multiple(
            sizeof(typename Problem::CDataType) * c_lds_desc.get_element_space_size(), 16);
        return smem_size_c;
    }
    ...
    CK_TILE_HOST_DEVICE static constexpr index_t GetSmemSize()
    {
        ...
        return max(smem_size_a + smem_size_b, smem_size_c);
    }
};

// UniversalGemm Policy
struct UniversalGemmPipelineAgBgCrPolicy_block_quant
    : public UniversalGemmBasePolicy_block_quant<UniversalGemmPipelineAgBgCrPolicy>
{
    template <typename Problem>
