...
/*
 * Tutorial 14, Scenario 3: Row-Major with Minimal Padding
 *
 * Demonstrates reduced bank conflicts by adding minimal padding per row.
 *
 * Layout: Row-major [M, K] with 2-element (4-byte) padding per row
 * Write: Row-wise with extra stride
 * Read: Column-wise / transpose (significantly reduced conflicts)
 *
 * Without padding (64-byte rows):
 ...
 *
 * With 4-byte padding (68-byte stride):
 *   GCD(68, 128) = 4 -> cycles through all 32 banks
 *   Pattern spreads accesses across banks much better
 *
 * Trade-off: Only 6.25% more LDS usage (minimal overhead)
 *
 * Expected: LOW bank conflicts (reduced from baseline)
 */
...
struct PaddedRowMajorTransposeKernel
{
    static constexpr index_t kBlockSize = 256;
    static constexpr index_t kM = 64;   // Rows
    static constexpr index_t kK = 32;   // Columns (actual data)
    static constexpr index_t kPadding = 2;   // 2 elements = 4 bytes padding
    static constexpr index_t kKPadded = kK + kPadding;  // 34 elements per row = 68 bytes

    CK_TILE_HOST_DEVICE static constexpr index_t GetStaticLdsSize()
    {
        return kM * kKPadded * sizeof(DataType);
    }

    // LDS descriptor for writing [M, K] with PADDING
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorMK()
    {
        return make_naive_tensor_descriptor(
            make_tuple(number<kM>{}, number<kK>{}),        // Logical shape [M, K]
            make_tuple(number<kKPadded>{}, number<1>{}));  // Padded stride!
    }

    // Transposed LDS descriptor for reading [K, M] with padding
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorKM()
    {
        return make_naive_tensor_descriptor(
            make_tuple(number<kK>{}, number<kM>{}),        // Logical [K, M]
            make_tuple(number<1>{}, number<kKPadded>{}));  // Access strides (kKPadded for M)
    }
                                    ...
                                    index_t M, index_t K) const
    {
        ...
        for(index_t k_block = 0; k_block < K; k_block += kK)
        {
            ...
            // Load from global -> store to padded LDS
            auto reg_tile = load_tile(gmem_window_in);
            store_tile(lds_window_mk, reg_tile);
            block_sync_lds();

            // Transpose read from LDS (REDUCED conflicts due to padding)
...
{
    std::cout << "\n=============================================\n";
    std::cout << "Tutorial 14.3: Row-Major with Padding\n";
    std::cout << "=============================================\n\n";

    std::cout << "Layout: Row-major [M, K] = [64, 32] with 2-element (4-byte) padding\n";
    std::cout << "Row stride: " << (32 + 2) << " elements = " << (32 + 2) * 2 << " bytes\n";
    std::cout << "Write: Row-wise (conflict-free)\n";
    std::cout << "Read: Column-wise transpose (reduced conflicts due to padding)\n\n";
    ...
    std::cout << "  LDS (padded): " << (kM * (32+2) * sizeof(DataType)) << " bytes\n";
    std::cout << "  LDS (no padding): " << (kM * 32 * sizeof(DataType)) << " bytes\n";
    std::cout << "  Overhead: 6.25%\n";
    if(num_iters > 1)
        std::cout << "  Iterations: " << num_iters << " (warmup: " << num_warmup << ")\n";
    std::cout << "\n";

    stream_config stream;
    constexpr index_t lds_size = PaddedRowMajorTransposeKernel<DataType>::GetStaticLdsSize();
    ...
    for(int i = 0; i < num_warmup; i++)
    {
        launch_kernel(stream,
                     make_kernel<block_size>(
                         PaddedRowMajorTransposeKernel<DataType>{},
                         dim3(grid_size),
                         dim3(block_size),
                         lds_size,
    ...
    std::cout << "\nExpected: LOW bank conflicts (reduced from baseline)\n";
    std::cout << "Bank pattern: stride=68 bytes, GCD(68,128)=4 -> cycles through 32 banks\n";
