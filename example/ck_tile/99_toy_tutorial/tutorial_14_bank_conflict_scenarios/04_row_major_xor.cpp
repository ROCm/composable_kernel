...
struct ProductionTransposeKernel
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorMK()
    {
        if constexpr (UseXor)
        {
            ...
            // We pick the *natural* row-major tuple order so strides are decreasing:
            //   - Dimension 0: B = M / MLdsLayer      (row axis, outermost, largest stride)
            //   - Dimension 1: A = K / KPack * Layer  (inner M/K mix, middle stride)
            //   - Dimension 2: C = KPack              (innermost, stride 1 = vector axis)
            constexpr auto lds_desc_0 = make_naive_tensor_descriptor(
                make_tuple(number<kM / MLdsLayer>{},           // B = 64/2 = 32
                           number<kK / kKPack * MLdsLayer>{},  // A = 32/8*2 = 8
                           number<kKPack>{}),                  // C = 8
                make_tuple(number<kK * MLdsLayer>{},           // stride B = 64  (largest)
                           number<kKPack>{},                   // stride A = 8
                           number<1>{}),                       // stride C = 1
                number<kKPack>{},
                number<1>{});

            // Step 2: Apply XOR transform on (B, A)
            //
            // This is the KEY operation that reduces bank conflicts.
            // xor_t::calculate_lower_index (coordinate_transform.hpp:1325) gives:
            //     low[0] = up[0]
            //     low[1] = up[1] XOR (up[0] % up_lengths[1])
            // With up_lengths = (B_eff, A) and up = (B, A):
            //     A_low = A XOR (B % A)   <- B (m-index) permutes A (k-index)
            //     B_low = B
            // Net effect: spread consecutive K-column accesses across all 32 banks.
            // Physical addresses are permuted; the logical [M, K] view is preserved.
            constexpr auto lds_desc_permuted = transform_tensor_descriptor(
                lds_desc_0,
                make_tuple(make_xor_transform(make_tuple(number<kM / MLdsLayer>{},            // B_eff
                                                         number<kK / kKPack * MLdsLayer>{})), // A
                           make_pass_through_transform(number<kKPack>{})),
                make_tuple(sequence<0, 1>{},   // lower dims: (B, A) are (0, 1)
                           sequence<2>{}),
                make_tuple(sequence<0, 1>{},   // upper dims: (B, A) stay at (0, 1)
                           sequence<2>{}));

            // Step 3: Unmerge A -> (Layer, K/Pack)
            //
            // Transform: [B, A, Pack] -> [B, Layer, K/Pack, Pack]
            //            [32, 8, 8]   -> [32, 2, 4, 8]
            constexpr auto lds_desc_unmerged = transform_tensor_descriptor(
                lds_desc_permuted,
                make_tuple(make_pass_through_transform(number<kM / MLdsLayer>{}),   // B
                           make_unmerge_transform(                                  // A -> (L, K0)
                               make_tuple(number<MLdsLayer>{}, number<kK / kKPack>{})),
                           make_pass_through_transform(number<kKPack>{})),          // C
                make_tuple(sequence<0>{}, sequence<1>{},    sequence<2>{}),
                make_tuple(sequence<0>{}, sequence<1, 2>{}, sequence<3>{}));

            // Step 4: Merge back to [M, K]
            //
            // Transform: [B, Layer, K/Pack, Pack] -> [M, K]
            //            [32, 2, 4, 8]            -> [64, 32]
            //   - Merge (B, Layer)  -> M  (output dim 0)
            //   - Merge (K/Pack, Pack) -> K (output dim 1)
            //
            // Logical view is [M, K], physical addresses are XOR-permuted.
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorKM()
    {
        if constexpr (UseXor)
        {
            ...
            // Step 1: Reshape (same as write) -- (B, A, C) natural row-major order.
            constexpr auto lds_desc_0 = make_naive_tensor_descriptor(
            ...
            // low[1] = up[1] XOR (up[0] % up_lengths[1]) -> A_low = A XOR (B % A).
            constexpr auto lds_desc_permuted = transform_tensor_descriptor(
                lds_desc_0,
            ...
            // Step 3: Unmerge A -> (Layer, K/Pack). Upper space: (B=0, L=1, K0=2, C=3).
            constexpr auto lds_desc_unmerged = transform_tensor_descriptor(
                lds_desc_permuted,
            ...
            // Step 4: Merge back to [K, M] -- SWAPPED output order vs write descriptor.
            //
            // Transform: [B, Layer, K/Pack, Pack] -> [K, M]
            //            [32, 2, 4, 8]            -> [32, 64]
            //
            // Compare to write descriptor (produces [M, K]):
            //   Write: (B, Layer)    -> M   (output dim 0)
            //          (K/Pack, Pack) -> K  (output dim 1)
            //   Read:  (K/Pack, Pack) -> K  (output dim 0)  <- swapped
            //          (B, Layer)    -> M   (output dim 1)  <- swapped
            //
            // This creates the transposed [K, M] view of the SAME XOR-permuted bytes.
            // Physical memory is unchanged; only the logical interpretation differs.
            constexpr auto lds_desc = transform_tensor_descriptor(
                lds_desc_unmerged,
                make_tuple(
                ...
                make_tuple(sequence<2, 3>{},   // (K/Pack, Pack)  -> output dim 0 (K)
                           sequence<0, 1>{}),  // (B,      Layer) -> output dim 1 (M)
                make_tuple(sequence<0>{}, sequence<1>{}));
...
bool run_test(const std::string& test_name)
{
    ...
    for(index_t m = 0; m < M; ++m)
        for(index_t k = 0; k < K; ++k)
            ...
            // h_input[m * K + k] = static_cast<DataType>(i++);
    ...
    // warmup + timed bench (100 iters) for parity with other tutorials
    ...
    std::cout << "  Average time: " << avg_us << " us\n";
    std::cout << "  Bandwidth: "
              << (M * K * sizeof(DataType) * 2) / (avg_us * 1000.0f)
              << " GB/s\n";
...
int main()
{
    ...
    // Run BOTH versions for comparison profiling
    // [[maybe_unused]] bool plain_passed = run_test<false>("Test 1: Plain LDS (NO XOR)");
    [[maybe_unused]] bool xor_passed = run_test<true>("Test 2: XOR LDS");
