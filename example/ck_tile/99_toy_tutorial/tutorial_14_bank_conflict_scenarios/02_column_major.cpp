...
/*
 * Tutorial 14, Scenario 2: Column-Major Layout
 *
 * Demonstrates conflict-free column reads by storing in column-major order.
 *
 * Layout: Column-major [K, M] with stride M (columns are contiguous)
 * Write: Store data column-by-column
 * Read: Column-wise (contiguous in memory, conflict-free)
 *
 * Memory layout:
 *   Col 0: [row0, row1, ..., row63] at addresses [0-127]
 *   Col 1: [row0, row1, ..., row63] at addresses [128-255]
 *   ...
 *
 * Reading column 0: addresses 0, 2, 4, 6, ... (contiguous, no conflicts)
 *
 * Expected: LOW bank conflicts for transpose reads
 */
...
struct ColumnMajorTransposeKernel
{
    ...
    // LDS descriptor for writing [M, K] in COLUMN-MAJOR layout
    // Physical: store columns contiguously so transpose read is conflict-free
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorMK()
    {
        // Column-major: stride for K is 1, stride for M is kK
        // But we want columns contiguous, so: stride_m=1, stride_k=kM
        return make_naive_tensor_descriptor(
            make_tuple(number<kM>{}, number<kK>{}),
            make_tuple(number<1>{}, number<kM>{}));  // Column-major strides
    }

    // Transposed LDS descriptor for reading [K, M]
    // Since we stored column-major, reading columns is contiguous!
    CK_TILE_HOST_DEVICE static constexpr auto MakeLdsDescriptorKM()
    {
        return make_naive_tensor_descriptor(
            make_tuple(number<kK>{}, number<kM>{}),
            make_tuple(number<kM>{}, number<1>{}));  // Contiguous M access!
    }
                                    ...
                                    index_t M, index_t K) const
    {
        ...
        for(index_t k_block = 0; k_block < K; k_block += kK)
        {
            ...
            // Load from global -> store to LDS (column-major)
            auto reg_tile = load_tile(gmem_window_in);
            store_tile(lds_window_mk, reg_tile);
            block_sync_lds();

            // Transpose read from LDS (conflict-free due to column-major!)
...
{
    std::cout << "\n==========================================\n";
    std::cout << "Tutorial 14.2: Column-Major LDS Layout\n";
    std::cout << "==========================================\n\n";

    std::cout << "Layout: Column-major (columns contiguous in LDS)\n";
    std::cout << "Write: Column-major store\n";
    std::cout << "Read: Column-wise transpose (conflict-free)\n\n";
    ...
    constexpr index_t lds_size = ColumnMajorTransposeKernel<DataType>::GetStaticLdsSize();
    ...
    for(int i = 0; i < num_warmup; i++)
    {
        launch_kernel(stream,
                     make_kernel<block_size>(
                         ColumnMajorTransposeKernel<DataType>{},
                         dim3(grid_size),
                         dim3(block_size),
                         lds_size,
    ...
    std::cout << "\nExpected: LOW bank conflicts for transpose read\n";
    std::cout << "Reason: Columns are contiguous in LDS (addresses 0,2,4,6,...)\n";
