// PURE Read-only LDS test with XOR - pre-initialize LDS with NO bank conflicts
// Expected: 0 read conflicts (XOR eliminates transpose conflicts)
#include <iostream>
#include <vector>
...
struct PureReadKernel
{
                                    ...
                                    index_t K) const
    {
        ...
        // Use store_tile with XOR descriptor to initialize LDS - still conflict-free
        ...
        store_tile(lds_window_mk, reg_tile);  // XOR write - still conflict-free!
        block_sync_lds();

        // NOW DO TRANSPOSE READS with XOR - should be conflict-free!
...
int main()
{
    ...
    std::cout << "Pure Read WITH XOR completed\n";
    return 0;
}
