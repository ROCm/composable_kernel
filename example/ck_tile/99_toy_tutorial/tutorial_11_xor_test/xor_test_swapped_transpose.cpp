...
/*
 * Tutorial 11: XOR Descriptor Test - SWAPPED TRANSPOSE VERSION
 *
 * This variant swaps the two transpose operations:
 * - Original Step 1: strides transpose dimensions (via stride ordering)
 * - Original Step 2: sequence transpose (via sequence<1, 0>)
 *
 * SWAPPED VERSION:
 * - Step 1: NO stride transpose (dimensions in original order)
 * - Step 2: NO sequence transpose (sequence<0, 1>)
 *
 * Purpose: Test if these two transpose methods are equivalent.
 */
...
// Minimal XOR test kernel with SWAPPED transposes
template<typename DataType>
struct XorTestKernelSwapped
{
                                    ...
                                    index_t K) const
    {
        ...
        // ========================================================================
        // Create XOR-swizzled LDS descriptor - SWAPPED VERSION
        // ========================================================================
        ...
        // Step 1: Reshape - NO transpose in strides (keep original dimension order)
        // Original had: dimensions [K/Pack*Layer, M/Layer, Pack] with strides [Pack, K*Layer, 1]
        // Swapped: dimensions [M/Layer, K/Pack*Layer, Pack] with strides [K*Layer, Pack, 1]
        constexpr auto lds_desc_0 = make_naive_tensor_descriptor(
            make_tuple(number<kM / MLdsLayer>{},
                       number<kK / kKPack * MLdsLayer>{},
                       number<kKPack>{}),
            make_tuple(number<kK * MLdsLayer>{}, number<kKPack>{}, number<1>{}),
            number<kKPack>{},
            number<1>{});

        // Step 2: XOR permute - NO sequence transpose (keep original order)
        // Original had: sequence<1, 0> to swap dimensions
        // Swapped: sequence<0, 1> to keep dimensions in order
        ...
        // Step 3: Unmerge - swapped to operate on dim 1 instead of dim 0
        ...
        // Step 4: Merge back to [M, K] - swapped dimension order
...
{
    std::cout << "\n========================================\n";
    std::cout << "Tutorial 11: XOR Test - SWAPPED TRANSPOSE\n";
    std::cout << "========================================\n\n";
    ...
    std::cout << "  VERSION: Swapped transposes\n\n";

    stream_config stream;
    constexpr index_t lds_size = XorTestKernelSwapped<DataType>::GetStaticLdsSize();

    d_input.ToDevice(h_input.data(), M * K * sizeof(DataType));

    launch_kernel(stream,
                 make_kernel<block_size>(
                     XorTestKernelSwapped<DataType>{},
                     dim3(grid_size),
                     dim3(block_size),
                     lds_size,
    ...
    if(passed)
    {
        std::cout << "SUCCESS! SWAPPED transpose version also works!\n";
        std::cout << "This suggests the two transpose operations are equivalent.\n";
    }
    else
    {
        std::cout << "FAILED! Swapped transposes produce different behavior.\n";
        std::cout << "The transpose order/method matters for correctness.\n";
    }
