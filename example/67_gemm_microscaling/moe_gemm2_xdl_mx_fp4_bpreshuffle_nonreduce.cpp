...
#include "ck/library/reference_tensor_operation/cpu/reference_moe_mx_gemm2_nonreduce.hpp"
...
using DeviceOpInstance = ck::tensor_operation::device::DeviceMoeGemmMXBPreShuffle<      
    ...
    ck::BlockGemmPipelineScheduler::Intrawave, ck::BlockGemmPipelineVersion::v3, 0, false, false, MulRoutedWeight,
    ck::index_t, A0DataType, B0DataType,
    true, false>;
// clang-format on
...
int main(int argc, char* argv[])
{
    ...
    Tensor<EDataType> e_t_n_host_result(
        HostTensorDescriptor({tokens, topk, N}, {N * topk, N, 1}, Bypass{}));
    ...
    if(do_verification)
    {
        ...
        using ReferenceGemmInstance =
            ck::tensor_operation::host::ReferenceMoeMXGemm2NonReduce<A0DataType,
                                                                     XDataType,
                                                                     B0DataType,
                                                                     XDataType,
                                                                     D2DataType,
                                                                     float, // using float for
                                                                            // Cshuffle type in
                                                                            // reference
                                                                     AccDataType,
                                                                     PassThrough,
