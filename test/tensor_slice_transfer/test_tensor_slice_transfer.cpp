...
__global__ void packed_cast_kernel(float x1, float x2, ck::bhalf2_t* output)
{
    ...
    dst_buffer.template AsType<ck::bhalf2_t>()(I0) =
        bf16x2_convert_rne<ck::bhalf2_t, float>(x1, x2);
...
template <index_t TestCShuffleMXdlPerWavePerShuffle,
          index_t TestCShuffleNXdlPerWavePerShuffle,
          index_t TestM2,
          index_t TestM4,
          typename SrcData,
          typename DstData,
          bool UsePackedCast,
          bool TransferOutputToGlobalMemory = true,
          int NRepeats                      = 1>
__global__ void
testVGPRToLDSTransfer_kernel(SrcData* input_data, DstData* output_data, index_t num_elements)
{
    // Thread buffer (VGPR)
    ...
    // LDS buffer, this can be the same for each thread since we are interested in the threadwise
    // transfer performance/correctness.
    ...
    constexpr auto buffer_size =
        TestCShuffleMXdlPerWavePerShuffle * TestCShuffleNXdlPerWavePerShuffle * TestM2 * TestM4;
    ...
    StaticBufferTupleOfVector<AddressSpaceEnum::Vgpr, SrcData, num_vectors, vector_size, true>
        src_thread_buf;
    ...
    {
        static_for<0, buffer_size, 1>{}(
            [&](auto i) { src_thread_buf(i) = input_data[offset_in_global_memory + i.value]; });
    }
    ...
    using TransferType = std::conditional_t<
        UsePackedCast,
        ThreadwiseTensorSliceTransfer_v1r3_packed_cast<SrcData,
                                                       DstData,
                                                       ...
                                                       >,
        ThreadwiseTensorSliceTransfer_v1r3<SrcData,
                                           DstData,
                                           decltype(c_thread_desc),
                                           decltype(lds_desc),
                                           ElementOp,
    ...
    if(thread_id < num_elements)
    {
        static_for<0, NRepeats, 1>{}([&](auto i) {
            ...
            auto lds_buf =
                make_dynamic_buffer<AddressSpaceEnum::Lds>(&lds_data[offset_in_lds], buffer_size);

            thread_transfer.Run(
                c_thread_desc, src_slice_origin_index, src_thread_buf, lds_desc, lds_buf);
        });
    }
...
void run_single_packed_cast_test(const float x1,
                                 const float x2,
                                 std::function<void(float, float, ck::bhalf2_t*)> launch_kernel)
{
    ...
    hip_check_error(hipMemcpy(
        &value_after_cast_host, value_after_cast_dev, sizeof(ck::bhalf2_t), hipMemcpyDeviceToHost));
...
void run_packed_cast_test(std::function<void(float, float, ck::bhalf2_t*)> launch_kernel)
{
    ...
    std::vector<float> exact_in_both{0.0f,
                                     1.0f,
                                     2.0f,
                                     8.0f,
                                     32.0f,
    ...
    for(int i = 0; i < num_vals; i++)
    {
        for(int j = 0; j < num_vals; j++)
        {
            ...
            run_single_packed_cast_test(
                exact_in_both_value, exact_fp32_not_bf16_value, launch_kernel);
            run_single_packed_cast_test(
                exact_in_both_value, -exact_fp32_not_bf16_value, launch_kernel);
            run_single_packed_cast_test(
                -exact_in_both_value, exact_fp32_not_bf16_value, launch_kernel);
            run_single_packed_cast_test(
                -exact_in_both_value, -exact_fp32_not_bf16_value, launch_kernel);
            run_single_packed_cast_test(
                exact_fp32_not_bf16_value, exact_in_both_value, launch_kernel);
            run_single_packed_cast_test(
                exact_fp32_not_bf16_value, -exact_in_both_value, launch_kernel);
            run_single_packed_cast_test(
                -exact_fp32_not_bf16_value, exact_in_both_value, launch_kernel);
...
class VGPRToLDSTransferTest : public ::testing::Test
{
    ...
    void run_perf_test(float required_speedup)
    {
        ...
        const auto packed_cast_time = run<TestCShuffleMXdlPerWavePerShuffle,
                                          TestCShuffleNXdlPerWavePerShuffle,
                                          TestM2,
                                          TestM4,
                                          true,
                                          NRepeats>(num_iters, num_warmup_iters);
        const auto baseline_time    = run<TestCShuffleMXdlPerWavePerShuffle,
                                          TestCShuffleNXdlPerWavePerShuffle,
                                          TestM2,
                                          TestM4,
                                          false,
        ...
        std::cout << "Performance test results for case: " << "MXdlPerWavePerShuffle="
                  << TestCShuffleMXdlPerWavePerShuffle
                  << ", NXdlPerWavePerShuffle=" << TestCShuffleNXdlPerWavePerShuffle
                  << ", M2=" << TestM2 << ", M4=" << TestM4 << std::endl;
        std::cout << "Baseline average execution time = "
                  << (baseline_time.has_value() ? *baseline_time : default_value) << " ms"
                  << std::endl;
        std::cout << "Packed cast average execution time = "
                  << (packed_cast_time.has_value() ? *packed_cast_time : default_value) << " ms"
                  << std::endl;
        ...
        if(baseline_time && packed_cast_time)
        {
            ...
            EXPECT_GT(speedup, required_speedup)
                << "Packed cast should be at least " << 100.0f * required_speedup
    ...
    void run()
    {
        std::ignore = run<TestCShuffleMXdlPerWavePerShuffle,
                          TestCShuffleNXdlPerWavePerShuffle,
                          TestM2,
                          TestM4,
                          UsePackedCast,
    ...
    {
        return run<TestCShuffleMXdlPerWavePerShuffle,
                   TestCShuffleNXdlPerWavePerShuffle,
                   TestM2,
                   TestM4,
                   UsePackedCast,
                   true>(true, num_iters, num_warmup_iters);
    };
    ...
    std::optional<float> run(bool time_kernel, index_t num_iters, index_t num_warmup_iters)
    {
        if constexpr(UseGpu)
        {
            return run_device<TestCShuffleMXdlPerWavePerShuffle,
                              TestCShuffleNXdlPerWavePerShuffle,
                              TestM2,
                              TestM4,
                              UsePackedCast,
                              NRepeats>(time_kernel, num_iters, num_warmup_iters);
        }
        else
        {
    ...
    {
        constexpr index_t elements_per_thread =
            TestCShuffleMXdlPerWavePerShuffle * TestCShuffleNXdlPerWavePerShuffle * TestM2 * TestM4;
        ...
        HIP_CHECK_ERROR(hipMemcpy(
            d_input, h_input.data(), total_elements * sizeof(float), hipMemcpyHostToDevice));
        ...
        if(time_kernel)
        {
            ...
            auto kernel = testVGPRToLDSTransfer_kernel<TestCShuffleMXdlPerWavePerShuffle,
                                                       TestCShuffleNXdlPerWavePerShuffle,
                                                       TestM2,
                                                       TestM4,
                                                       float,
                                                       ...
                                                       false,
                                                       NRepeats>;
            kernel_average_execution_time = launch_and_time_kernel(
                stream_config, kernel, grid, block, 0, d_input, d_output, total_elements);
        }
        else
        {
            testVGPRToLDSTransfer_kernel<TestCShuffleMXdlPerWavePerShuffle,
                                         TestCShuffleNXdlPerWavePerShuffle,
                                         TestM2,
                                         TestM4,
                                         float,
                                         ck::bhalf_t,
                                         UsePackedCast>
                <<<grid, block>>>(d_input, d_output, total_elements);
            HIP_CHECK_ERROR(hipDeviceSynchronize());

            // Copy results back
            HIP_CHECK_ERROR(hipMemcpy(h_output.data(),
                                      d_output,
                                      total_elements * sizeof(ck::bhalf_t),
            ...
            for(index_t i = 0; i < total_elements; ++i)
            {
                ...
                if(val_error > abs_tolerance)
                {
                    ...
                    if(errors <= 10)
                    {
                        std::cout << "Error at " << i << ": got " << output_val << ", expected "
                                  << ref_val << ", error " << val_error << std::endl;
                    }
                }
            }
...
TEST_F(VGPRToLDSTransferTest, FloatToBhalf_device_NoPack)
{
    ...
    run<TestCShuffleMXdlPerWavePerShuffle,
        TestCShuffleNXdlPerWavePerShuffle,
        TestM2,
        TestM4,
        UsePackedCast,
