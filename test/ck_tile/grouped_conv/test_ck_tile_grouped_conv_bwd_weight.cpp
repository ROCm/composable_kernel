...
struct BuildKernel
{
    ...
    using GemmUniversalTraits = TileGemmUniversalTraits<
        ConvTraits::FixedGemmParams::kPadM,
    ...
    using GemmPipelineProblem = GemmPipelineProblem<
        PrecType, // OutDataType (A in bwd weight)
...
static GroupedConvBwdWeightHostArgs create_2d_host_args(
    index_t G, index_t N, index_t K, index_t C,
    index_t Y, index_t X, index_t Hi, index_t Wi,
    index_t stride_y, index_t stride_x,
    index_t dilation_y, index_t dilation_x,
    index_t left_pad_y, index_t left_pad_x,
    index_t right_pad_y, index_t right_pad_x,
    index_t k_batch = 1)
{
    auto conv_param = conv::ConvParam{
        2, G, N, K, C, {Y, X}, {Hi, Wi},
        {stride_y, stride_x}, {dilation_y, dilation_x},
        {left_pad_y, left_pad_x}, {right_pad_y, right_pad_x}};

    return GroupedConvBwdWeightHostArgs{
...
TEST_F(GroupedConvBwdWeightIsSupportedArgumentTest, NonFloatDoubleOutputLimitsKBatch)
{
    ...
    auto kargs_128 = typename Kernel::GroupedConvBwdWeightKernelArgsSpecialized(host_args_kbatch_128);
    ...
    auto kargs_129 = typename Kernel::GroupedConvBwdWeightKernelArgsSpecialized(host_args_kbatch_129);
