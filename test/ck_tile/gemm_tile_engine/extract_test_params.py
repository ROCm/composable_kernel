...
def extract_test_params(config_file, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        for i, params in enumerate(test_params):
            ...
            f.write(f"    {{{params['m']}, {params['n']}, {params['k']}, {params['split_k']}}}{comma}\n")
        
        f.write("};\n")
    
    print(f"Extracted {len(test_params)} test parameters from {config_file} -> {output_file}")

def main():
    parser = argparse.ArgumentParser(description="Extract test parameters from config JSON")
    parser.add_argument("--config_file", required=True, help="Input config JSON file")
    parser.add_argument("--output_file", required=True, help="Output test parameters file")
