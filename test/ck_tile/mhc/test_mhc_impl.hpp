...
public:
    ...
    void RunGenericTest()
    {
        ...
        std::cout << "DEBUG h_x[0, 0:4]: " << h_x(0, 0) << ", " << h_x(0, 1) << ", " 
                  << h_x(0, 2) << ", " << h_x(0, 3) << std::endl;
        std::cout << "DEBUG h_x[1, 0:4]: " << h_x(1, 0) << ", " << h_x(1, 1) << ", " 
                  << h_x(1, 2) << ", " << h_x(1, 3) << std::endl;
        ...
        using BlockShape = ck_tile::Generic2dBlockShape<
            ck_tile::sequence<1, 256>,    // Block tile size [M, N] - 1 row, 256 columns
        ...
        using KernelCKTile = ck_tile::ManifoldConstrainedHyperConnectionTiled<
            Problem,
            ck_tile::MHCDefaultPolicy,
        ...
        ck_tile::launch_kernel(
            ...
            ck_tile::make_kernel<kBlockPerCu>(
                ...
                B, n, C, r, alpha_pre, alpha_post, alpha_res, bias));
        ...
        std::cout << "\nOutput tensor (first 2 batches, all " << output_dim << " elements):" << std::endl;
        ...
        ck_tile::launch_kernel(
            ...
            ck_tile::make_kernel<kBlockPerCu>(
                ...
                B, C, r, alpha_pre, alpha_post, alpha_res, bias));  // n removed, now template param
    ...
    void RunExpansionParallelTest()
    {
        ...
        std::cout << "  Expansion factor (n): " << n << std::endl;
        std::cout << "  Channels per stream (C): " << C << std::endl;
        std::cout << "  Grid size: " << output_dim << " (one block per expansion stream)" << std::endl;
        std::cout << "========================================" << std::endl;
        ...
        using KernelExpansionParallel = ck_tile::ManifoldConstrainedHyperConnectionTiled<
            Problem, ck_tile::MHCDefaultPolicy, 4>;
        ...
        ck_tile::launch_kernel(
            ...
            ck_tile::make_kernel<kBlockPerCu>(
                ...
                static_cast<float*>(d_phi_mem.GetDeviceBuffer()),
                static_cast<float*>(d_output_mem.GetDeviceBuffer()),
                B, C, r, alpha_pre, alpha_post, alpha_res, bias));
        ...
        for (int b = 0; b < std::min(2, B); b++)
        {
            ...
            {
                std::cout << h_output(b, i);
                if (i < std::min(8, output_dim) - 1) std::cout << ", ";
            }
            std::cout << " ...]" << std::endl;
        }
