...
{
    static_for<4, 12, 4>{}([](auto k_factor) {
        static_for<1, 33, 1>{}([&](auto i) {
            ...
            static constexpr bool isValid =
                (i == 16 && k_factor == 8) || (i == 32 && k_factor == 2);
...
// TODO: Originally we had tests here with composition in M, N and K. However, we are temporarily
// disabling M and N composition for the integration. Furthermore, automatic K composition is
// dangerous with scale operations because the scale values are calculated by the caller, and K
// composition would mean that we would need longer scale vectors which are then split up per Mma
// call, which neither the caller code nor our Scale Pipeline code currently have support for.
...
//         CTensor c;
//         __builtin_memcpy(
//             &a, static_cast<const uint8_t*>(a_per_lane) + lane * sizeof(ATensor),
//             sizeof(ATensor));
//         __builtin_memcpy(
//             &b, static_cast<const uint8_t*>(b_per_lane) + lane * sizeof(BTensor),
//             sizeof(BTensor));
//         __builtin_memset(&c, 0, sizeof(CTensor));
