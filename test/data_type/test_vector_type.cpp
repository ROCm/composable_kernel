...
__global__
void testVectorizedAccess_kernel(float* input, float* output)
{
...
TEST(vector_type, TestVectorizedAccess_bf16_device)
{
    ...
    hip_check_error(hipMemcpy(d_input, input_values.data(), input_values.size() * sizeof(float), hipMemcpyHostToDevice));
    ...
    hip_check_error(hipMemcpy(output_values.data(), d_output, output_values.size() * sizeof(float), hipMemcpyDeviceToHost));
