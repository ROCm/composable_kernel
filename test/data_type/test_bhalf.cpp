    ...
    for(int i = 0; i < NumElements; i++)
    {
        for(int j = 0; j < NumElements; j++)
        {
            ...
            if constexpr(PackedCast == CastMode::Packed)
            {
                ...
                *buffer_range =
                    ck::bf16x2_convert_rne<ck::bhalf2_t, float>(buffer_float[i], buffer_float[j]);
            }
            else
            {
...
void run_performance_test()
{
    ...
    hip_check_error(hipMemcpy(
        input_dev, input_host.data(), sizeof(float) * NumElements, hipMemcpyHostToDevice));
    ...
    const float baseline_time = launch_and_time_kernel(stream_config,
                                                       baseline_kernel,
                                                       grid_size,
                                                       block_size,
                                                       shared_mem_size,
                                                       ...
                                                       output_dev);
    hip_check_error(hipMemcpy(
        output_host.data(), output_dev, sizeof(ck::bhalf_t) * NumElements, hipMemcpyDeviceToHost));

    const float packed_time = launch_and_time_kernel(stream_config,
                                                     packed_kernel,
                                                     grid_size,
                                                     block_size,
                                                     shared_mem_size,
    ...
    std::cout << "Packed cast time ( " << NumElements << " elements): " << packed_time << " ms"
              << std::endl;
    std::cout << "Baseline cast time ( " << NumElements << " elements): " << baseline_time << " ms"
              << std::endl;
...
TEST(BHALF_T, PackedCastRoundtrip)
{
    ...
    for(int i = 0; i < num_vals; i++)
    {
        for(int j = 0; j < num_vals; j++)
        {
            cast_roundtrip<<<1, 1>>>(float2{float_vals[i], float_vals[j]},
