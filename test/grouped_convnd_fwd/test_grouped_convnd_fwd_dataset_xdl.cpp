...
std::vector<ck::utils::conv::ConvParam> load_csv_test_cases(const std::string& filename)
{
    ...
    while (std::getline(file, line)) {
        ...
        if (row.size() < 19) {  // Need at least 19 columns for 2D (excluding TestName)
        ...
        try {
            ...
            // CSV Format: NDim,Groups,BatchSize,OutChannels,InChannels,KernelH,KernelW,InputH,InputW,OutputH,OutputW,StrideH,StrideW,DilationH,DilationW,LeftPadH,LeftPadW,RightPadH,RightPadW,TestName
            ...
            if (NDim == 2) {
                // 2D Convolution: {NDim, Groups, BatchSize, OutChannels, InChannels, {KernelH,KernelW}, {InputH,InputW}, {StrideH,StrideW}, {DilationH,DilationW}, {LeftPadH,LeftPadW}, {RightPadH,RightPadW}}
                ...
                if (row.size() < 26) {
                    std::cerr << "WARNING: 3D convolution on line " << line_number 
                              << " needs 26+ columns, has " << row.size() << ", skipping" << std::endl;
                    continue;
                }
                // 3D Convolution: {NDim, Groups, BatchSize, OutChannels, InChannels, {KernelD,KernelH,KernelW}, {InputD,InputH,InputW}, {OutputD,OutputH,OutputW}, {StrideD,StrideH,StrideW}, {DilationD,DilationH,DilationW}, {LeftPadD,LeftPadH,LeftPadW}, {RightPadD,RightPadH,RightPadW}}
                ck::utils::conv::ConvParam param = {
                    NDim,                                                          // NDim = 3
                    Groups,                                                        // Groups
                    BatchSize,                                                     // Batch size
                    ...
                    {std::stoi(row[14]), std::stoi(row[15]), std::stoi(row[16])}, // Stride: {D, H, W}
                    {std::stoi(row[17]), std::stoi(row[18]), std::stoi(row[19])}, // Dilation: {D, H, W}
                    {std::stoi(row[20]), std::stoi(row[21]), std::stoi(row[22])}, // Left pad: {D, H, W}
                    {std::stoi(row[23]), std::stoi(row[24]), std::stoi(row[25])}  // Right pad: {D, H, W}
                };
                conv_params.push_back(param);
                
            } else {
                std::cerr << "WARNING: Unsupported NDim=" << NDim << " on line " << line_number << ", skipping" << std::endl;
            }
            
        } catch (const std::exception& e) {
            std::cerr << "ERROR: Failed to parse line " << line_number << ": " << e.what() << std::endl;
            continue;
        }
    }
...
{
    protected:
    using DataType  = std::tuple_element_t<0, Tuple>;      // Extract data type from tuple (fp32, fp16, bf16, int8)
    using InLayout  = std::tuple_element_t<1, Tuple>;      // Extract input tensor layout (NHWGC, NDHWGC, etc.)
    using WeiLayout = std::tuple_element_t<2, Tuple>;      // Extract weight tensor layout (GKYXC, GKZYXC, etc.)
    using OutLayout = std::tuple_element_t<3, Tuple>;      // Extract output tensor layout (NHWGK, NDHWGK, etc.)
    ...
    void Run()
    {
        ...
        for(auto& param : conv_params)
        {
            ...
            pass = pass && ck::profiler::profile_grouped_conv_fwd_impl<NDimSpatial,
                               ...
                               param); // ConvParam: {NDim, Groups, Batch, OutChannels, InChannels, KernelSize, InputSize, ...}
        }
...
using KernelTypes2d = ::testing::Types<std::tuple<float, NHWGC, GKYXC, NHWGK>,        // fp32 test
...
using KernelTypes3d = ::testing::Types<std::tuple<float, NDHWGC, GKZYXC, NDHWGK>,     // fp32 3D test
...
TYPED_TEST(TestGroupedConvndFwd2d, Test2D)
{
    ...
    for (const auto& csv_path : csv_paths) {
        ...
        if (!csv_cases.empty()) {
            ...
            std::cout << "Loaded " << csv_cases.size() << " 2D test cases from " << csv_path << std::endl;
            loaded = true;
            break;
        }
    }
...
TYPED_TEST(TestGroupedConvndFwd3d, Test3D)
{
    ...
    for (const auto& csv_path : csv_paths) {
        ...
        if (!csv_cases.empty()) {
            ...
            std::cout << "Loaded " << csv_cases.size() << " 3D test cases from " << csv_path << std::endl;
            loaded = true;
            break;
        }
    }
