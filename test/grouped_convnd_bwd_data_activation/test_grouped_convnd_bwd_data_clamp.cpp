...
#include "profiler/profile_grouped_conv_bwd_data_clamp_impl.hpp"
...
class TestGroupedConvndBwdDataXdl : public ::testing::Test
{
    ...
    void Run()
    {
        ...
        std::cout << conv_params.size() << std::endl;
        for(auto split_k : split_ks)
        {
            for(auto& param : conv_params)
            {
                pass = pass && ck::profiler::profile_grouped_conv_bwd_data_clamp_impl<NDimSpatial,
                                                                                OutLayout,
                                                                                WeiLayout,
                                                                                InLayout,
                                                                                DataType,
...
using KernelTypes2d = ::testing::Types<std::tuple<ck::half_t, NHWGK, GKYXC, NHWGC>,
                                       std::tuple<ck::bhalf_t, NHWGK, GKYXC, NHWGC>,
                                       std::tuple<float, NHWGK, GKYXC, NHWGC>>;

using KernelTypes3d = ::testing::Types<std::tuple<ck::half_t, NDHWGK, GKZYXC, NDHWGC>,
