...
/*
 * \brief Layout-aware wrapper for naive backward data convolution
 *
...
template <typename InLayout,
          ...
          typename WeiElementwiseOperation,
          typename OutElementwiseOperation>
void conv_bwd_data_with_layouts(TIn* p_in_grad,
                                const TWei* p_wei,
                                ...
                                hipStream_t stream = nullptr)
{
    ...
    auto naive_to_input  = compute_permutation(naive_input_order, user_input_order);
    ...
    bool needs_input_xform  = (naive_input_order != user_input_order);
    ...
    long_index_t input_length = dims.N * dims.Di * dims.Hi * dims.Wi * dims.C;
    int grid_size             = (input_length + block_size - 1) / block_size;

    hipLaunchKernelGGL((naive_conv_bwd_data_ndhwc_kzyxc_ndhwk<TIn,
                                                              TWei,
                                                              TOut,
                                                              TAcc,
    ...
    // Step 6: Transform input gradient back to user layout
    if(needs_input_xform)
    {
        std::vector<index_t> input_dims = build_naive_input_dims(dims, NDimSpatial);
        layout_transform::launch_generic_transpose<TIn>(
            p_in_grad_naive, p_in_grad, input_dims, naive_to_input, stream);
    }
