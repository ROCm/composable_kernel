#ifndef DEVICE_BNORM_FWD_INSTANCE_WITH_REDUCE_MULTIBLOCK_HPP
#define DEVICE_BNORM_FWD_INSTANCE_WITH_REDUCE_MULTIBLOCK_HPP

#include "reduction_operator_mapping.hpp"
#include "device_bnorm_fwd_instance_impl_common.hpp"
#include "device_bnorm_fwd_nhwc_c_with_reduce_multiblock.hpp"
...
void add_device_bnorm_fwd_with_reduce_multiblock_instance(
    std::vector<DeviceBatchNormFwdPtr>& device_op_instances)
{
    static_for<0, std::tuple_size<bnorm_fwd_configuration_1_instances>::value, 1>{}([&](auto i) {
        ...
        static_for<0, std::tuple_size<bnorm_fwd_configuration_2_instances>::value, 1>{}([&](auto
            ...
            // and modes are added for NHWC layout + Spatial Batch-Norm mode, will expand the codes
            // when more layouts and modes are added
...
#define ADD_BNORM_FWD_WITH_REDUCE_MULTIBLOCK_INST(InOutDataType, AccDataType)             \
    template void                                                                         \
        add_device_bnorm_fwd_with_reduce_multiblock_instance<InOutDataType, AccDataType>( \
            std::vector<DeviceBatchNormFwdPtr> & device_op_instances)

#define ADD_BNORM_FWD_WITH_REDUCE_MULTIBLOCK_INST_REF(InOutDataType, AccDataType)         \
