...
{
    virtual bool IsSupportedArgument(const GroupedConvFwdHostArgs& args) const = 0;
    virtual float
    Run(const GroupedConvFwdHostArgs& args, bool time_kernel, int n_warmup, int n_repeat) const = 0;
    virtual std::string GetName(const GroupedConvFwdHostArgs& args) const                       = 0;
    ...
    GroupedConvolutionForwardBaseInvoker&
    operator=(const GroupedConvolutionForwardBaseInvoker&)                       = default;
    GroupedConvolutionForwardBaseInvoker(GroupedConvolutionForwardBaseInvoker&&) = default;
    GroupedConvolutionForwardBaseInvoker&
    operator=(GroupedConvolutionForwardBaseInvoker&&) = default;
                                                  ...
                                                  OutElementwiseOperation>
{
    ...
    using GemmUniversalTraits = ck_tile::TileGemmUniversalTraits<
        ...
        DoubleSmemBuffer,
        typename GroupedConvTraitsType::template GroupedConvImplicitGemmTraitsFwd<1>::AsLayout,
        typename GroupedConvTraitsType::template GroupedConvImplicitGemmTraitsFwd<1>::BsLayout,
        typename GroupedConvTraitsType::template GroupedConvImplicitGemmTraitsFwd<1>::CLayout,
    ...
    using GemmPipelineProblem = ck_tile::GemmPipelineProblem<
        ...
        AccDataType,
        GemmShape,
        typename GroupedConvTraitsType::template GroupedConvImplicitGemmTraitsFwd<1>,
    ...
    bool IsSupportedArgument(const GroupedConvFwdHostArgs& args) const override
    {
        if(args.k_batch > 1)
        {
    ...
    float Run(const GroupedConvFwdHostArgs& args,
              bool time_kernel,
              int n_warmup = 5,
              int n_repeat = 50) const override
    {
        ...
        const auto Run =
            [&](const auto has_hot_loop_, const auto tail_number_, const auto memory_operation_) {
                ...
                // std::cout << std::endl << has_hot_loop_v << " " <<
                // static_cast<int>(tail_number_.value) << " " << gemm_k << " " << num_loop;
    ...
    std::string GetName(const GroupedConvFwdHostArgs& args) const override
    {
