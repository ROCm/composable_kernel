...
struct BlockFmhaBwdDQDKDVPipelineKRKTRVRIGLP
{
    ...
    dq(const QDramBlockWindowTmp& q_dram_block_window_tmp,
       ...
       FmhaDropout& dropout) const
    {
        ...
        KDataType* kt_lds_ptr = k_lds_ptr;
    ...
    template <typename QDramBlockWindowTmp,
              ...
              typename PositionEncoding>
    CK_TILE_HOST_DEVICE auto
    dk_dv(const QDramBlockWindowTmp& q_dram_block_window_tmp,
          const KDramBlockWindowTmp& k_dram_block_window_tmp,
          const VDramBlockWindowTmp& v_dram_block_window_tmp,
          const BiasDramBlockWindowTmp& bias_dram_block_window_tmp,
          const RandValDramBlockWindowTmp& randval_dram_block_window_tmp,
          ...
          FmhaDropout& dropout) const
    {
        ...
        auto do_lds_read_window =
            make_tile_window(do_lds_window.get_bottom_tensor_view(),
                             ...
                             Policy::template MakeOGradRegSliceBlockDescriptor<Problem>());
        // dOT: Reg ->Reg ->LDS
        // dot reuse do lds memory
        OGradDataType* dot_lds_ptr = static_cast<OGradDataType*>(do_lds_ptr);
        ...
        while(i_total_loops < num_total_loop)
        {
            ...
            // __builtin_amdgcn_sched_barrier(0); // TODO(need opt)
    ...
    template <typename QDramBlockWindowTmp,
              ...
              typename PositionEncoding>
    CK_TILE_HOST_DEVICE auto
    fused_dqdkdv(const QDramBlockWindowTmp& q_dram_block_window_tmp,
                 const KDramBlockWindowTmp& k_dram_block_window_tmp,
                 const VDramBlockWindowTmp& v_dram_block_window_tmp,
                 const BiasDramBlockWindowTmp& bias_dram_block_window_tmp,
                 const RandValDramBlockWindowTmp& randval_dram_block_window_tmp,
               ...
               FmhaDropout& dropout) const
    {
        ...
        if constexpr(kQKHeaddim > 256 && kVHeaddim > 256)
        {
            dq(q_dram_block_window_tmp, k_dram_block_window_tmp, v_dram_block_window_tmp,
               bias_dram_block_window_tmp, randval_dram_block_window_tmp, do_dram_block_window_tmp,
               lse_dram_block_window_tmp, d_dram_block_window_tmp, dq_dram_block_window_tmp,
               dbias_dram_block_window_tmp, mask, position_encoding, raw_scale, scale, rp_undrop,
               scale_rp_undrop, smem_ptr, dropout);

            block_sync_lds();

            return dk_dv(q_dram_block_window_tmp, k_dram_block_window_tmp, v_dram_block_window_tmp,
                        bias_dram_block_window_tmp, randval_dram_block_window_tmp, do_dram_block_window_tmp,
                        lse_dram_block_window_tmp, d_dram_block_window_tmp, dq_dram_block_window_tmp,
                        dbias_dram_block_window_tmp, mask, position_encoding, raw_scale, scale, rp_undrop,
                        scale_rp_undrop, smem_ptr, dropout);
        ...
        {
            return fused_dqdkdv(q_dram_block_window_tmp, k_dram_block_window_tmp, v_dram_block_window_tmp,
                                bias_dram_block_window_tmp, randval_dram_block_window_tmp, do_dram_block_window_tmp,
                                lse_dram_block_window_tmp, d_dram_block_window_tmp, dq_dram_block_window_tmp,
                                dbias_dram_block_window_tmp, mask, position_encoding, raw_scale, scale, rp_undrop,
                                scale_rp_undrop, smem_ptr, dropout);
