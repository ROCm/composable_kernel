...
// TODO
template <typename Problem_, typename Policy_ = void>
struct PartitionedBlockReduce2d
{
    // using Problem         = ck_tile::remove_cvref_t<Problem_>;
    // using ComputeDataType = typename Problem::ComputeDataType;
    // using BlockShape      = typename Problem::BlockShape;

    // template <typename BufferType, typename ThreadClusterLengths, typename ThreadClusterArrangeOrder, typename ReduceFunc>
    // CK_TILE_DEVICE static void Reduce(BufferType& work_buffer,
    //                                   ComputeDataType& in_out_value,
    //                                   ThreadClusterLengths,
    //                                   ThreadClusterArrangeOrder,
    //                                   const ReduceFunc& reduce_func)
    // {
    //     constexpr index_t BlockSize = BlockShape::BlockSize;
    //     static_assert(BlockSize == ThreadClusterLengths::At(0) * ThreadClusterLengths::At(1),
    //                   "Product of cluster lengths must equal BlockSize!"); // TODO

    //     constexpr auto BufferLength_M = ThreadClusterLengths::At(0);
    //     constexpr auto BufferLength_K = ThreadClusterLengths::At(1);

    //     constexpr auto block_buf_desc_m_k = make_naive_tensor_descriptor_packed(
    //         make_tuple(number<BufferLength_M>{}, number<BufferLength_K>{}));

    //     constexpr auto thread_cluster_desc =
    //         make_cluster_descriptor(ThreadClusterLengths{}, ThreadClusterArrangeOrder{});

    //     constexpr auto cluster_len_shift = get_shift<BufferLength_K>(); // TODO: get_shift
    ...
    //     // Reduction within the thread cluster
    ...
    //             ComputeDataType opData1 = work_buffer[offset1];
    //             ComputeDataType opData2 = work_buffer[offset2];
    //             opData1 = reduce_func(opData1, opData2);
