...
// =======================================
// This kernel implements multiple 2D reduction operation that reduces data along the specified dimensions
// of a matrix.
...
struct MultiReduce
{
    ...
    static constexpr index_t CalculateInputVectorSize()
    {
        ...
        constexpr index_t thread_tile_vector_size = S::ThreadTile_N; // In the continuous dimension, within the tile
        ...
        constexpr index_t stride_based_vector_size =
            is_innermost_contiguous ? ck_tile::min(memory_vector_size, thread_tile_vector_size) : 1; // Move at "vectorization" steps if continuous otherwise 1 step
                                   ...
                                   index_t output_tensor_offset) const
    {
        ...
        static_assert(number_operations > 0, "Error: At least one reduction operation must be specified!");
        ...
        const auto kept_merge_transform   = make_merge_transform(kept_lens); // Dimension(s) not reduced are being flattened
        const auto reduce_merge_transform = make_merge_transform(reduce_lens); // Dimension(s) to reduce are being flattened

        const auto custom_padding_values = ck_tile::apply([](auto... args) {
            return ck_tile::make_tuple(type_convert<XDataType>(args.template GetIdentityValue<ComputeDataType>())...);
        }, reduce_funcs); // Get the identity element for each operation

        constexpr auto x_tensor_vector_size = CalculateInputVectorSize<InputShape, ReduceDims>(); // Move at "vectorization" steps if continuous otherwise 1 step

        auto desc = make_naive_tensor_descriptor(
            input_shape, input_strides, number<x_tensor_vector_size>{}, number<1>{}); // Create the tensor descriptor

        auto buffer_view = make_buffer_view<address_space_enum::global>(
            p_x, desc.get_element_space_size(), custom_padding_values.get(number<0>{})); // Input tensor buffer view, currently using the first operation identity element as padding?? TODO: check this

        const auto x_tensor = tensor_view<decltype(buffer_view), decltype(desc)>{buffer_view, desc}; // Tensor view over the buffer view and tensor descriptor
        const auto transformed_x_tensor = pad_tensor_view(
            ...
            sequence<0, 1>{}); // Effective transform the input tensor to 2D (kept, reduced) and pad to block size
        ...
        auto y_tile_windows = generate_tuple(
            [&]([[maybe_unused]] auto i) {
                const auto y_tensor_view = make_naive_tensor_view<address_space_enum::global>(
                    p_y_tuple+(i*output_tensor_offset), kept_lens, kept_strides, number<y_tensor_vector_size>{}, number<1>{});
        ...
        __shared__ char smem[Policy::template GetSmemSize<Problem>()]; // shared memory is reused for each operation

        const auto merged_reduce_len =
            transformed_x_tensor.get_tensor_descriptor().get_lengths().at(number<1>{}); // Get the last dimension size (reduced dimension)
        index_t num_n_tile_iteration =
            __builtin_amdgcn_readfirstlane(integer_divide_ceil(merged_reduce_len, S::Block_N)); // Figure out the number of iterations needed to cover the reduced dimension with Block size N

        auto block_reduce2d      = Policy::template GetBlockReduce2d<Problem>(); // Get the block reduction , at thread level, function
        auto block_reduce2d_sync = Policy::template GetBlockReduce2dSync<Problem>(); // Get the block sync, at warp level, function 
        auto block_reduce2d_cross_warp_sync =
            Policy::template GetBlockReduce2dCrossWarpSync<Problem>(); // Get the block for the cross warp level sync 
        ...
        static_for<0, number_operations, 1>{}([&](auto i){
            ...
            auto x_window = make_tile_window(transformed_x_tensor,
                                    make_tuple(number<S::Block_M>{}, number<S::Block_N>{}),
                                    {iM, 0},
                                    Policy::template MakeXBlockTileDistribution<Problem>()); // Input tile windows and prep the block
            ...
            set_tile(y_compute, reduce_funcs.get(number<i>{}).template GetIdentityValue<ComputeDataType>());
            ...
            block_reduce2d_cross_warp_sync(y_compute, static_cast<void*>(smem), reduce_funcs.get(number<i>{}));
