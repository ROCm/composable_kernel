...
{
    static_assert(MainEpilogueTuple::size() >= 1, "EpilogueChainer requires at least 1 main epilogue");
...
private:
    ...
    template<typename ODramWindow, typename OAccTile, typename DsDramWindows, typename InitArgs, typename MainArgsTuple, typename FinalArgs>
    CK_TILE_DEVICE static auto ExecuteSequential(ODramWindow& out_dram_window,
                                                  const OAccTile& o_acc_tile,
                                                  const DsDramWindows& ds_dram_windows,
                                                  void* p_smem,
                                                  ...
                                                  const FinalArgs& final_args)
    {
        auto context = ck_tile::apply(
            [&](auto&&... unpacked_args) {
                return InitEpilogue{}(out_dram_window, o_acc_tile, ds_dram_windows, p_smem,
...
public:
    ...
    template <typename ODramWindow, typename OAccTile, typename DsDramWindows, 
              typename InitArgs, typename MainArgsTuple, typename FinalArgs>
    CK_TILE_DEVICE auto operator()(ODramWindow& out_dram_window,
                                   const OAccTile& o_acc_tile,
                                   const DsDramWindows& ds_dram_windows,
                                   void* p_smem,
    ...
    {
        ExecuteSequential(out_dram_window, o_acc_tile, ds_dram_windows, p_smem, 
                         init_args, main_args, final_args);
    }
