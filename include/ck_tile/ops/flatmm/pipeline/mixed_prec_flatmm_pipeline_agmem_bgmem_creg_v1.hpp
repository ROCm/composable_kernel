...
//
//     static constexpr int ContinuousKPerThread      = 32; // it's fixed for fp4
//     static constexpr int ContinuousScaleMPerThread = 2;  // it's fixed for fp8
//     static constexpr int ContinuousScaleNPerThread = 2;  // it's fixed for fp4
//     static constexpr int ContinuousScaleKPerThread = 2;  // it's fixed for fp4
//     static constexpr index_t flatKPerWarp          = 64 * ContinuousKPerThread;
// };
...
//     static constexpr index_t MIterPerWarp = kMPerBlock / (MWarp * WG::kM);
//     static constexpr index_t NIterPerWarp = kNPerBlock / (NWarp * WG::kN);
//     static constexpr index_t KIterPerWarp = kKPerBlock / WG::kK; // 256 / 128 = 2
//
//     static constexpr index_t KFlatPerBlockPerIter = flatKPerWarp;
//     static constexpr index_t NFlatPerBlockPerIter = flatNPerWarp;
//
...
//
//     static constexpr int ContinuousKPerThread      = Problem::ContinuousKPerThread;
//     static constexpr int ContinuousScaleMPerThread = Problem::ContinuousScaleMPerThread;
//     static constexpr int ContinuousScaleNPerThread = Problem::ContinuousScaleNPerThread;
//     static constexpr int ContinuousScaleKPerThread = Problem::ContinuousScaleKPerThread;
//
...
//         WarpTile::at(I2) / (get_warp_size() / WarpTile::at(I1)); // 8
//
//     static constexpr int XDL_PerWeightK = 1;                                          // 1 for tile block = 128 load 128 per instrution
//     static constexpr int XDL_PerScaleK  = XDL_PerWeightK * ContinuousScaleKPerThread; // 4
//     static constexpr int XDL_PerScaleN  = ContinuousScaleNPerThread;                  // 2
//     static_assert(XDL_PerScaleK % XDL_PerWeightK == 0);
//     static_assert(KIterPerWarp % XDL_PerScaleK == 0);
...
//     {
//         return PipelinePolicy::template MakeAScaleDramTileDistribution<Problem>();
//     }
//
//     template <typename ADramBlockWindowTmp,
//               typename AElementFunction,
//               typename BFlatBlockWindowTmp,
//               typename DequantAWindow,
//               typename DequantBFlatWindow>
//     CK_TILE_HOST_DEVICE auto operator()(ADramBlockWindowTmp a_copy_dram_window_,
//                                         const AElementFunction& a_element_func,
//                                         const BFlatBlockWindowTmp& b_flat_dram_block_window_tmp,
//                                         const DequantAWindow& scale_a_window,
//                                         const DequantBFlatWindow& scale_b_flat_window,
//                                         const index_t num_loop,
//                                         const index_t k_padded_zeros,
//                                         void* p_smem_ping,
...
//
//         auto a_copy_dram_window = replace_bottom_tensor_view(
//             PipelinePolicy::template TransformF8xF4_ATensorView<Problem>(
//                 a_copy_dram_window_.get_bottom_tensor_view()),
//             a_copy_dram_window_);
//
//         // =======  a scale related start  ====== 
//         auto scale_a_dram_window = make_tile_window(
//             scale_a_window.get_bottom_tensor_view(),
//             make_tuple(number<WG::kM>{}, number<WG::kK / 32>{}), // TODO actually 64 / 16 = 4 && 128 / 32 =4
//             scale_a_window.get_window_origin(),
//             PipelinePolicy::template MakeAScaleDramTileDistribution<Problem>());
//         // ping pong buffer for scale A
//         statically_indexed_array<
//             statically_indexed_array<decltype(scale_a_dram_window), KIterPerWarp>,
//             MIterPerWarp>
//             scale_a_dram_windows;
//         statically_indexed_array<decltype(load_tile(scale_a_dram_window)), m_preload>
//             scale_a_tile_tensor;
//         // =======  a scale related end ====== 
//
//         static_for<0, MIterPerWarp, 1>{}([&](auto mIter) {
...
//                                  {mIter * kMPerBlock, kIter * kKPerBlock / 32});
//             });
//         });
//
//         // A tile in LDS
...
//
//         constexpr auto write_a_lds_block_desc =
//             PipelinePolicy::template MakeF8xF4_WriteALdsBlockDescriptor<Problem>();
//         constexpr auto read_a_lds_block_desc =
//             PipelinePolicy::template MakeF8xF4_ReadALdsBlockDescriptor<Problem>();
//
//         auto write_a_lds_block_ping =
//             make_tensor_view<address_space_enum::lds>(p_a_lds_ping, write_a_lds_block_desc);
//         auto write_a_lds_block_pong =
...
//                              make_tuple(number<WG::kM>{}, number<WG::kK>{}),
//                              {iMWarp * WG::kM, 0},
//                              PipelinePolicy::template MakeF8xF4_ALDS_TileDistribution<Problem>());
//         auto a_warp_window_pong_tmp =
//             make_tile_window(read_a_lds_block_pong,
//                              make_tuple(number<WG::kM>{}, number<WG::kK>{}),
//                              {iMWarp * WG::kM, 0},
...
//             NIterPerWarp>
//             b_flat_dram_windows;
//         statically_indexed_array<statically_indexed_array<MXFP4_Buffer, MXFP4KPerWarp>,
//                                  NIterPerWarp>
//             b_warp_tensor_ping;
//         statically_indexed_array<statically_indexed_array<MXFP4_Buffer, MXFP4KPerWarp>,
//                                  NIterPerWarp>
...
//             a_warp_tensor(loadIter) =
//                 load_tile(a_warp_windows_ping(number<mIter>{})(number<kIter>{}));
//             scale_a_tile_tensor(loadIter) =
//                 load_tile(scale_a_dram_windows(number<mIter>{})(number<kIter>{}));
//         });
//
//         __builtin_amdgcn_sched_barrier(0);
//
//         // statically_indexed_array<typename WG::BWarpTensor, NIterPerWarp> dequant_B_n;
//
//         //TODO: maybe need quant scale(change fp32 scale into fp8e8m0)
//
//         auto convert_ascale_e8m0 = [&](auto a_scale_fp32) {
//             return bit_cast<int32_t>(uint8x4_t{ 0, 0, 0, bit_cast<uint8_t>(static_cast<e8m0_t>(a_scale_fp32)) });
//         };
//
//         // MAIN LOOP
//         index_t iCounter = (num_loop - 1) / 2;
...
//                 static_for<0, MIterPerWarp, 1>{}([&](auto mIter) {
//                     constexpr auto AwarpIter = (kIter * MIterPerWarp + mIter) % m_preload;
//                     static_for<0, NIterPerWarp / ContinuousScaleNPerThread, 1>{}([&](auto nIter) {
//                         static_for<0, ContinuousScaleNPerThread, 1>{}([&](auto inxdl) {
//                             // read C warp tensor from C block tensor
//                             CWarpTensor c_warp_tensor;
//
//                             c_warp_tensor.get_thread_buffer() = c_block_tile.get_y_sliced_thread_data(
//                                 merge_sequences(sequence<mIter, nIter * ContinuousScaleNPerThread + inxdl>{}, c_warp_y_index_zeros),
//                                 merge_sequences(sequence<1, 1>{}, c_warp_y_lengths));
//
//                             // warp GEMM
//                             WG{}.template operator()<0, 0>(c_warp_tensor,
//                                  a_warp_tensor(number<AwarpIter>{}),
//                                  b_warp_tensor_ping(nIter * ContinuousScaleNPerThread + inxdl)(kIter),
//                                  convert_ascale_e8m0(scale_a_tile_tensor(number<AwarpIter>{}).get_thread_buffer()[0]),
//                                  convert_ascale_e8m0(scale_b_warp_tensor_ping(nIter / number<XDL_PerScaleN>{})(
//                                      kIter / number<XDL_PerScaleK>{}).get_thread_buffer()[0]));
//
//                             // write C warp tensor into C block tensor
//                             c_block_tile.set_y_sliced_thread_data(
//                                 merge_sequences(sequence<mIter, nIter * ContinuousScaleNPerThread + inxdl>{}, c_warp_y_index_zeros),
...
//                         a_warp_tensor(number<AwarpIter>{}) =
//                             load_tile(a_warp_windows_ping(number<AmIter>{})(number<AkIter>{}));
//                         scale_a_tile_tensor(number<AwarpIter>{}) =
//                             load_tile(scale_a_dram_windows(number<AmIter>{})(number<AkIter>{}));
//                     }
//
//                     // barrier
//                     if constexpr((kIter == KIterPerWarp - 1) && (mIter == MIter_2nd_last))
...
struct F8xMXF4FlatmmPipelineAGmemBGmemCRegV1 : FlatmmPipelineAGmemBGmemCRegV1<Problem, PipelinePolicy>
{
                                   ...
                                   void* __restrict__ p_smem_pong) const
    {
        ...
        // auto a_dram_window = a_copy_dram_window_tmp;
 	// auto a_tile = load_tile(a_dram_window);
            // if (threadIdx.x == 8)
	    // {
	    // printf("trheadIdx.x %d, token_idx0,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_tile.get_thread_buffer()[0]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx1,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_tile.get_thread_buffer()[1]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx2,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_tile.get_thread_buffer()[2]),
	    //     		   );
	    // }
        ...
        auto a_lds_block_pong_load =
            make_tensor_view<address_space_enum::lds>(p_a_lds_pong, a_load_lds_block_desc);
        ...
        auto a_store_lds_window_ping = make_tile_window(
            a_lds_block_ping_load, make_tuple(number<kMPerBlock>{}, number<kKPerBlock>{}), {0, 0});
        auto a_store_lds_window_pong = make_tile_window(
            a_lds_block_pong_load, make_tuple(number<kMPerBlock>{}, number<kKPerBlock>{}), {0, 0});
	...
	// constexpr auto span_q2d = decltype(load_tile(a_warp_window_pong))::get_distributed_spans();
	...
	// 	    a_warp_tensor(I0).get_tile_distribution(), i_j_idx);
	// 
	// 	auto row_id = tile_idx.at(I0);
	// 	auto col_id = tile_idx.at(I1);
	// 
	// 	printf("load_lds_warp_1 index [%d, %d] ", row_id, col_id);
	// 	printf("%f ,", ck_tile::type_convert<float>(a_warp_tensor(I0)[i_j_idx]));
	//     });
	//     printf("\n");
	// });
	//
	...
	{
            // if (threadIdx.x <= 15) 
	    // if (threadIdx.x % 16 == 0 && threadIdx.x < 64 && blockIdx.x==0) // >= 7 &&  threadIdx.x <= 15)
	    // {
	    // printf("trheadIdx.x %d, token_idx0,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[0]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx1,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[1]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx2,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[2]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx-3,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[29]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx-2,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[30]),
	    //     		   );
	    // printf("trheadIdx.x %d, token_idx-1,%f \n", threadIdx.x,
	    //     		   type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[31]),
	    //     		   );
	    // printf("trheadIdx.x %d, scale, warptile 0: %d \n", threadIdx.x,
	    //     		   type_convert<int32_t>(scale_a_tile_tensor_ping(I0)(I0).get_thread_buffer()[0]),
	    //     		   );
	    // // printf("trheadIdx.x %d, scale, warptile 1: %d \n", threadIdx.x,
	    // //     		   type_convert<int32_t>(scale_a_tile_tensor_ping(I0)(I1).get_thread_buffer()[0]),
	    // //     		   );
	    // }
	    // float sum = 0.f;
            //       float* p_sum =reinterpret_cast<float *>( p_smem_ping);
	    // for (int i = 0; i < 32; ++i){
	    //         sum += type_convert<float>(a_warp_tensor(I0).get_thread_buffer()[i]);
	    //         p_sum[threadIdx.x] = sum;
	    // }
	    // __syncthreads();
	    // sum = 0.f;
	    // if (threadIdx.x == 0){
	    ...
	    // for (int i = 0; i < 32; ++i){
	    //         sum += type_convert<float>(a_warp_tensor(I2).get_thread_buffer()[i]);
	    //         p_sum[threadIdx.x] = sum;
	    // }
	    // __syncthreads();
	    // sum = 0.f;
