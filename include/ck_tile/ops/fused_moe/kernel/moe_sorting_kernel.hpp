...
struct MoeSortingKernel
{
    ...
    template <typename T, typename F, index_t wave_size_ = CK_GPU_ARCH_WARP_SIZE>
...
#pragma unroll Problem_::InternalLoadUnroll
        ...
        else
        {
            ...
            static constexpr index_t experts_per_wave = CK_GPU_ARCH_WARP_SIZE / Problem::ExpertTile;
            {
    ...
    {
        const index_t tid            = static_cast<index_t>(threadIdx.x);
        const index_t wid            = __builtin_amdgcn_readfirstlane(tid / CK_GPU_ARCH_WARP_SIZE);
...
#pragma unroll Problem::SubTokenTile
...

        {
            if(wid == 0)
            {
                ...
                int i_e_          = 0;
                int local_cumsum_ = 0;
                for(; i_e_ < num_experts; i_e_ += CK_GPU_ARCH_WARP_SIZE)
                {
                    ...
                    wave_cumsum<int, CK_GPU_ARCH_WARP_SIZE>(local_cumsum_);
                    ...
                    if constexpr(Problem::LocalExpertMasking)
                    {
                        ...
                        wave_cumsum<int, CK_GPU_ARCH_WARP_SIZE>(local_masking);
                        if((i_e_ + lid) < num_experts)
                ...
                if((lid + i_e_ - CK_GPU_ARCH_WARP_SIZE) == (num_experts - 1))
                {
...
struct MoeSortingMultiPhaseKernel_P1
{
    ...
    {
        return BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE * sizeof(IndexType);
    }
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        index_t lane_id = threadIdx.x % CK_GPU_ARCH_WARP_SIZE;
        index_t wave_id = threadIdx.x / CK_GPU_ARCH_WARP_SIZE;
        ...
        {
            index_t c = 0;
            for(auto i = 0; i < (BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE); i++)
            {
                c += s[i];
            }
...
struct MoeSortingMultiPhaseKernel_P2
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetSmemSize()
    {
        ...
        return (4 + 2 * BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE) * sizeof(IndexType);
    }
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        for(index_t i = 0; i < loops; i++)
        {
            ...
            impl::moe_sorting_wave_cumsum<IndexType, CK_GPU_ARCH_WARP_SIZE>(cumsum_a);
            impl::moe_sorting_wave_cumsum<IndexType, CK_GPU_ARCH_WARP_SIZE>(cumsum_b);

            __syncthreads();
            if(lane_id == CK_GPU_ARCH_WARP_SIZE - 1)
            {
                s[4 + wave_id]                         = cumsum_a;
                s[4 + wave_id + BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE] = cumsum_b;
            }

            __syncthreads();

            // reduce cross wave
            static_for<0, BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE - 1, 1>{}([&](auto i_w) {
                IndexType prev_a = s[4 + i_w];
                IndexType prev_b = s[4 + i_w + BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE];
...
struct MoeSortingMultiPhaseKernel_P3
{
    ...
    CK_TILE_HOST_DEVICE static constexpr auto GetSmemSize()
    {
        return (4 + BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE) * sizeof(IndexType);
    }
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        int eid     = blockIdx.x;
        int wave_id = threadIdx.x / CK_GPU_ARCH_WARP_SIZE;
        int lane_id = threadIdx.x % CK_GPU_ARCH_WARP_SIZE;
        ...
        for(int i = 0; i < loops; i++)
        {
            ...
            int cumsum = i_show;
            impl::moe_sorting_wave_cumsum<int, CK_GPU_ARCH_WARP_SIZE>(cumsum);
...
CK_TILE_HOST constexpr auto moe_sorting_get_smem_size_p23(int num_experts_)
{
    ...
    return (4 + 2 * BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE + expert_cumsum_elem) * sizeof(int);
}
} // namespace impl
    ...
    CK_TILE_DEVICE void operator()(Kargs kargs) const
    {
        ...
        extern __shared__ char smem[];
        {
            ...
            const IndexType* p_local_expert_mask =
                static_cast<const IndexType*>(kargs.p_local_expert_mask);
            IndexType* p_expert_cumsum      = reinterpret_cast<IndexType*>(kargs.p_expert_cumsum);
            IndexType* p_expert_cumsum_smem = s + 4 + 2 * BLOCK_SIZE / CK_GPU_ARCH_WARP_SIZE;
            IndexType* p_total_tokens_post_pad =
                reinterpret_cast<IndexType*>(kargs.p_total_tokens_post_pad);
