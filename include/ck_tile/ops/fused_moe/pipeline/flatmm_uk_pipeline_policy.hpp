...
struct GemmPipelineFlatmmPolicy
{
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto GetAlignment_B()
    {
    ...
    {
        constexpr index_t Block_M_   = Problem::BlockShape::Block_M;
        constexpr index_t Block_K_   = Problem::BlockShape::Block_K;
        constexpr index_t NumWarps_  = Problem::BlockShape::NumWaves;
    ...
    CK_TILE_HOST_DEVICE static constexpr auto MakeGlobalTileDistribution_G()
    {
        ...
        if constexpr(PermuteEnum == FusedMoeGemmWeightPermuteEnum::b_nr_kr_waveflatten)
        {
            ...
            return MakeGlobalTileDistribution_Nr_Kr_W<S_::WavePerBlock_N,
                                                      S_::WavePerBlock_K,
                                                      S_::Repeat_N, /// hidden_radio_0,
                                                      S_::Repeat_K,
                                                      get_warp_size(),
                                                      GetAlignment_B<Problem>()>();
        }
    }
    ...
    template <typename Problem>
    CK_TILE_HOST_DEVICE static constexpr auto GetUK()
    {
        using S_ = typename Problem::BlockShape;
#if 0        
        if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::bf16_t> &&
        ...
        else if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::fp8_t> &&
        ...
        {
            return Flatmm_ff_32x256x512_1x4x1_16x16x64_FP8{};
        }
        else 
#endif        
        if constexpr(std::is_same_v<typename Problem::ADataType, ck_tile::fp8_t> &&
                          std::is_same_v<typename Problem::GDataType, ck_tile::fp8_t> &&
                          S_::Block_M == 32 && S_::Block_N == 128 && S_::Block_K == 256 &&
