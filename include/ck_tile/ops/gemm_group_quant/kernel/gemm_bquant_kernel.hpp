...
struct BQuantGemmKernel
{
    ...
    using BlockGemmShape = remove_cvref_t<typename GemmPipeline::BlockGemmShape>;
    ...
    CK_TILE_HOST static bool IsSupportedArgument(const BQuantGemmKernelArgs& kargs)
    {
        ...
        if(kargs.QK % GemmPipeline::GetVectorSizeBQ() != 0)
        {
            if(ck_tile::EnvIsEnabled(CK_TILE_ENV(CK_TILE_LOGGING)))
            {
                CK_TILE_ERROR("K is not a multiple of vector load size for BQ tensor!");
            }
            return false;
        }
                                                   ...
                                                   const SplitKBatchOffset& splitk_batch_offset)
    {
        ...
        const auto& b_tensor_view = [&]() {
            ...
            else
            {
                ...
                {
                    if constexpr(!GemmPipeline::kFlattenB)
                    {
                        return make_naive_tensor_view<address_space_enum::global>(
                            b_ptr,
