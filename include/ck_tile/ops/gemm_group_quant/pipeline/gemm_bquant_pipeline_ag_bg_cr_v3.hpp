...
struct BQuantGemmPipelineAgBgCrCompV3 : public BaseGemmPipelineAgBgCrCompV3<Problem>
{
    ...
    static constexpr bool kFlattenB = false;
    ...
    {
        // clang-format off
        return concat('_', "bquant_pipeline_AgBgCrCompV3", BlockSize,
                      concat('x', GetVectorSizeA(), GetVectorSizeBQ(), GetVectorSizeB(),  GetVectorSizeC()),
    ...
    CK_TILE_HOST static std::string Print()
    {
        ...
        constexpr index_t AQ_Buffer_Load_Inst_Num =
            MPerBlock * KPerBlockBQ / (BlockSize * GetVectorSizeBQ());
