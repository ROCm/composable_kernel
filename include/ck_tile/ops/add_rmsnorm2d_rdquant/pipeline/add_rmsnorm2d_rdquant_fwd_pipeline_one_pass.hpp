...
struct AddRmsnorm2dRdquantFwdPipelineOnePass
{
                                   ...
                                   void* smem) const
    {
        ...
        auto block_reduce2d_rms = Policy::template GetBlockReduce2d<Problem>();
        auto block_reduce2d_rms_sync = Policy::template GetBlockReduce2dSync<Problem>();
        auto block_reduce2d_rms_cross_warp_sync =
            Policy::template GetBlockReduce2dCrossWarpSync<Problem>();
        auto block_reduce2d_amax      = Policy::template GetBlockReduce2d<Problem>();
        auto block_reduce2d_amax_sync = Policy::template GetBlockReduce2dSync<Problem>();
        auto block_reduce2d_amax_cross_warp_sync =
            Policy::template GetBlockReduce2dCrossWarpSync<Problem>();
        ...
        auto square_sum = block_reduce2d_rms(x, 0, reduce_square_sum_func);
        block_reduce2d_rms_sync(square_sum, reduce_sum_func);
        block_reduce2d_rms_cross_warp_sync(square_sum, smem, reduce_sum_func);
        ...
        // rmsnorm computation
        auto y_compute = make_static_distributed_tensor<ComputeDataType>(x.get_tile_distribution());
        sweep_tile(y_compute, [&, inv_rms_ = inv_rms](auto idx) {
            ...
            y_compute(idx) = type_convert<ComputeDataType>(y_);
        });

        // compute absmax, each-thread->cross-lane->cross-warp
        auto absmax = block_reduce2d_amax(x, numeric<YScaleDataType>::min(), reduce_absmax_func);
        block_reduce2d_amax_sync(absmax, reduce_max_func);
        block_reduce2d_amax_cross_warp_sync(absmax, smem, reduce_max_func);
        ...
        // quantize to
        auto qy = make_static_distributed_tensor<QYDataType>(y_compute.get_tile_distribution());
        sweep_tile(qy, [&, yscale_ = yscale](auto idx) {
            constexpr auto i_idx = make_tuple(idx[number<0>{}]);
            constexpr auto j_idx = make_tuple(idx[number<1>{}]);
            auto qy_             = y_compute[idx] / yscale_[i_idx];
