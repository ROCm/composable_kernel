...
CK_TILE_DEVICE constexpr auto DecomposeGIndex(ck_tile::index_t flat_index,
                                              const ck_tile::index_t* G_lengths)
{
    std::array<ck_tile::index_t, NumDimG> g_indices;

    auto remaining = flat_index;
    static_for<NumDimG - 1, -1, -1>{}([&](auto i) {
...
CalculateGOffset(const std::array<ck_tile::index_t, NumDimG>& g_indices,
                 const ck_tile::index_t* g_strides)
{
