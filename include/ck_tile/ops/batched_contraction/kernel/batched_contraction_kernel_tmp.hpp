...
struct BatchedContractionKernelArgs
{
    ...
    // ✅ Hardcoded fixed arrays - no pointers!
    ck_tile::index_t G_lengths[2];   // Fixed size array
    ck_tile::index_t G_strides_A[2]; // Fixed size array
    ck_tile::index_t G_strides_B[2]; // Fixed size array
    ck_tile::index_t G_strides_E[2]; // Fixed size array
    ...
    ck_tile::index_t G_strides_Ds[ActualDTensor][2]; // Always at least [1][2]
...
struct BatchedContractionKernel
{
    ...
    MakeKernelArgs(const BatchedContractionHostArgs<NumDTensor>& host_args)
    {
        ...
        // ✅ Copy data directly into fixed arrays (hardcoded for 2D)
        // Handle case where host has fewer dimensions than our fixed array
        ck_tile::index_t host_dims = static_cast<ck_tile::index_t>(host_args.G_lengths.size());

        // Initialize with safe defaults
        kargs.G_lengths[0] = host_dims > 0 ? host_args.G_lengths[0] : 1;
        kargs.G_lengths[1] = host_dims > 1 ? host_args.G_lengths[1] : 1;

        kargs.G_strides_A[0] = host_dims > 0 ? host_args.G_strides_A[0] : 0;
        kargs.G_strides_A[1] = host_dims > 1 ? host_args.G_strides_A[1] : 0;

        kargs.G_strides_B[0] = host_dims > 0 ? host_args.G_strides_B[0] : 0;
        kargs.G_strides_B[1] = host_dims > 1 ? host_args.G_strides_B[1] : 0;

        kargs.G_strides_E[0] = host_dims > 0 ? host_args.G_strides_E[0] : 0;
        kargs.G_strides_E[1] = host_dims > 1 ? host_args.G_strides_E[1] : 0;
        ...
        {
            // When NumDTensor = 0, initialize the dummy element to avoid garbage
            kargs.G_strides_Ds[0][0] = 0;
            kargs.G_strides_Ds[0][1] = 0;
        }
        ...
        std::cout << "Debug: G_lengths[0]=" << kargs.G_lengths[0]
                  << ", G_lengths[1]=" << kargs.G_lengths[1] << std::endl;
        std::cout << "Debug: G_strides_A[0]=" << kargs.G_strides_A[0]
                  << ", G_strides_A[1]=" << kargs.G_strides_A[1] << std::endl;
    ...
    CK_TILE_DEVICE void operator()(const KernelArgs& kargs) const
    {
        ...
        // ✅ Direct array access - no pointers involved
        const auto g_indices = DecomposeGIndex<NumDimG>(i_batch_flat, kargs.G_lengths);

        // ✅ Direct array access for offset calculation
