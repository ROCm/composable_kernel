...
struct GemmKernel
{
    ...
    CK_TILE_DEVICE static auto MakeGemmPadViews(const TensorView& views)
    {
        ...
        const auto& ds_pad_view_tuple = generate_tuple(ds_pad_view, number<NumDTensor>{});

        return make_tuple(a_pad_view, b_pad_view, ds_pad_view_tuple, c_pad_view);
    }
    ...
    MakeGemmTileWindows(const PadView& views, const index_t i_m, const index_t i_n)
    {
        ...
        const auto ds_block_window = [&](auto i) {
        ...
        const auto& ds_block_window_tuple = generate_tuple(ds_block_window, number<NumDTensor>{});

        return make_tuple(a_block_window, b_block_window, ds_block_window_tuple, c_block_window);
    }
