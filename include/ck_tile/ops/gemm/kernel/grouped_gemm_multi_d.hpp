...
struct GroupedGemmMultiDHostArgs
{
    CK_TILE_HOST GroupedGemmMultiDHostArgs(const void* a_ptr_,
                                           const void* b_ptr_,
                                           const std::array<const void*, 2>& ds_ptr_,
                                           void* e_ptr_,
                                           index_t k_batch_,
                                           index_t M_,
                                           index_t N_,
                                           ...
                                           index_t stride_A_,
                                           index_t stride_B_,
                                           const std::array<index_t, 2>& stride_Ds_,
                                           index_t stride_E_)
        : a_ptr(a_ptr_),
          b_ptr(b_ptr_),
          ds_ptr(ds_ptr_),
    ...
    const void* a_ptr;
    const void* b_ptr;
    const std::array<const void*, 2> ds_ptr;
    union
    {
        void* e_ptr;
        void* c_ptr;
    ...
    index_t stride_A;
    index_t stride_B;
    const std::array<index_t, 2> stride_Ds;
...
struct GemmMultiDTransKernelArg
{
    UniversalGemmKernelArgs<1, 1, 2> group_karg;
    ck_tile::index_t block_start;
    ck_tile::index_t block_end;

    GemmMultiDTransKernelArg() = delete;
    GemmMultiDTransKernelArg(UniversalGemmKernelArgs<1, 1, 2>&& karg,
                             index_t bl_start,
                             index_t bl_end)
...
struct GroupedGemmMultiDKernel
{
    ...
    using Kernel = GroupedGemmMultiDKernel<TilePartitioner, GemmPipeline, EpiloguePipeline>;
    ...
    GetWorkSpaceSize(const std::vector<GroupedGemmMultiDHostArgs>& gemm_descs) -> std::size_t
    {
        return gemm_descs.size() * sizeof(GemmMultiDTransKernelArg);
    }

    CK_TILE_HOST static auto GetWorkSpaceSize(index_t group_count) -> std::size_t
    {
        return group_count * sizeof(GemmMultiDTransKernelArg);
    }
    ...
    CK_TILE_HOST static auto GridSize(const std::vector<GroupedGemmMultiDHostArgs>& gemm_descs)
    {
        index_t grid_size = 0;
    ...
    CK_TILE_HOST static auto MakeKargs(const std::vector<GroupedGemmMultiDHostArgs>& gemm_descs)
        -> std::vector<GemmMultiDTransKernelArg>
    {
        std::vector<GemmMultiDTransKernelArg> gemm_kernel_args_;
        ...
        for(std::size_t i = 0; i < gemm_descs.size(); ++i)
        {
            ...
            auto karg = UniversalGemmKernelArgs<1, 1, 2>{
    ...
    CK_TILE_HOST static bool IsSupportedArgument(const std::vector<GemmMultiDTransKernelArg>& kargs)
    {
    ...
    CK_TILE_DEVICE void Run(const UniversalGemmKernelArgs<1, 1, 2>& kargs,
    ...
    CK_TILE_DEVICE index_t FindGroupId(const GemmMultiDTransKernelArg* gemm_desc_ptr,
                                       index_t block_id,
                                   ...
                                   index_t group_count) const
    {
        ...
        const auto gemm_desc_ptr = reinterpret_cast<const GemmMultiDTransKernelArg*>(
