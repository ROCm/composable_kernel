...
#include "ck_tile/ops/gemm/block/block_gemm_areg_breg_creg_v3_default_policy.hpp"
...
template <typename Problem_,
          typename Policy_      = BlockGemmARegBRegCRegV3DefaultPolicy,
          bool TransposeC_      = false,
          bool BWarpContiguous_ = true>
struct BlockGemmARegBRegCRegV3
{
    ...
    static constexpr bool BWarpContiguous = BWarpContiguous_;
    ...
    CK_TILE_DEVICE static constexpr auto MakeBBlockDistributionEncode()
    {
        if constexpr(UseDefaultScheduler)
        {
            static_assert(BWarpContiguous == true); // TODO: support other cases
    ...
    CK_TILE_DEVICE static constexpr auto MakeCBlockDistributionEncode()
    {
        ...
        {
            using c_distr_hs_n = std::conditional_t<BWarpContiguous,
                                                    sequence<NIterPerWarp, NWarp>,
                                                    sequence<NWarp, NIterPerWarp>>;
            using c_distr_ps_minor =
                std::conditional_t<BWarpContiguous, sequence<1, 1>, sequence<1, 0>>;

            using c_distr_ys_minor =
                std::conditional_t<BWarpContiguous, sequence<0, 0>, sequence<0, 1>>;

            constexpr auto c_block_outer_dstr_encoding =
                tile_distribution_encoding<sequence<>,
                                           tuple<sequence<MIterPerWarp, MWarp>, c_distr_hs_n>,
                                           tuple<sequence<1, 2>>,
                                           tuple<c_distr_ps_minor>,
                                           c_distr_ys_major,
                                           c_distr_ys_minor>{};
            constexpr auto c_block_dstr_encode = detail::make_embed_tile_distribution_encoding(
                c_block_outer_dstr_encoding, typename WarpGemm::CWarpDstrEncoding{});
