...
struct BlockUniversalGemmDequantAsBsCr
{
    private:
    ...
    load_interleaved_pk_type(WarpTile& warp_tile,
                             const WarpWindow& warp_window,
                             [[maybe_unused]] const ComputeDataType& scale)
    {
        ...
        const fp16x2_t scale_x2{scale, scale};
    ...
    struct BlockGemmImpl<GemmPipelineScheduler::Default, GemmTraits>
    {
                                       ...
                                       const ComputeDataType& scale)
        {
            ...
            {
                load_interleaved_pk_type(a_warp_tile_, a_block_window, scale);
            }
            else
            {
            ...
            {
                load_interleaved_pk_type(b_warp_tile_, b_block_window, scale);
            }
            else
            {
    ...
    struct BlockGemmImpl<GemmPipelineScheduler::Interwave, GemmTraits>
    {
                                       ...
                                       const ComputeDataType& scale)
        {
            ...
            static_for<0, KRepeat, 1>{}([&](auto kIter) {
                LocalPrefetch<kIter.value>(a_block_window, b_block_window, scale);
    ...
    CK_TILE_DEVICE void LocalPrefetch(const ASmemBlockWindow& a_block_window,
                                      ...
                                      const ComputeDataType& scale)
    {
        block_gemm_impl_.LocalPrefetch(a_block_window, b_block_window, scale);
    }
    ...
    CK_TILE_DEVICE void operator()(CBlockTensor& c_block_tensor,
                                   ...
                                   const ComputeDataType& scale)
    {
        block_gemm_impl_(c_block_tensor, a_block_window, b_block_window, scale);
    }
