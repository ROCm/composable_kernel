...
// amd_wave_read_first_lane is the SGPR function from AMD GPU device to load 1 or a series of the memory to the SGPR registers.
...
__device__ inline auto amd_wave_read_first_lane(const Object& obj)
{
    ...
    for (size_t offset = 0; offset + SGPR_size <= ObjectSize; offset += SGPR_size) {
        uint32_t read_src;
        __builtin_memcpy(&read_src, src + offset, SGPR_size);
        read_src = __builtin_amdgcn_readfirstlane(read_src);
        __builtin_memcpy(dst + off, &read_src, SGPR_size);
    }
