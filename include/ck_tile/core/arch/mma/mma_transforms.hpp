...
// Gives access to MmaOp for type traits
template <typename MmaOp, typename TransformD = PassThroughTransform>
...
{
    using Pre =
        PreMmaTransform<MmaOp, PassThroughTransform, PassThroughTransform, PassThroughTransform>;

    using Post = PostMmaTransform<MmaOp, PassThroughTransform>;
};
...
struct TransformSelector<MmaOp, std::enable_if_t<mma_traits<MmaOp>::IsWmma && CK_TILE_ARCH_GFX11>>
{
    ...
    using Post = PostMmaTransform<MmaOp, wmma::UnpadTransformGfx11>;
};

// Can implement more TransformSelector specializations here for other architectures or
// situations...
