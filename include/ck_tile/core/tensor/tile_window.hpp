        ...
        : pre_computed_coords_{}
    {
        ...
        const auto partition_index = [&]() {
            if constexpr(Base::BottomTensorView::buffer_view::get_address_space() ==
                ...
                return detail::get_partition_index(this->tile_dstr_, bool_constant<false>{});
            else
                return detail::get_partition_index(this->tile_dstr_, bool_constant<true>{});
        }();

        const auto window_adaptor_thread_coord_tmp = make_tensor_adaptor_coordinate(
            tile_distribution.get_ps_ys_to_xs_adaptor(),
            container_concat(partition_index, array<index_t, Base::NDimY>{0}));
