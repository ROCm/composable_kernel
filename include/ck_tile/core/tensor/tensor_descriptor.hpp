                                    ...
                                    number<GuaranteedLastDimensionVectorLength> = number<-1>{})
{
    ...
    const auto element_space_size = container_reduce(lengths, multiplies<void, void>{}, long_number<1>{});
        ...
        [&](auto i) {
            ...
            else
            {
                return container_reduce(
                    lengths, multiplies<void, void>{}, number<stride_n_minus_2>{}, i + I1, number<N - 1>{}, I1);
            }
        },
        number<N>{});
