...
// TODO: consider CK_TILE_ARITHMETIC_USING_FLOAT
...
CK_TILE_HOST_DEVICE constexpr pk_fp4_t fp16x2_to_pk_fp4(const fp16x2_t& x) {
    ...
    return pk_fp4_t::pack(float_to_e2m1(type_convert<float>(x[0])), float_to_e2m1(type_convert<float>(x[1])));
#endif
}
...
#else
    return fp32x2_t{convert_to_float<pk_fp4_t>(unpack(number<0>{})), convert_to_float<pk_fp4_t>(unpack(number<1>{}))};
#endif
