...
struct merge_v2_magic_division : public base_transform<LowLengths::size(), 1>
{
    ...
    using UpLengths =
        decltype(make_tuple(container_reduce(LowLengths{}, multiplies<void, void>{}, number<1>{})));
    ...
    CK_TILE_HOST_DEVICE constexpr merge_v2_magic_division(const LowLengths& low_lengths)
        : low_lengths_{low_lengths},
          low_lengths_magic_divisor_{generate_tuple(
              ...
              number<NDimLow>{})},
          up_lengths_{make_tuple(container_reduce(low_lengths, multiplies<void, void>{}, I1))}
    {
...
struct merge_v3_division_mod : public base_transform<LowLengths::size(), 1>
{
    ...
    using LowLengthsScan =
        decltype(container_reverse_exclusive_scan(LowLengths{}, multiplies<void, void>{}, number<1>{}));
    ...
    CK_TILE_HOST_DEVICE constexpr merge_v3_division_mod(const LowLengths& low_lengths)
        : low_lengths_{low_lengths},
          low_lengths_scan_{
              container_reverse_exclusive_scan(low_lengths, multiplies<void, void>{}, number<1>{})},
          up_lengths_{make_tuple(container_reduce(low_lengths, multiplies<void, void>{}, number<1>{}))}
    {
        static_assert(LowerIndex::size() == NDimLow, "wrong!");
    }
...
struct unmerge : public base_transform<1, UpLengths::size()>
{
    ...
    using UpLengthsScan =
        decltype(container_reverse_exclusive_scan(UpLengths{}, multiplies<void, void>{}, number<1>{}));
    ...
    CK_TILE_HOST_DEVICE constexpr unmerge(const UpLengths& up_lengths)
        : up_lengths_{up_lengths},
          up_lengths_scan_{container_reverse_exclusive_scan(up_lengths, multiplies<void, void>{}, number<1>{})}
    {
    }
