...
CK_TILE_HOST void reference_softmax(HostTensorView<const ADataType> a_m_n,
                                    HostTensorView<BDataType> b_m_n)
{
    auto f = [&](auto m) {
    ...
    make_ParallelTensorFunctor(f, b_m_n.get_lengths()[0])(std::thread::hardware_concurrency());
}
} // namespace ck_tile
