                                            ...
                                            const ACCElementOp& acc_element_op = {})
{
    ...
    constexpr index_t PackedSize = std::is_same_v<BDataType, pk_fp4_t> ? 2 : 1;

    auto f_mn = [&](auto m, auto n) {
        AccDataType v_acc  = 0;
        AccDataType pasual = 0;
        for(std::size_t k = 0; k < (K / PackedSize); k++)
        {
            using ComputeType = float;
            auto b_scale = type_convert<int32_t>(q((PackedSize * k) / QuantGroupSize::kK, n)) - 127;
