...
// todo refactor it so it will look more like bwd weight
// removing for from kernel will probably be needed so all data will be processed at one go
// because current logic expects seperate launches for every example in batch
// add combined groups logic after calculating it on paper
template <
    index_t NDimSpatial,
    ...
    bool DoPadGemmM,
    bool DoPadGemmN>
struct TransformConvBwdDataToGemmV2
{
                                  ...
                                  const std::array<index_t, NDimSpatial + 3>& out_g_n_k_wos_strides)
{
    ...
    if constexpr(is_same_v<ALayout, tensor_layout::convolution::NDHWGK>)
    {
    ...
    const index_t K, const index_t Z, const index_t Y, const index_t X, const index_t C)
{
    ...
    if constexpr(is_same_v<BLayout, tensor_layout::convolution::GKZYXC>)
    {
...
template <index_t NDim, typename enable_if<NDim == 2, bool>::type = false>
constexpr auto make_wei_grid_desc(
    const index_t K, const index_t Y, const index_t X, const index_t C)
{
                                 ...
                                 const std::array<index_t, NDimSpatial + 3>& in_g_n_c_wis_strides)
{
    ...
    if constexpr(is_same_v<CLayout, tensor_layout::convolution::GNDHWC> ||
                                 ...
                                 const std::array<index_t, NDimSpatial + 3>& in_g_n_c_wis_strides)
{
    ...
    // refactor this into MakeABCDescriptor, similar to as in /root/workspace/composable_kernel/include/ck/tensor_operation/operator_transform/transform_conv_bwd_weight_to_gemm_v2.hpp
