...
struct GridwiseGemmConvFwdPreshuffleMultipleD_xdl_cshuffle
{
    ...
    // only full parts of C dim are processed, so f.e. for 
    // KPerBlock = 64, X = 3 and Y = 3, only X * Y(KPerBlock / (X * Y)), 
    // so 63 elements would be processed, that should be supported inside block gemm
    static constexpr auto CSlice = Number<KPerBlock / (Y * X)>{};
    ...
    static constexpr auto PaddedCSlice = Number<8>{}; // hardcore for now, should enable even elem distribution between all threads
    ...
    // Assume we are processing whole parts of W from image, so f.e. for resnet case 1x128x56x56
    // WSlice must be 56, and HSlice must be Y or higher. In case of H/W Padding the LDS1 memory must
    // be bigger
    __host__ __device__ static constexpr auto GetABlockDescriptor_CSlice_HSlice_WSlice()
    {
    ...
    __host__ __device__ static auto GetABlockDescriptor_A_NHoWoCYX()
    {
        ...
        constexpr auto nchw_slice_desc = make_naive_tensor_descriptor(
            make_tuple(NSlice, CSlice, HSlice, WSlice),
            make_tuple(CSlice * HSlice * WSlice, HSlice * WSlice, WSlice, I1));
        ...
        constexpr auto nhowocyx_desc = transform_tensor_descriptor(
                    nchw_padded_slice_desc,
                    make_tuple(make_pass_through_transform(NSlice),
                               make_pass_through_transform(CSlice),
                               make_embed_transform(make_tuple(HoutSlice, Y),
                                                    make_tuple(ConvStrideH, ConvDilationH)),
                               make_embed_transform(make_tuple(WoutSlice, X),
                                                    make_tuple(ConvStrideW, ConvDilationW))),
                    make_tuple(Sequence<0>{}, Sequence<1>{}, Sequence<2>{}, Sequence<3>{}),
    ...
    __host__ __device__ static constexpr index_t GetSharedMemoryNumberOfByte()
    {
        ...
        constexpr auto a_block1_space_size_aligned = math::integer_least_multiple(
            a_block_desc_chw_slice.GetElementSpaceSize(), max_lds_align);

        constexpr auto a_block2_space_size_aligned = math::integer_least_multiple(
        ...
        return math::max(a_block1_space_size_aligned * sizeof(AComputeDataType) +
                         a_block2_space_size_aligned * sizeof(AComputeDataType) +
                               ...
                               const Block2ETileMap& block_2_etile_map)
    {
        const auto a_grid_buf = make_dynamic_buffer<AddressSpaceEnum::Global>(
            p_a_grid, a_grid_desc_nchw.GetElementSpaceSize());
        ...
        const index_t m_block_data_idx_on_grid_nchw = __builtin_amdgcn_readfirstlane(block_work_idx[I0] * HoutSlice);
        ...
        // A matrix in LDS memory, dst of blockwise copy
        constexpr auto a_block2_desc_ak0_m_ak1 = GetABlockDescriptor_AK0PerBlock_MPerBlock_AK1();

        // B matrix in LDS memory, dst of blockwise copy
        constexpr auto b_block_desc_bk0_n_bk1 = GetBBlockDescriptor_BK0PerBlock_NPerBlock_BK1();


//         // Step1: Take C_slice, H_slice, W_slice from NCHW tensor    
...
//         // then change blockwise gemm to use TrueKPerBlock
        ...
        auto a_global_into_lds1_blockwise_copy =
            ThreadGroupTensorSliceTransfer_v4r1<ThisThreadBlock,
                                                ...
                                                Sequence<1, 8, 4, 2>,//ABlockTransferThreadClusterLengths_AK0_M_AK1, // should be <1, 8, 4, 2>
                ...
                a_grid_desc_nchw,
                make_multi_index(0, 0, m_block_data_idx_on_grid_nchw, 0), // shift!
                a_element_op,
                a_block1_desc_cslice_hslice_wslice,
                make_multi_index(0, 0, 0, 0),
                ck::tensor_operation::element_wise::PassThrough{});

        a_global_into_lds1_blockwise_copy.RunRead(a_grid_desc_nchw, a_grid_buf);

        auto a_lds1_into_lds2_blockwise_copy =
            ThreadGroupTensorSliceTransfer_v4r1<ThisThreadBlock,
                                                AElementwiseOperation,
                                                ck::tensor_operation::element_wise::PassThrough,
                                                InMemoryDataOperationEnum::Set,
                                                ...
                                                2,
                                                ABlockTransferSrcScalarPerVector, // probably 1
                                                ABlockTransferDstScalarPerVector_AK1,
                                                1,
                                                1,
        ...
        // LDS allocation for A and B: be careful of alignment
        constexpr auto a_block1_space_size_aligned = math::integer_least_multiple(
            a_block1_desc_cslice_hslice_wslice.GetElementSpaceSize(), max_lds_align);

        constexpr auto a_block2_space_size_aligned = math::integer_least_multiple(
            a_block2_desc_ak0_m_ak1.GetElementSpaceSize(), max_lds_align);

        auto a_block_buf1 = make_dynamic_buffer<AddressSpaceEnum::Lds>(
            static_cast<AComputeDataType*>(p_shared),
            a_block1_desc_cslice_hslice_wslice.GetElementSpaceSize());

        auto a_block_buf2 = make_dynamic_buffer<AddressSpaceEnum::Lds>(
            static_cast<AComputeDataType*>(p_shared) + + a_block1_space_size_aligned, 
            a_block2_desc_ak0_m_ak1.GetElementSpaceSize());

        auto b_block_buf = make_dynamic_buffer<AddressSpaceEnum::Lds>(
            static_cast<BComputeDataType*>(p_shared) + a_block1_space_size_aligned + a_block2_space_size_aligned,
        ...
        gridwise_gemm_pipeline.template Run<HasMainKBlockLoop>(a_grid_desc_nchw, // global
                                                               ...
                                                               a_global_into_lds1_blockwise_copy,
                                                               a_lds1_into_lds2_blockwise_copy,
                                                               a_grid_buf,
                                                               a_block_buf1,
                                                               a_block_buf2,
