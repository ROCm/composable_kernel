...
#include "ck/tensor_operation/gpu/block/blockwise_gemm_pipeline_xdlops_mx_b_preshuffle_selector.hpp"
...
{
#if(!defined(__HIP_DEVICE_COMPILE__) || defined(__gfx9__) || true) // FF_DBG
...
template <typename ALayout,
          ...
          index_t MPerBlock,
          index_t NPerBlock,
          index_t KPerBlock,    // 256  FF_DBG
          index_t AK1Value,     // 16   FF_DBG
          index_t BK1Value,
          index_t MPerXdl,
          index_t NPerXdl,
          index_t MXdlPerWave,
          ...
          bool PermuteA = false,
          bool PermuteB = false>
struct GridwiseGemmMX_xdl_cshuffle_bpreshuffle_v3
{
    ...
    using mfma_selector = MfmaSelector<ComputeTypeA, MPerXdl, NPerXdl, ComputeTypeB, is_single_rate_mfma, is_scale_mfma>;
    ...
    struct Argument : public tensor_operation::device::BaseArgument, public Problem
    {
        __host__ Argument(const ADataType* p_a_grid_,
                          ...
                          ck::e8m0_bexp_t*  dbg_f8_   = nullptr,
                          ck::f4x2_pk_t*    dbg_f4pk_ = nullptr)
            : Problem{M_,
                      N_,
                      K_,
                      StrideA_,
        ...
        int*                dbg_i32;
        float*              dbg_f32;
        ck::e8m0_bexp_t*    dbg_f8;
        ck::f4x2_pk_t*      dbg_f4pk;
    };
    ...
    __device__ static constexpr auto GetBShflBlockDescriptor_BK0PerBlock_NPerBlock_BK1()
    {
    ...
    /*
    // reference block pipeline
    using LDSTypeA  = ADataType;
    using LDSTypeB  = BDataType;
    using BlockwiseFlatmmPipe =
        remove_cvref_t<decltype(BlockGemmBPreshufflePipeline_Selector<
                                ...
                                decltype(GetABlockDescriptor_AK0PerBlock_MPerBlock_AK1()),
                                decltype(GetBBlockDescriptor_BK0PerBlock_NPerBlock_BK1()),
                                decltype(MakeAMmaTileDescriptor_M0_M1_M2_K(GetABlockDescriptor_AK0PerBlock_MPerBlock_AK1())),
                                decltype(MakeBMmaTileDescriptor_N0_N1_N2_K(GetBBlockDescriptor_BK0PerBlock_NPerBlock_BK1())),
                                ABlockTransferSrcScalarPerVector,
                                BBlockTransferSrcScalarPerVector,
                                MPerBlock,
                                NPerBlock,
                                   ...
                                   c_grid_desc_mblock_mperblock_nblock_nperblock)
    {
        ...
        ck::e8m0_bexp_t*    dbg_f8      = arg->dbg_f8;
        ck::f4x2_pk_t*      dbg_f4pk    = arg->dbg_f4pk;
        ...
        //dbg_f32[gid * FF_DBG_CNT] = gid * 1.0f;
#endif
...
#if 1 // FF_DBG
        // B **********************************************************************************
#pragma region DBG_FLAT_B        
        const auto b_grid_buf_shfl = make_dynamic_buffer<AddressSpaceEnum::Global>(p_b_grid, b_grid_desc_bpreshuffled.GetElementSpaceSize());
        
        constexpr auto b_shfl_block_desc_bk0_n_bk1 = GetBShflBlockDescriptor_BK0PerBlock_NPerBlock_BK1();

        // B matrix threadwise copy, using threadwiseTensorSliceTransfer_v2
        auto b_block_buf_shfl = make_static_buffer<AddressSpaceEnum::Vgpr, BDataType>(b_shfl_block_desc_bk0_n_bk1.GetElementSpaceSize());

        auto b_blockwise_copy_shfl =
            ThreadwiseTensorSliceTransfer_v2<BDataType,
                                            BDataType,
                                            decltype(b_grid_desc_bpreshuffled),
                                            decltype(b_shfl_block_desc_bk0_n_bk1),
        ...
        constexpr index_t NRepeat = NXdlPerWave;
        static constexpr auto b_thread_desc_ = make_naive_tensor_descriptor_packed(make_tuple(Number<NRepeat>{}, I1, Number<KRepeat>{}, Number<KPack>{}));
        auto b_thread_buf = make_static_buffer<AddressSpaceEnum::Vgpr, BDataType>(b_thread_desc_.GetElementSpaceSize());
        StaticallyIndexedArray<decltype(b_thread_buf), Number<2>{}> b_thread_bufs;
        constexpr auto b_block_origin_idx = make_tuple(I0, I0, I0, I0);
        
        b_blockwise_copy_shfl.Run(
            b_grid_desc_bpreshuffled,
            b_grid_buf_shfl,
            b_shfl_block_desc_bk0_n_bk1,
            b_block_origin_idx,
            b_thread_buf);

        dbg_i32[gid * FF_DBG_CNT + 0] = thd_id;
        dbg_i32[gid * FF_DBG_CNT + 1] = b_grid_desc_bpreshuffled.GetElementSpaceSize();
        dbg_i32[gid * FF_DBG_CNT + 2] = b_shfl_block_desc_bk0_n_bk1.GetElementSpaceSize();
        dbg_i32[gid * FF_DBG_CNT + 3] = b_thread_desc_.GetElementSpaceSize();

        dbg_f4pk[gid * FF_DBG_CNT + 0] = b_thread_buf[Number<0>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 1] = b_thread_buf[Number<1>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 2] = b_thread_buf[Number<2>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 3] = b_thread_buf[Number<3>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 4] = b_thread_buf[Number<4>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 5] = b_thread_buf[Number<5>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 6] = b_thread_buf[Number<6>{}];
        dbg_f4pk[gid * FF_DBG_CNT + 7] = b_thread_buf[Number<7>{}];
#pragma endregion DBG_FLAT_B

        return;
        // Scale A ****************************************************************************
#pragma region DBG_SCALE_A        
        ...
        // Prefetch a_scales
        constexpr ck::index_t MPerXDL = MPerXdl;
        constexpr ck::index_t MRepeat = MXdlPerWave;
        constexpr ck::index_t KRepeat = BlockwiseGemmPipe::Base::KRepeat;
        constexpr ck::index_t MWaves = BlockwiseGemmPipe::Base::MWaves;
        constexpr ck::index_t ScalesPerKBlockSize = BlockwiseGemmPipe::ScalesPerKBlockSize;
        constexpr ck::index_t ScalesPerXdlopsRunPerThread = BlockwiseGemmPipe::ScalesPerXdlopsRunPerThread;
        auto xdlops_gemm = BlockwiseGemmPipe::xdlops_gemm;
        auto a_scale_grid_desc = a_scale_grid_desc_am_ak;
        auto a_scale_thread_desc = BlockwiseGemmPipe::a_scale_thread_desc;
        auto a_scale_thread_desc_copy = BlockwiseGemmPipe::a_scale_thread_desc_copy;
        ...
        static_for<0, MRepeat, 1>{}([&](auto m0) {
            static_for<0, KRepeat, 1>{}([&](auto k0) {
                static_for<0, ScalesPerXdlopsRunPerThread, 1>{}([&](auto s) {
                    ...
                    a_scale_thread_buf(Number<a_scale_offset>{}) = a_scale_thread_buf_copy[Number<0>{}];
        ...
        dbg_i32[gid * FF_DBG_CNT + 0] = thd_id;
        dbg_i32[gid * FF_DBG_CNT + 1] = MRepeat;
        dbg_i32[gid * FF_DBG_CNT + 2] = KRepeat;
        dbg_i32[gid * FF_DBG_CNT + 3] = ScalesPerXdlopsRunPerThread;
        dbg_i32[gid * FF_DBG_CNT + 4] = a_scale_thread_desc.GetElementSpaceSize();

        dbg_f8[gid * FF_DBG_CNT + 0] = a_scale_thread_buf[Number<0>{}];
        dbg_f8[gid * FF_DBG_CNT + 1] = a_scale_thread_buf[Number<1>{}];
        dbg_f8[gid * FF_DBG_CNT + 2] = a_scale_thread_buf[Number<2>{}];
        dbg_f8[gid * FF_DBG_CNT + 3] = a_scale_thread_buf[Number<3>{}];
        dbg_f8[gid * FF_DBG_CNT + 4] = a_scale_thread_buf[Number<4>{}];
        dbg_f8[gid * FF_DBG_CNT + 5] = a_scale_thread_buf[Number<5>{}];
        dbg_f8[gid * FF_DBG_CNT + 6] = a_scale_thread_buf[Number<6>{}];
        dbg_f8[gid * FF_DBG_CNT + 7] = a_scale_thread_buf[Number<7>{}];

        // restore row id and advance to the next set of scales
        a_scale_thread_copy.MoveSrcSliceWindow(a_scale_grid_desc,
                                               make_multi_index(-MPerBlock, ScalesPerKBlockSize));
#pragma endregion DBG_SCALE_A                                               
#endif
                               ...
                               const Problem& problem)
    {
        ...
        //dbg_i32[gid * FF_DBG_CNT + 0] = thd_id;
        //dbg_f32[gid * FF_DBG_CNT + 1] = gid * 1.0f;
#endif
