...
struct GridwiseGemmSoftmax_k0mk1_k0nk1_mn_xdl_cshuffle_v1
{
    ...
    using ThreadReduceSrcDesc_M_N = decltype(make_naive_tensor_descriptor_packed(
        make_tuple(Number<MThreadSliceSize>{}, Number<NThreadSliceSize>{})));
    using ThreadReduceDstDesc_M =
        decltype(make_naive_tensor_descriptor_packed(make_tuple(Number<MThreadSliceSize>{})));

    using BlockwiseMaxReduce = PartitionedBlockwiseReduction<FloatReduceAcc,
                                                             BlockSize,
    ...
    using ThreadwiseMaxReduce = ThreadwiseReduction<FloatReduceAcc,
                                                    ThreadReduceSrcDesc_M_N,
                               ...
                               const Block2CTileMap& block_2_ctile_map)
    {
        ...
        auto d_grid_buf = make_dynamic_buffer<AddressSpaceEnum::Global>(
            p_d_grid, c_grid_desc_mblock_mperblock_nblock_nperblock.GetElementSpaceSize());
        ...
        // shuffle C + softmax + write out
        {
            ...
            //     (CShuffleMXdlPerWavePerShuffle * MWave * MPerXdl) / MThreadClusterSize;
            ...
            static_for<0, num_access, 1>{}([&](auto access_id) {
                ...
                // TODO - extract following into softmax_blockwise
                {
                    threadwise_src_load.Run(c_reduce_block_desc_mperblock_nperblock,
                                            c_shuffle_block_buf,
                                            thread_buffer_desc,
                                            make_tuple(I0, I0),
                                            in_thread_buf);
                    ...
                    static_for<0, MThreadSliceSize, 1>{}([&](auto iM) {
                        ...
                        static_for<0, NThreadSliceSize, 1>{}([&](auto iN) {
                            ...
                            out_thread_buf(Number<offset>{}) =
                                alpha * in_thread_buf(Number<offset>{}) / accu_value_buf(iM);
                        });
                    });
