...
struct GridwiseGemmMultiD_xdl_cshuffle_v3
{
        ...
        index_t K, index_t KPad, index_t N, index_t NPad, index_t StrideB, index_t BK0)
    {
        ...
        //     }
        // }();
        const auto b_grid_desc_nraw_kraw_shuffle_n0_k0_k1_n1_k2 = [&]() {
            if constexpr(is_same<tensor_layout::gemm::ColumnMajor, BLayout>::value)
            {
                return make_naive_tensor_descriptor(make_tuple(N, BK0Number, BK1Number),
                                                    make_tuple(StrideB, BK1Number, I1));
            }
        }();

        const auto b_grid_desc_nraw_kraw_n_k = [&]() {
            if constexpr(is_same<tensor_layout::gemm::ColumnMajor, BLayout>::value)
            {
                return make_naive_tensor_descriptor(make_tuple(N, BK0Number, BK1Number),
                                                    make_tuple(StrideB, BK1Number, I1));
