...
struct TriAdd
{
    ...
    template <>
    __host__ __device__ constexpr void
    operator()<float>(float& y, const float& x0, const float& x1, const float& x2) const
    {
        y = x0 + x1 + x2;
    };
    ...
    template <>
    __host__ __device__ constexpr void
    operator()<double>(double& y, const double& x0, const double& x1, const double& x2) const
    {
        y = x0 + x1 + x2;
    };
    ...
    template <>
    __host__ __device__ constexpr void
    operator()<float>(float& y, const float& x0, const half_t& x1, const float& x2) const
    {
        y = x0 + type_convert<half_t>(x1) + x2;
    };

    template <>
    __host__ __device__ constexpr void
    operator()<half_t>(half_t& y, const float& x0, const half_t& x1, const half_t& x2) const
    {
        y = type_convert<half_t>(x0) + x1 + x2;
    };

    template <>
    __host__ __device__ constexpr void
    operator()<half_t>(half_t& y, const half_t& x0, const half_t& x1, const half_t& x2) const
    {
        y = x0 + x1 + x2;
    };
    ...
    template <>
    __host__ __device__ constexpr void
    operator()<bhalf_t>(bhalf_t& y, const bhalf_t& x0, const bhalf_t& x1, const bhalf_t& x2) const
    {
        const float x1_tmp = ck::type_convert<float>(x0);
        const float x2_tmp = ck::type_convert<float>(x1);
        const float x3_tmp = ck::type_convert<float>(x2);
        const float y_tmp  = x1_tmp + x2_tmp + x3_tmp;
        y                  = ck::type_convert<bhalf_t>(y_tmp);
    }

    template <>
    __host__ __device__ constexpr void
    operator()<int8_t>(int8_t& y, const int8_t& x0, const int8_t& x1, const int8_t& x2) const
    {
        y = x0 + x1 + x2;
    };
};
...
struct TriSubtract
{
    template <typename T>
    __host__ __device__ constexpr void
    operator()(T& y, const T& x0, const T& x1, const T& x2) const;
...
struct TriBilinear
{
    ...
    {
        y = alpha_ * x0 + beta_ * x1 + gamma_ * x2;
    };
    ...
    {
        y = type_convert<half_t>(alpha_ * x0 + beta_ * ck::type_convert<float>(x1) + gamma_ * x2);
    };
...
struct TriAddRelu
{
    ...
    __host__ __device__ constexpr void operator()<float, float, float, float>(float& y,
                                                                              ...
                                                                              const float& x2) const
    {
        const float a = x0 + x1 + x2;
        y             = a > 0.0f ? a : 0.0f;
    };

    template <>
    __host__ __device__ constexpr void operator()<double, double, double, double>(
        double& y, const double& x0, const double& x1, const double& x2) const
    {
        const double a = x0 + x1 + x2;
        y              = a > 0.0 ? a : 0.0;
    };
    ...
    template <>
    __host__ __device__ constexpr void operator()<float, float, half_t, float>(
        float& y, const float& x0, const half_t& x1, const float& x2) const
    {
        const float a = x0 + type_convert<float>(x1) + x2;
        y             = a > 0.0f ? a : 0.0f;
    };
};
