...
__device__ inline bhalf4_t i4_to_bhalf4_scale_truncate(int q, const ck::bhalf2_t& scale)
{
    ...
    res.template AsType<bhalf_t>()(Number<1>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n4 - 8) * s1);
    res.template AsType<bhalf_t>()(Number<2>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n1 - 8) * s0);
    res.template AsType<bhalf_t>()(Number<3>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n5 - 8) * s1);
                                                          ...
                                                          const ck::bhalf2_t& scaled_zp)
{
    ...
    vector_type<bhalf_t, 4> res;
    res.template AsType<bhalf_t>()(Number<0>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n0 - 8) * s0 - z0);
    res.template AsType<bhalf_t>()(Number<1>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n4 - 8) * s1 - z1);
    res.template AsType<bhalf_t>()(Number<2>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n1 - 8) * s0 - z0);
    res.template AsType<bhalf_t>()(Number<3>{}) =
        fp32_to_bhalf_truncate(static_cast<float>(n5 - 8) * s1 - z1);
...
struct DequantPack8WithZpTruncate
{
                                                  ...
                                                  const ck::bhalf2_t& scaled_zp) const
    {
        ...
        static_for<0, 4, 1>{}([&](auto i) {
            ...
            const float v1 = type_convert<float>(v_v.template AsType<bhalf_t>()[Number<1>{}]);
            vector_type<bhalf_t, 2> r;
            r.template AsType<bhalf_t>()(Number<0>{}) = fp32_to_bhalf_truncate(v0 * s0 - z0);
            r.template AsType<bhalf_t>()(Number<1>{}) = fp32_to_bhalf_truncate(v1 * s1 - z1);
            dst.template AsType<bhalf2_t>()(i)        = r.template AsType<bhalf2_t>()[Number<0>{}];
        });
...
// AIESW-32282: trait that maps a "B element-op" carrier type passed at the
// device-op surface to the (sym, asym) pair of element-ops actually invoked
// inside the threadwise transfer. The CK W4A16 b_scale path compiles BOTH
// the sym (3-arg) and asym (4-arg) dequant branches in the same pipeline
// instance (the BZeroPointStruct dispatch is `if constexpr` but both
// branches must compile), so we need two distinct types with the matching
// arity. The carrier passed via the device-op's `BElementwiseOperation`
// template slot identifies the asym variant; the trait derives the matching
// sym counterpart at compile time. Callers that don't care (the legacy
// PassThrough convention) get `void` for both slots — the threadwise
// transfer's own defaults (DequantPack8 / DequantPack8WithZp) kick in and
// the generated code is bit-identical to the pre-AIESW-32282 build.
template <typename T>
struct DequantPolicyFor
{
    using sym_type  = void;
    using asym_type = void;
};
template <>
struct DequantPolicyFor<DequantPack8WithZp>
{
