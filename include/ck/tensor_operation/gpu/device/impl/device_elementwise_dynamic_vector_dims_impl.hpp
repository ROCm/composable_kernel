                                 ...
                                 const std::array<index_t, NumDim>& desc_strides)
{
    ...
    static auto GenerateInOutGridDescTuple()
    {
        ...
        return generate_tuple(
            [&](auto) {
                return MakeDescriptor(ones, ones, ones, ones);
            },
            Number<NumTensors>{});
    };
    ...
    struct Invoker : public BaseInvoker
    {
        float Run(const Argument& arg, const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            const bool in_out_same_vector_dim = GetLowestStrideDim(arg.inStridesArray_[0]) == GetLowestStrideDim(arg.outStridesArray_[0]);

            const auto kernel = in_out_same_vector_dim ? kernel_elementwise<GridwiseElementwiseOpSameInOutVectorDim,
                                                                    InGridDescTuple,
                                                                    OutGridDescTuple,
                                                                    InDataTypePointerTuple,
                                                                    OutDataTypePointerTuple,
