...
#include "ck/tensor_operation/gpu/device/device_gemm_multiple_d_scale_ab.hpp"
...
struct DeviceGemmMultipleDScaleAB_Xdl_CShuffle
    : public DeviceGemmMultipleDScaleAB<ALayout,
                                        BLayout,
                                        DsLayout,
                                        ELayout,
                                        ADataType,
                                        ...
                                        CDEElementwiseOperation>
{
    ...
    struct Reduce2D
    {
        ...
        static constexpr bool PropagateNan         = true;
        ...
        float Run(const std::array<index_t, 2> arrInLengths_1,
                  const std::array<index_t, 2> arrInStrides_1,
                  const void* p_input,
                  void* p_workpace,
                  void* p_output,
                  const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            std::array<index_t, 1> arrOutStrides{1};
            ...
            {
                std::cout << "The runtime parameters seems not supported by the DeviceReduce "
                             "instance, exiting!"
                          << std::endl;
            };
            ...
            float kern_time = 0;
            kern_time += invoker_ptr_1->Run(argument_ptr_1.get(), stream_config);
            kern_time += invoker_ptr_2->Run(argument_ptr_2.get(), stream_config);
            return kern_time;
        }
    };
    ...
    struct Invoker : public BaseInvoker
    {
        ...
        float Run(const Argument& arg, const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            {
                kern_time += launch_kernel(integral_constant<bool, true>{});
            }
            else
            {
                kern_time += launch_kernel(integral_constant<bool, false>{});
            }
    ...
    std::string GetTypeString() const override
    {
        ...
        // clang-format off
        str << "DeviceGemmMultipleDScaleAB_Xdl_CShuffle"
            << "<"
            << BlockSize << ", "
            << MPerBlock << ", "
            << NPerBlock << ", "
