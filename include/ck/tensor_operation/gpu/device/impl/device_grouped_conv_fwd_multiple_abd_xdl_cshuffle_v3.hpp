...
__launch_bounds__(CK_MAX_THREAD_PER_BLOCK, MinimumOccupancy)
    ...
    kernel_grouped_conv_fwd_xdl_cshuffle_v3(typename GridwiseGemm::Argument karg,
                                            ...
                                            const index_t num_group)
{
#if defined(__gfx9__)
    // offset base pointer for each work-group
    const index_t k_idx = __builtin_amdgcn_readfirstlane(blockIdx.z * num_k_per_block);
    const index_t g_idx = __builtin_amdgcn_readfirstlane(blockIdx.y % num_group);
    const index_t n_idx = __builtin_amdgcn_readfirstlane(blockIdx.y / num_group);
                                             ...
                                             BComputeDataType>
{
    ...
    using GridwiseGemmTwoStage = GridwiseGemmMultiD_xdl_cshuffle_v3<
        tensor_layout::gemm::RowMajor,
        tensor_layout::gemm::ColumnMajor,
        DsLayout,
        tensor_layout::gemm::RowMajor,
        ...
        BDataType,
        AccDataType,
        AccDataType, // CShuffleDataType in AccDataType for two stage case
        DsDataType,
        AccDataType, // EDataType in AccDataType for two stage case
        AElementwiseOperation,
        BElementwiseOperation,
        CDEElementwiseOperation,
        GemmSpec,
    ...
    using GridwiseElementwiseOutputCast =
        GridwiseElementwise<Tuple<EGridDesc_M_N>,
                            Tuple<EGridDesc_M_N>,
    ...
    struct Argument : public BaseArgument
    {
        ...
        EGridDesc_M_N ce_elementwise_grid_desc_m_n_;
                      ...
                      const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            const auto Run = [&](const auto& kernel) {
                if(stream_config.flush_cache)
                {
                    ...
                    ck::utility::RotatingMemWrapper<typename GridwiseGemmType::Argument> rotating_mem(
                        gemm_arg_,
            ...
            if(has_main_k_block_loop)
            {
                ...
                {
                    const auto kernel =
                        kernel_grouped_conv_fwd_xdl_cshuffle_v3<GridwiseGemmType,
                                                                ComputePtrOffset,
                ...
                else if constexpr(BlkGemmPipelineVer == BlockGemmPipelineVersion::v2)
                {
                    ...
                    else if(GridwiseGemmType::CalculateKBlockLoopTailNum(K_split) == TailNumber::Full)
                    {
                        const auto kernel =
                            kernel_grouped_conv_fwd_xdl_cshuffle_v3<GridwiseGemmType,
                                                                    ComputePtrOffset,
                    ...
                    {
                        if(GridwiseGemmType::CalculateKBlockLoopTailNum(K_split) == TailNumber::Three)
                        {
                            const auto kernel = kernel_grouped_conv_fwd_xdl_cshuffle_v3<
                                GridwiseGemmType,
                                ComputePtrOffset,
                    ...
                    {
                        if(GridwiseGemmType::CalculateKBlockLoopTailNum(K_split) == TailNumber::Four)
                        {
                            const auto kernel = kernel_grouped_conv_fwd_xdl_cshuffle_v3<
                                GridwiseGemmType,
                                ComputePtrOffset,
                    ...
                    {
                        if(GridwiseGemmType::CalculateKBlockLoopTailNum(K_split) == TailNumber::Five)
                        {
                            const auto kernel = kernel_grouped_conv_fwd_xdl_cshuffle_v3<
                                GridwiseGemmType,
                                ComputePtrOffset,
                    ...
                    {
                        if(GridwiseGemmType::CalculateKBlockLoopTailNum(K_split) == TailNumber::Seven)
                        {
                            const auto kernel = kernel_grouped_conv_fwd_xdl_cshuffle_v3<
                                GridwiseGemmType,
                                ComputePtrOffset,
