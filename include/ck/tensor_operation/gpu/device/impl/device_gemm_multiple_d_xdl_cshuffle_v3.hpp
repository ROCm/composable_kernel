                                                                           ...
                                                                           CElementwiseOperation>
{
    ...
    static constexpr bool IsHWSupportAtomicAdd =
        (CDEShuffleBlockTransferScalarPerVectors{}[I0] > 1) || (sizeof(CDataType) > 2);
    ...
    struct Invoker : public BaseInvoker
    {
        float Run(const Argument& arg, const StreamConfig& stream_config = StreamConfig{})
        {
            ...
            {
                throw std::runtime_error("wrong! AtomicAdd is not valid for SplitK");
            }
            ...
            if(has_main_k_block_loop)
            {
                             ...
                             BlkGemmPipelineVer == BlockGemmPipelineVersion::v3)
                {
                    if(arg.KBatch > 1)
                    {
                        if constexpr(IsHWSupportAtomicAdd)
                        {
