...
class Task(object):
    ...
    def __init__(
        ...
        self.log_file = Task._logname(
            self.output_dir, self.test_binary, test_name, self.execution_number
        )
    ...
    def _logname(output_dir, test_binary, test_name, execution_number):
        ...
        if output_dir is None:
            (log_handle, log_name) = tempfile.mkstemp(
                prefix="gtest_parallel_", suffix=".log"
            )
            os.close(log_handle)
            return log_name
    ...
    def run(self, timeout_per_test, gpu_id=None):
        begin = time.time()
        with open(self.log_file, "w") as log:
            # Set up environment with GPU assignment if specified
            env = os.environ.copy()
            if gpu_id is not None:
                env["HIP_VISIBLE_DEVICES"] = str(gpu_id)

            # Get the absolute path to the test binary and its directory
            # This handles both relative and absolute paths correctly
            abs_test_binary = os.path.abspath(self.test_binary)
            test_binary_dir = os.path.dirname(abs_test_binary)

            # Update the test command to use the absolute path
            abs_test_command = [abs_test_binary] + self.test_command[1:]

            task = subprocess.Popen(
                abs_test_command, stdout=log, stderr=log, env=env, cwd=test_binary_dir
            )
            try:
...
class TaskManager(object):
    ...
    def register_exit(self, task):
        ...
        self.times.record_test_time(
            task.test_binary, task.test_name, task.last_execution_time
        )
        if self.test_results:
            self.test_results.log(
                task.test_name, task.runtime_ms / 1000.0, task.exit_code
            )
    ...
    def run_task(self, task, timeout_per_test, gpu_id=None):
        for try_number in range(self.times_to_retry + 1):
            self.__register_start(task)
            task.run(timeout_per_test, gpu_id)
        ...
        for task_key in sorted(stats, key=stats.__getitem__):
            ...
            self.out.permanent_line(
                ...
                % (
                    ...
                    else (" (%d interrupted)" % num_interrupted),
                )
            )
...
class TestTimes(object):
    ...
    def __init__(self, save_file):
        ...
        for (test_binary, test_name), runtime in times.items():
            if (
                type(test_binary) is not str
                or type(test_name) is not str
...
def find_tests(binaries, additional_args, options, times):
    ...
    for test_binary in binaries:
        ...
        # Get absolute path and directory for the test binary
        abs_test_binary = os.path.abspath(test_binary)
        test_binary_dir = os.path.dirname(abs_test_binary)

        # Create list command with absolute path
        abs_list_command = [abs_test_binary] + additional_args + ["--gtest_list_tests"]
        if options.gtest_also_run_disabled_tests:
            abs_list_command += ["--gtest_also_run_disabled_tests"]
        if options.gtest_filter != "":
            abs_list_command += ["--gtest_filter=" + options.gtest_filter]

        try:
            # Run the list command from the binary's directory so relative paths work
            test_list = subprocess.check_output(
                abs_list_command, stderr=subprocess.STDOUT, cwd=test_binary_dir
            )
...
def execute_tasks(
    ...
    class WorkerFn(object):
        def __init__(self, tasks, running_groups, timeout_per_test, available_gpus):
            self.tasks = tasks
        ...
        def __call__(self):
            while True:
                ...
                with self.task_lock:
                    for task_id in range(len(self.tasks)):
                        ...
                        if self.available_gpus:
                            gpu_id = self.available_gpus[
                                self.task_counter % len(self.available_gpus)
                            ]
                            self.task_counter += 1
                ...
                task_manager.run_task(task, self.timeout_per_test, gpu_id)
    ...
    try:
        ...
        worker_fn = WorkerFn(tasks, running_groups, timeout_per_test, available_gpus)
...
def default_options_parser():
    ...
    parser.add_option(
        "--gtest_color", type="string", default="yes", help="color output"
    )
    ...
    parser.add_option(
        ...
        type="int",
        default=None,
        help="Interrupt all remaining processes after the given time (in seconds).",
    )
    parser.add_option(
        "--timeout_per_test",
        type="int",
        default=None,
        help="Interrupt single processes after the given time (in seconds).",
    )
    parser.add_option(
        "--serialize_test_cases",
        action="store_true",
        default=False,
        help="Do not run tests from the same test case in parallel.",
    )
    return parser
...
def main():
    ...
    if options.output_dir is not None and not os.path.isdir(options.output_dir):
        parser.error(
            "--output_dir value must be an existing directory, "
    ...
    if options.shard_count < 1:
        parser.error(
            "Invalid number of shards: %d. Must be at least 1." % options.shard_count
        )
    if not (0 <= options.shard_index < options.shard_count):
        parser.error(
            "Invalid shard index: %d. Must be between 0 and %d "
    ...
    task_manager = TaskManager(
        times, logger, test_results, Task, options.retry_failed, options.repeat + 1
    )

    # Get available GPUs based on HIP_VISIBLE_DEVICES and --gpus option
    available_gpus = get_available_gpus(options.gpus) if options.gpus > 0 else None
    ...
    if options.repeat > 1 and (task_manager.failed or task_manager.started):
        logger.summarize(
            task_manager.passed, task_manager.failed, task_manager.started.values()
        )
