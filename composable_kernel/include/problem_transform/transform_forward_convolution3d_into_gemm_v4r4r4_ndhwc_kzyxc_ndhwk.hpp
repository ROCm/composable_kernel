...
template <typename... In,
          ...
          index_t GemmK1Value>
__host__ __device__ constexpr auto
transform_forward_convolution3d_into_gemm_v4r4r4_nhwc_kyxc_nhwk_pad_splitN_v2(
    const TensorDescriptor<In...>& in_n_di_hi_wi_c_grid_desc,
    const TensorDescriptor<Wei...>& wei_k_z_y_x_c_grid_desc,
    const TensorDescriptor<Out...>& out_n_do_ho_wo_k_grid_desc,
    const ConvStrides& conv_strides,
...
// N1 = 2;
    ...
    const auto in_n_dip_hip_wip_c_grid_desc = transform_tensor_descriptor(
        in_n_di_hi_wi_c_grid_desc,
        make_tuple(make_pass_through_transform(N1),
                   make_pad_transform(Di, InLeftPadD, InRightPadD),
                   make_pad_transform(Hi, InLeftPadH, InRightPadH),
                   make_pad_transform(Wi, InLeftPadW, InRightPadW),
                   make_pass_through_transform(C)),
    ...
    // C: output tensor
    const auto out_gemmm_gemmn_grid_desc = transform_tensor_descriptor(
        make_naive_tensor_descriptor_packed<true>(make_tuple(N1 * Do * Ho * Wo, K)),
        make_tuple(make_pass_through_transform(N1 * Do * Ho * Wo),
                   make_pass_through_transform(K)),
        make_tuple(Sequence<0>{}, Sequence<1>{}),
        make_tuple(Sequence<0>{}, Sequence<1>{}));
