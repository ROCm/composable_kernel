...
#include "transform_forward_convolution3d_into_gemm_v4r4r4_nhwc_kyxc_nhwk.hpp"
...
extern "C" __global__ void convolution3d_forward_implicit_gemm_v4r4_xdlops_nhwc_kyxc_nhwk_prepare(
    int n,
    int di,
    int hi,
    int wi,
    ...
    void* p_c_blockid_to_m0_n0_block_cluster_adaptor)
{
    ...
    const index_t do = (di + leftPadD + rightPadD - convDilationZ * (z - 1) - 1) / convStrideD + 1;
    ...
    const auto descs = transform_forward_convolution3d_into_gemm_v4r4r4_nhwc_kyxc_nhwk_pad(
        ...
        make_tuple(convStrideD, convStrideH, convStrideW),
        make_tuple(convDilationZ, convDilationY, convDilationX),
        make_tuple(leftPadD, leftPadH, leftPadW),
        make_tuple(rightPadD, rightPadH, rightPadW),
        Number<K1>{});
...
#endif
        convolution3d_forward_implicit_gemm_v4r4_xdlops_nhwc_kyxc_nhwk(
            ...
            const void CONSTANT* p_c_blockid_to_m0_n0_block_cluster_adaptor)
{
    ...
    constexpr auto in_n_di_hi_wi_c_desc =
        make_naive_tensor_descriptor_packed(make_tuple(256, 28, 28, 28, 256));
    constexpr auto wei_k_z_y_x_c_desc =
        make_naive_tensor_descriptor_packed(make_tuple(256, 3, 3, 3, 256));
    constexpr auto out_n_do_ho_wo_k_desc =
        make_naive_tensor_descriptor_packed(make_tuple(256, 28, 28, 28, 256));
