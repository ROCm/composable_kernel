            ...
            parallel
            {
                ...
                stage("Build CK and run Tests on Navi")
                {
                    ...
                    environment{
                        ...
                        execute_args = """ cd ../client_example && rm -rf build && mkdir build && cd build && cmake -D CMAKE_PREFIX_PATH="${env.WORKSPACE}/install;/opt/rocm" -DCMAKE_CXX_FLAGS=" --offload-arch=gfx1030 --offload-arch=gfx1100 --offload-arch=gfx1101" -D CMAKE_CXX_COMPILER="${build_compiler()}" .. && make -j """
